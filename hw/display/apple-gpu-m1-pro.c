/*
 * Apple M1 Pro GPU (16-core, unified memory, integrated)
...
#define GPU_VENDOR_ID  0x106B
#define GPU_DEVICE_ID  0xA002
#define GPU_SUBSYS_VID 0x106B
#define GPU_SUBSYS_DID 0xA002
#define GPU_REVISION   0xA1
#define GPU_CLASS      0x0300
#define GPU_MEM_MHZ    1296
#define NV_BAR0_SIZE (16*MiB)
#define NV_BAR1_SIZE (64*MiB)
#define NV_BAR3_SIZE (8*MiB)
#define NV_BAR5_SIZE (64*KiB)
#define GPU_PMC_BOOT_0  0x61000002
...
#define GPU_CLK_BASE    1296
#define GPU_CLK_BOOST   1296
...
#define GPU_VRAM_MB     16

#define TYPE_APPLE_GPU_M1_PRO "apple-gpu-m1-pro"
OBJECT_DECLARE_SIMPLE_TYPE(AppleGpuM1ProState, APPLE_GPU_M1_PRO)
struct AppleGpuM1ProState {
...
static uint32_t gpu_clk(AppleGpuM1ProState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AppleGpuM1ProState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AppleGpuM1ProState *s=(AppleGpuM1ProState*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"apple-gpu-m1-pro-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&bops,s,"apple-gpu-m1-pro-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&bops,s,"apple-gpu-m1-pro-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&bops,s,"apple-gpu-m1-pro-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_apple_gpu_m1_pro[]={DEFINE_PROP_UINT32("gpu-count",AppleGpuM1ProState,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",AppleGpuM1ProState,gpu_name)};
static const VMStateDescription vms_apple_gpu_m1_pro={.name="apple-gpu-m1-pro",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AppleGpuM1ProState),VMSTATE_UINT32(clock_mhz,AppleGpuM1ProState),VMSTATE_UINT64(clock_last_ns,AppleGpuM1ProState),VMSTATE_UINT32(gpu_count,AppleGpuM1ProState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0xA002;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="Apple M1 Pro GPU (16-core, unified memory, integrated)"; dc->vmsd=&vms_apple_gpu_m1_pro; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_apple_gpu_m1_pro);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_apple_gpu_m1_pro={.name=TYPE_APPLE_GPU_M1_PRO,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AppleGpuM1ProState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_apple_gpu_m1_pro(void){type_register_static(&ti_apple_gpu_m1_pro);}
type_init(reg_apple_gpu_m1_pro)
