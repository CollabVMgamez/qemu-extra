/*
 * ARM Mali-G78 MP24 (Exynos 2100, integrated)
...
#define GPU_DEVICE_ID  0xB078
#define GPU_SUBSYS_VID 0x13B5
#define GPU_SUBSYS_DID 0xB078
...
#define GPU_PMC_BOOT_0  0x13B50004
...
#define TYPE_MALI_G78 "mali-g78"
OBJECT_DECLARE_SIMPLE_TYPE(MaliG78State, MALI_G78)
struct MaliG78State {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    MaliG78State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    ...
    memory_region_init_io(&s->bar1,OBJECT(s),&bops,s,"mali-g78-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&bops,s,"mali-g78-ramin",NV_BAR3_SIZE);
...
static void ci(ObjectClass *k, const void *d) {
    ...
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0xB078;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="ARM Mali-G78 MP24 (Exynos 2100, integrated)"; dc->vmsd=&vms_mali_g78; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_mali_g78);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_mali_g78={.name=TYPE_MALI_G78,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(MaliG78State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_mali_g78(void){type_register_static(&ti_mali_g78);}
type_init(reg_mali_g78)
