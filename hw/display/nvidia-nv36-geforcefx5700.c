/*
 * NVIDIA GeForce FX 5700 (NV36, AGP 8x, 128MB DDR, 2004)
...
#define GPU_DEVICE_ID  0x0341
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x0341
#define GPU_REVISION   0x00
#define GPU_CLASS      0x0300
#define GPU_MEM_MHZ    425
...
#define GPU_CLK_BASE    425
#define GPU_CLK_BOOST   425
#define AGP_CAP_OFFSET  0x40

#define TYPE_NVIDIA_NV36_GEFORCEFX5700 "nvidia-nv36-geforcefx5700"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaNv36Geforcefx5700State, NVIDIA_NV36_GEFORCEFX5700)
struct NvidiaNv36Geforcefx5700State {
...
static uint32_t gpu_clk(NvidiaNv36Geforcefx5700State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaNv36Geforcefx5700State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaNv36Geforcefx5700State *s=(NvidiaNv36Geforcefx5700State*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-nv36-geforcefx5700-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b135ops,s,"nvidia-nv36-geforcefx5700-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b135ops,s,"nvidia-nv36-geforcefx5700-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b135ops,s,"nvidia-nv36-geforcefx5700-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_nvidia_nv36_geforcefx5700[]={DEFINE_PROP_UINT32("gpu-count",NvidiaNv36Geforcefx5700State,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",NvidiaNv36Geforcefx5700State,gpu_name),
    DEFINE_PROP_STRING("board-partner",NvidiaNv36Geforcefx5700State,board_partner)};
static const VMStateDescription vms_nvidia_nv36_geforcefx5700={.name="nvidia-nv36-geforcefx5700",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaNv36Geforcefx5700State),VMSTATE_UINT32(intr_en,NvidiaNv36Geforcefx5700State),VMSTATE_UINT32(clock_mhz,NvidiaNv36Geforcefx5700State),VMSTATE_UINT64(clock_last_ns,NvidiaNv36Geforcefx5700State),
        VMSTATE_UINT32(gpu_count,NvidiaNv36Geforcefx5700State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x0341;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA GeForce FX 5700 (NV36, AGP 8x, 128MB DDR, 2004)"; dc->vmsd=&vms_nvidia_nv36_geforcefx5700; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_nvidia_nv36_geforcefx5700);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_nv36_geforcefx5700={.name=TYPE_NVIDIA_NV36_GEFORCEFX5700,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaNv36Geforcefx5700State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_nv36_geforcefx5700(void){type_register_static(&ti_nvidia_nv36_geforcefx5700);}
type_init(reg_nvidia_nv36_geforcefx5700)
