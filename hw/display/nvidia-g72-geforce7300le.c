/*
 * NVIDIA GeForce 7300 LE (G72, AGP 8x via HSI, 128MB DDR2, 2006)
...
#define GPU_DEVICE_ID  0x01D1
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x01D1
...
#define GPU_PMC_BOOT_0  0x0000000B
...
#define TYPE_NVIDIA_G72_GEFORCE7300LE "nvidia-g72-geforce7300le"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaG72Geforce7300leState, NVIDIA_G72_GEFORCE7300LE)
struct NvidiaG72Geforce7300leState {
...
static uint32_t gpu_clk(NvidiaG72Geforce7300leState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaG72Geforce7300leState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaG72Geforce7300leState *s=(NvidiaG72Geforce7300leState*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-g72-geforce7300le-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b135ops,s,"nvidia-g72-geforce7300le-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b135ops,s,"nvidia-g72-geforce7300le-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b135ops,s,"nvidia-g72-geforce7300le-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_nvidia_g72_geforce7300le[]={DEFINE_PROP_UINT32("gpu-count",NvidiaG72Geforce7300leState,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",NvidiaG72Geforce7300leState,gpu_name),
    DEFINE_PROP_STRING("board-partner",NvidiaG72Geforce7300leState,board_partner)};
static const VMStateDescription vms_nvidia_g72_geforce7300le={.name="nvidia-g72-geforce7300le",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaG72Geforce7300leState),VMSTATE_UINT32(intr_en,NvidiaG72Geforce7300leState),VMSTATE_UINT32(clock_mhz,NvidiaG72Geforce7300leState),VMSTATE_UINT64(clock_last_ns,NvidiaG72Geforce7300leState),
        VMSTATE_UINT32(gpu_count,NvidiaG72Geforce7300leState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x01D1;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA GeForce 7300 LE (G72, AGP 8x via HSI, 128MB DDR2, 2006)"; dc->vmsd=&vms_nvidia_g72_geforce7300le; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_nvidia_g72_geforce7300le);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_g72_geforce7300le={.name=TYPE_NVIDIA_G72_GEFORCE7300LE,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaG72Geforce7300leState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_g72_geforce7300le(void){type_register_static(&ti_nvidia_g72_geforce7300le);}
type_init(reg_nvidia_g72_geforce7300le)
