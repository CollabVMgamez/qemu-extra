/*
 * ATI Radeon HD 3650 (RV635, PCIe, 512MB DDR2, 2008)
...
#define GPU_DEVICE_ID  0x9598
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x9598
...
#define TYPE_ATI_RADEON_HD3650 "ati-radeon-hd3650"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeonHd3650State, ATI_RADEON_HD3650)
struct AtiRadeonHd3650State {
...
static uint32_t gpu_clk(AtiRadeonHd3650State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeonHd3650State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeonHd3650State *s=(AtiRadeonHd3650State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-hd3650-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-hd3650-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-hd3650-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-hd3650-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_hd3650={.name="ati-radeon-hd3650",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeonHd3650State),VMSTATE_UINT32(intr_en,AtiRadeonHd3650State),VMSTATE_UINT32(clock_mhz,AtiRadeonHd3650State),VMSTATE_UINT64(clock_last_ns,AtiRadeonHd3650State),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x9598;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="ATI Radeon HD 3650 (RV635, PCIe, 512MB DDR2, 2008)"; dc->vmsd=&vms_ati_radeon_hd3650; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRadeonHd3650State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_radeon_hd3650={.name=TYPE_ATI_RADEON_HD3650,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRadeonHd3650State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_radeon_hd3650(void){type_register_static(&ti_ati_radeon_hd3650);}
type_init(reg_ati_radeon_hd3650)
