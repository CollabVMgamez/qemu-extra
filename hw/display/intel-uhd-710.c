/*
 * Intel UHD Graphics 710 (Alder Lake-S GT1, 16EU)
 * Vendor: 0x8086, Device: 0x4690
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-710
...
#define GPU_DEV      0x4690
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2290
#define GPU_REV      0x0C
...
#define TYPE_INTELUHD710 "intel-uhd-710"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhd710State, INTELUHD710)
struct IntelUhd710State {
...
static uint32_t igpu_clk(IntelUhd710State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhd710State *s = INTELUHD710(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-710-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-710-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhd710State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-710",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhd710State),VMSTATE_UINT32(clock_mhz,IntelUhd710State),VMSTATE_UINT64(clock_last_ns,IntelUhd710State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics 710 (Alder Lake-S GT1, 16EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHD710,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhd710State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
