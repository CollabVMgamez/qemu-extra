/*
 * Intel HD Graphics 540 (Skylake GT2, 24EU)
 * Vendor: 0x8086, Device: 0x191B
 * Class: 0x0300, PCIe
 * Usage: -device intel-hd-540
...
#define GPU_DEV      0x191B
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x221B
#define GPU_REV      0x06
...
#define TYPE_INTELHD540 "intel-hd-540"
OBJECT_DECLARE_SIMPLE_TYPE(IntelHd540State, INTELHD540)
struct IntelHd540State {
...
static uint32_t igpu_clk(IntelHd540State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelHd540State *s = INTELHD540(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-hd-540-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-hd-540-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelHd540State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-hd-540",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelHd540State),VMSTATE_UINT32(clock_mhz,IntelHd540State),VMSTATE_UINT64(clock_last_ns,IntelHd540State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel HD Graphics 540 (Skylake GT2, 24EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELHD540,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelHd540State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
