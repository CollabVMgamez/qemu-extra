/* Rockchip Mali-T860 MP4 (RK3399)
 * Vendor: 0x1D87, Device: 0x8600
 * Usage: -device rockchip-mali-t860
...
#define GPU_DEV 0x8600
#define GPU_SS_V 0x1D87
#define GPU_SS_D 0x0001
#define GPU_CLASS 0x0302
#define BAR0_SIZE (4*MiB)
#define BAR1_SIZE (64*MiB)
#define TYPE_ROCKCHIP_MALI_T860 "rockchip-mali-t860"
OBJECT_DECLARE_SIMPLE_TYPE(RockchipMaliT860State, ROCKCHIP_MALI_T860)
struct RockchipMaliT860State { PCIDevice parent_obj; MemoryRegion bar0,bar1; uint32_t clock_mhz; uint64_t clock_last_ns; char *gpu_name; };
...
static void realize(PCIDevice*p,Error**e){{
    RockchipMaliT860State*s=(RockchipMaliT860State*)p; s->clock_mhz=0;s->clock_last_ns=0;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0,s,"rockchip-mali-t860-mmio",BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1,s,"rockchip-mali-t860-vram",BAR1_SIZE);
    pci_register_bar(p,2,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
}}
static const Property props[]={DEFINE_PROP_STRING("gpu-name",RockchipMaliT860State,gpu_name),};
static const VMStateDescription vms={.name="rockchip-mali-t860",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,RockchipMaliT860State),VMSTATE_UINT32(clock_mhz,RockchipMaliT860State),VMSTATE_UINT64(clock_last_ns,RockchipMaliT860State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass*k,const void*d){DeviceClass*dc=DEVICE_CLASS(k);PCIDeviceClass*pc=PCI_DEVICE_CLASS(k);pc->realize=realize;pc->vendor_id=GPU_VID;pc->device_id=GPU_DEV;pc->class_id=GPU_CLASS;pc->revision=0;dc->desc="Rockchip Mali-T860 MP4 (RK3399)";dc->vmsd=&vms;dc->hotpluggable=false;device_class_set_props(dc,props);set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);}
static const TypeInfo ti={.name=TYPE_ROCKCHIP_MALI_T860,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(RockchipMaliT860State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
