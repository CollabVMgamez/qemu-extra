/*
 * NVIDIA GeForce GTX 1060 Mobile (GP106, Pascal, Mobile)
...
#define GPU_DEVICE_ID  0x1C20
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x1108
...
#define GPU_CLK_BASE    1404
#define GPU_MEM_MHZ     2002
#define GPU_CLK_BOOST   1671
#define GPU_PCIE_GEN    3
#define TYPE_NVIDIA_GTX1060M "nvidia-gtx1060m"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx1060mState, NVIDIA_GTX1060M)
struct NvidiaGtx1060mState {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaGtx1060mState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaGtx1060mState *s=(NvidiaGtx1060mState*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-gtx1060m-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-gtx1060m-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-gtx1060m-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-gtx1060m-vgaio",NV_BAR5_SIZE);
...
static const Property gpu_multi_props_NvidiaGtx1060mState[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaGtx1060mState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaGtx1060mState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaGtx1060mState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="NVIDIA GeForce GTX 1060 Mobile (GP106, Pascal, Mobile)"; dc->vmsd=&vms_nvidia_gtx1060m; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaGtx1060mState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_gtx1060m={.name=TYPE_NVIDIA_GTX1060M,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaGtx1060mState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_gtx1060m(void){type_register_static(&ti_nvidia_gtx1060m);}
type_init(reg_nvidia_gtx1060m)
