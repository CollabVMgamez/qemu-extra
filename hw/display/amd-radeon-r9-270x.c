/*
 * AMD Radeon R9 270X (Curacao, PCIe x16, 2GB GDDR5, 2013)
...
#define GPU_DEVICE_ID  0x6810
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x6810
...
#define TYPE_AMD_RADEON_R9_270X "amd-radeon-r9-270x"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRadeonR9270xState, AMD_RADEON_R9_270X)
struct AmdRadeonR9270xState {
...
static uint32_t gpu_clk(AmdRadeonR9270xState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRadeonR9270xState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRadeonR9270xState *s=(AmdRadeonR9270xState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-radeon-r9-270x-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-radeon-r9-270x-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-radeon-r9-270x-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-radeon-r9-270x-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_radeon_r9_270x={.name="amd-radeon-r9-270x",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRadeonR9270xState),VMSTATE_UINT32(intr_en,AmdRadeonR9270xState),VMSTATE_UINT32(clock_mhz,AmdRadeonR9270xState),VMSTATE_UINT64(clock_last_ns,AmdRadeonR9270xState),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x6810;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="AMD Radeon R9 270X (Curacao, PCIe x16, 2GB GDDR5, 2013)"; dc->vmsd=&vms_amd_radeon_r9_270x; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRadeonR9270xState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_radeon_r9_270x={.name=TYPE_AMD_RADEON_R9_270X,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRadeonR9270xState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_radeon_r9_270x(void){type_register_static(&ti_amd_radeon_r9_270x);}
type_init(reg_amd_radeon_r9_270x)
