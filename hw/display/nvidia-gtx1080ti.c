/*
 * NVIDIA GeForce GTX 1080 Ti (GP102, Pascal)
...
#define GPU_DEVICE_ID  0x1B06
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x1106
...
#define GPU_PFB_PARTS   0x0000000B
#define GPU_PFB_REFCTRL 0x80000063
#define VPD_CAP_OFFSET 0x70
#define GPU_CLK_BASE    1480
#define GPU_MEM_MHZ     2750
#define GPU_CLK_BOOST   1582
#define GPU_PCIE_GEN    3
#define TYPE_NVIDIA_GTX1080TI "nvidia-gtx1080ti"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx1080tiState, NVIDIA_GTX1080TI)
struct NvidiaGtx1080tiState {
...
static uint32_t gpu_clk(NvidiaGtx1080tiState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaGtx1080tiState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaGtx1080tiState *s=(NvidiaGtx1080tiState*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-gtx1080ti-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-gtx1080ti-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-gtx1080ti-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-gtx1080ti-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_gtx1080ti={.name="nvidia-gtx1080ti",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaGtx1080tiState),VMSTATE_UINT32(intr_en,NvidiaGtx1080tiState),VMSTATE_UINT32(pfifo_intr_en,NvidiaGtx1080tiState),VMSTATE_UINT32(clock_mhz,NvidiaGtx1080tiState),VMSTATE_UINT64(clock_last_ns,NvidiaGtx1080tiState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaGtx1080tiState[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaGtx1080tiState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaGtx1080tiState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaGtx1080tiState, board_partner),
};
...
static void reg_nvidia_gtx1080ti(void){type_register_static(&ti_nvidia_gtx1080ti);}
type_init(reg_nvidia_gtx1080ti)
