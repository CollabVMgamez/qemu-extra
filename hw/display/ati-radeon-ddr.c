/*
 * ATI Radeon DDR (R100, AGP, 32MB DDR, 2000)
...
#define TYPE_ATI_RADEON_DDR "ati-radeon-ddr"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeonDdrState, ATI_RADEON_DDR)
struct AtiRadeonDdrState {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeonDdrState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeonDdrState *s=(AtiRadeonDdrState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-ddr-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-ddr-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-ddr-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-ddr-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_ddr={.name="ati-radeon-ddr",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeonDdrState),VMSTATE_UINT32(intr_en,AtiRadeonDdrState),VMSTATE_UINT32(clock_mhz,AtiRadeonDdrState),VMSTATE_UINT64(clock_last_ns,AtiRadeonDdrState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeonDdrState[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeonDdrState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeonDdrState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeonDdrState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="ATI Radeon DDR (R100, AGP, 32MB DDR, 2000)"; dc->vmsd=&vms_ati_radeon_ddr; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRadeonDdrState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_radeon_ddr={.name=TYPE_ATI_RADEON_DDR,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRadeonDdrState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_radeon_ddr(void){type_register_static(&ti_ati_radeon_ddr);}
type_init(reg_ati_radeon_ddr)
