/*
 * Intel Arc Pro A40 (ACM-G11, 75W, 4GB)
 * Vendor: 0x8086, Device: 0x56BC
 * Class: 0x0302, PCIe
 * Usage: -device intel-arc-pro-a40
...
#define GPU_DEV      0x56BC
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x22BC
#define GPU_REV      0x05
...
#define TYPE_INTELARCPROA40 "intel-arc-pro-a40"
OBJECT_DECLARE_SIMPLE_TYPE(IntelArcProA40State, INTELARCPROA40)
struct IntelArcProA40State {
...
static uint32_t igpu_clk(IntelArcProA40State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelArcProA40State *s = INTELARCPROA40(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-arc-pro-a40-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-arc-pro-a40-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelArcProA40State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-arc-pro-a40",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelArcProA40State),VMSTATE_UINT32(clock_mhz,IntelArcProA40State),VMSTATE_UINT64(clock_last_ns,IntelArcProA40State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Arc Pro A40 (ACM-G11, 75W, 4GB)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELARCPROA40,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelArcProA40State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
