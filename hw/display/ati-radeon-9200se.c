/*
 * ATI Radeon 9200 SE (RV280, AGP 8x, 64MB DDR, 2003)
...
#define GPU_DEVICE_ID  0x5964
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x5964
...
#define GPU_MEM_MHZ     166
#define GPU_CLK_BOOST   200
#define GPU_PCIE_GEN    1
#define GPU_VRAM_MB     64
#define TYPE_ATI_RADEON_9200SE "ati-radeon-9200se"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeon9200seState, ATI_RADEON_9200SE)
struct AtiRadeon9200seState {
...
static uint32_t gpu_clk(AtiRadeon9200seState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeon9200seState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeon9200seState *s=(AtiRadeon9200seState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-9200se-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-9200se-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-9200se-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-9200se-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_9200se={.name="ati-radeon-9200se",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeon9200seState),VMSTATE_UINT32(intr_en,AtiRadeon9200seState),VMSTATE_UINT32(clock_mhz,AtiRadeon9200seState),VMSTATE_UINT64(clock_last_ns,AtiRadeon9200seState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeon9200seState[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeon9200seState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeon9200seState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeon9200seState, board_partner),
};
...
static void reg_ati_radeon_9200se(void){type_register_static(&ti_ati_radeon_9200se);}
type_init(reg_ati_radeon_9200se)
