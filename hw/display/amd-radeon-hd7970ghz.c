/*
 * AMD Radeon HD 7970 GHz (Tahiti, PCIe x16, 3GB GDDR5, 2012)
...
#define GPU_DEVICE_ID  0x6798
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x6799
...
#define GPU_VRAM_MB     3072
#define TYPE_AMD_RADEON_HD7970GHZ "amd-radeon-hd7970ghz"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRadeonHd7970ghzState, AMD_RADEON_HD7970GHZ)
struct AmdRadeonHd7970ghzState {
...
static uint32_t gpu_clk(AmdRadeonHd7970ghzState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRadeonHd7970ghzState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRadeonHd7970ghzState *s=(AmdRadeonHd7970ghzState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-radeon-hd7970ghz-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-radeon-hd7970ghz-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-radeon-hd7970ghz-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-radeon-hd7970ghz-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_radeon_hd7970ghz={.name="amd-radeon-hd7970ghz",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRadeonHd7970ghzState),VMSTATE_UINT32(intr_en,AmdRadeonHd7970ghzState),VMSTATE_UINT32(clock_mhz,AmdRadeonHd7970ghzState),VMSTATE_UINT64(clock_last_ns,AmdRadeonHd7970ghzState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AmdRadeonHd7970ghzState[] = {
    DEFINE_PROP_STRING("gpu-name", AmdRadeonHd7970ghzState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AmdRadeonHd7970ghzState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AmdRadeonHd7970ghzState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x6798;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="AMD Radeon HD 7970 GHz (Tahiti, PCIe x16, 3GB GDDR5, 2012)"; dc->vmsd=&vms_amd_radeon_hd7970ghz; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRadeonHd7970ghzState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_radeon_hd7970ghz={.name=TYPE_AMD_RADEON_HD7970GHZ,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRadeonHd7970ghzState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_radeon_hd7970ghz(void){type_register_static(&ti_amd_radeon_hd7970ghz);}
type_init(reg_amd_radeon_hd7970ghz)
