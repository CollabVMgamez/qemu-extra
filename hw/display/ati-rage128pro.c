/*
 * ATI Rage 128 Pro (AGP 4x, 32MB SDR, 1999)
...
#define GPU_DEVICE_ID  0x5046
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x5046
...
#define TYPE_ATI_RAGE128PRO "ati-rage128pro"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRage128proState, ATI_RAGE128PRO)
struct AtiRage128proState {
...
static uint32_t gpu_clk(AtiRage128proState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRage128proState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRage128proState *s=(AtiRage128proState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-rage128pro-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-rage128pro-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-rage128pro-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-rage128pro-vgaio",NV_BAR5_SIZE);
...
static void reg_ati_rage128pro(void){type_register_static(&ti_ati_rage128pro);}
type_init(reg_ati_rage128pro)
