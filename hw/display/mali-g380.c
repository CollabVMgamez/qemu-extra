/* ARM Mali-G380 (IoT/embedded)
 * Vendor: 0x13B5, Device: 0x8380
 * Usage: -device mali-g380
...
#define GPU_DEV 0x8380
#define GPU_SS_V 0x13B5
#define GPU_SS_D 0x0013
#define GPU_CLASS 0x0302
#define BAR0_SIZE (4*MiB)
#define BAR1_SIZE (64*MiB)
#define TYPE_MALI_G380 "mali-g380"
OBJECT_DECLARE_SIMPLE_TYPE(MaliG380State, MALI_G380)
struct MaliG380State { PCIDevice parent_obj; MemoryRegion bar0,bar1; uint32_t clock_mhz; uint64_t clock_last_ns; char *gpu_name; };
...
static void realize(PCIDevice*p,Error**e){
    MaliG380State*s=(MaliG380State*)p; s->clock_mhz=0;s->clock_last_ns=0;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0,s,"mali-g380-mmio",BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1,s,"mali-g380-vram",BAR1_SIZE);
    pci_register_bar(p,2,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
}
static const Property props[]={DEFINE_PROP_STRING("gpu-name",MaliG380State,gpu_name),};
static const VMStateDescription vms={.name="mali-g380",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,MaliG380State),VMSTATE_UINT32(clock_mhz,MaliG380State),VMSTATE_UINT64(clock_last_ns,MaliG380State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass*k,const void*d){DeviceClass*dc=DEVICE_CLASS(k);PCIDeviceClass*pc=PCI_DEVICE_CLASS(k);pc->realize=realize;pc->vendor_id=GPU_VID;pc->device_id=GPU_DEV;pc->class_id=GPU_CLASS;pc->revision=0;dc->desc="ARM Mali-G380 (IoT/embedded)";dc->vmsd=&vms;dc->hotpluggable=false;device_class_set_props(dc,props);set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);}
static const TypeInfo ti={.name=TYPE_MALI_G380,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(MaliG380State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
