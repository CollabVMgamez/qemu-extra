/*
 * NVIDIA L40S (AD102, Ada Lovelace, 48GB GDDR6X)
...
#define GPU_DEVICE_ID  0x26B9
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x17AF
...
#define GPU_PFB_BOOT_0  0x00000016
#define GPU_PFB_PARTS   0x0000000C
#define GPU_PFB_REFCTRL 0x800000A3
#define VPD_CAP_OFFSET 0x70
#define GPU_CLK_BASE    1065
#define GPU_MEM_MHZ     1750
#define GPU_CLK_BOOST   2520
#define GPU_PCIE_GEN    4
#define TYPE_NVIDIA_L40S "nvidia-l40s"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaL40sState, NVIDIA_L40S)
struct NvidiaL40sState {
...
static uint32_t gpu_clk(NvidiaL40sState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaL40sState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaL40sState *s=(NvidiaL40sState*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-l40s-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-l40s-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-l40s-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-l40s-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_l40s={.name="nvidia-l40s",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaL40sState),VMSTATE_UINT32(intr_en,NvidiaL40sState),VMSTATE_UINT32(pfifo_intr_en,NvidiaL40sState),VMSTATE_UINT32(clock_mhz,NvidiaL40sState),VMSTATE_UINT64(clock_last_ns,NvidiaL40sState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaL40sState[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaL40sState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaL40sState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaL40sState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x26B9;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA L40S (AD102, Ada Lovelace, 48GB GDDR6X)"; dc->vmsd=&vms_nvidia_l40s; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaL40sState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_l40s={.name=TYPE_NVIDIA_L40S,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaL40sState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_l40s(void){type_register_static(&ti_nvidia_l40s);}
type_init(reg_nvidia_l40s)
