/*
 * AMD Radeon R7 260X (Bonaire, PCIe x16, 2GB GDDR5, 2013)
...
#define GPU_DEVICE_ID  0x6658
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x6658
...
#define TYPE_AMD_RADEON_R7_260X "amd-radeon-r7-260x"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRadeonR7260xState, AMD_RADEON_R7_260X)
struct AmdRadeonR7260xState {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRadeonR7260xState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRadeonR7260xState *s=(AmdRadeonR7260xState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-radeon-r7-260x-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-radeon-r7-260x-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-radeon-r7-260x-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-radeon-r7-260x-vgaio",NV_BAR5_SIZE);
...
static void reg_amd_radeon_r7_260x(void){type_register_static(&ti_amd_radeon_r7_260x);}
type_init(reg_amd_radeon_r7_260x)
