/*
 * Intel GNA 1.0 (Tiger Lake Neural Accelerator)
 * Vendor: 0x8086, Device: 0x9A19
 * Class: 0x1180 (Processing Accelerator)
 * Usage: -device intel-gna-1-0
...
#define GPU_DEV      0x9A19
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2219
#define GPU_REV      0x00
#define GPU_CLASS    0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_INTELGNA10 "intel-gna-1-0"
OBJECT_DECLARE_SIMPLE_TYPE(IntelGna10State, INTELGNA10)
struct IntelGna10State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    IntelGna10State *s = INTELGNA10(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-gna-1-0-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="intel-gna-1-0",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelGna10State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel GNA 1.0 (Tiger Lake Neural Accelerator)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELGNA10,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelGna10State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
