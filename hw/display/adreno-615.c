/* Qualcomm Adreno 615 (Snapdragon 718X, auto)
 * Vendor: 0x17CB, Device: 0x6015
 * Usage: -device adreno-615
...
#define GPU_DEV 0x6015
#define GPU_SS_V 0x17CB
#define GPU_SS_D 0x0060
#define GPU_CLASS 0x0302
#define BAR0_SIZE (4*MiB)
#define BAR1_SIZE (64*MiB)
#define TYPE_ADRENO_615 "adreno-615"
OBJECT_DECLARE_SIMPLE_TYPE(Adreno615State, ADRENO_615)
struct Adreno615State { PCIDevice parent_obj; MemoryRegion bar0,bar1; uint32_t clock_mhz; uint64_t clock_last_ns; char *gpu_name; };
...
static void realize(PCIDevice*p,Error**e){
    Adreno615State*s=(Adreno615State*)p; s->clock_mhz=0;s->clock_last_ns=0;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0,s,"adreno-615-mmio",BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1,s,"adreno-615-vram",BAR1_SIZE);
    pci_register_bar(p,2,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
}
static const Property props[]={DEFINE_PROP_STRING("gpu-name",Adreno615State,gpu_name),};
static const VMStateDescription vms={.name="adreno-615",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,Adreno615State),VMSTATE_UINT32(clock_mhz,Adreno615State),VMSTATE_UINT64(clock_last_ns,Adreno615State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass*k,const void*d){DeviceClass*dc=DEVICE_CLASS(k);PCIDeviceClass*pc=PCI_DEVICE_CLASS(k);pc->realize=realize;pc->vendor_id=GPU_VID;pc->device_id=GPU_DEV;pc->class_id=GPU_CLASS;pc->revision=0;dc->desc="Qualcomm Adreno 615 (Snapdragon 718X, auto)";dc->vmsd=&vms;dc->hotpluggable=false;device_class_set_props(dc,props);set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);}
static const TypeInfo ti={.name=TYPE_ADRENO_615,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(Adreno615State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
