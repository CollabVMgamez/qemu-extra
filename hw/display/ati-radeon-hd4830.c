/*
 * ATI Radeon HD 4830 (RV770, PCIe x16, 512MB GDDR3, 2009)
...
#define GPU_DEVICE_ID  0x944A
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x944A
...
#define GPU_CLK_BASE    575
#define GPU_MEM_MHZ     900
#define GPU_CLK_BOOST   575
#define GPU_PCIE_GEN    2
#define GPU_VRAM_MB     512
#define TYPE_ATI_RADEON_HD4830 "ati-radeon-hd4830"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeonHd4830State, ATI_RADEON_HD4830)
struct AtiRadeonHd4830State {
...
static uint32_t gpu_clk(AtiRadeonHd4830State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeonHd4830State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeonHd4830State *s=(AtiRadeonHd4830State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-hd4830-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-hd4830-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-hd4830-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-hd4830-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_hd4830={.name="ati-radeon-hd4830",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeonHd4830State),VMSTATE_UINT32(intr_en,AtiRadeonHd4830State),VMSTATE_UINT32(clock_mhz,AtiRadeonHd4830State),VMSTATE_UINT64(clock_last_ns,AtiRadeonHd4830State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeonHd4830State[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeonHd4830State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeonHd4830State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeonHd4830State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x944A;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="ATI Radeon HD 4830 (RV770, PCIe x16, 512MB GDDR3, 2009)"; dc->vmsd=&vms_ati_radeon_hd4830; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRadeonHd4830State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_radeon_hd4830={.name=TYPE_ATI_RADEON_HD4830,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRadeonHd4830State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_radeon_hd4830(void){type_register_static(&ti_ati_radeon_hd4830);}
type_init(reg_ati_radeon_hd4830)
