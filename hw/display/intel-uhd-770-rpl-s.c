/*
 * Intel UHD Graphics 770 (Raptor Lake-S GT1, 32EU)
 * Vendor: 0x8086, Device: 0xA780
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-770-rpl-s
...
#define GPU_DEV      0xA780
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2280
#define GPU_REV      0x0C
...
#define TYPE_INTELUHD770RPLS "intel-uhd-770-rpl-s"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhd770RplSState, INTELUHD770RPLS)
struct IntelUhd770RplSState {
...
static uint32_t igpu_clk(IntelUhd770RplSState *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhd770RplSState *s = INTELUHD770RPLS(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-770-rpl-s-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-770-rpl-s-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhd770RplSState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-770-rpl-s",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhd770RplSState),VMSTATE_UINT32(clock_mhz,IntelUhd770RplSState),VMSTATE_UINT64(clock_last_ns,IntelUhd770RplSState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics 770 (Raptor Lake-S GT1, 32EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHD770RPLS,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhd770RplSState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
