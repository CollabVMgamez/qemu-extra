/*
 * AMD Radeon RX 7700 XT (Navi32, RDNA 3.0)
...
#define GPU_DEVICE_ID  0x747E
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x7700
...
#define GPU_PMC_BOOT_0  0x168000C1
...
#define GPU_CLK_BASE    1700
#define GPU_MEM_MHZ     2250
#define GPU_CLK_BOOST   2599
#define GPU_PCIE_GEN    4
#define TYPE_AMD_RX7700XT "amd-rx7700xt"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRx7700xtState, AMD_RX7700XT)
struct AmdRx7700xtState {
...
static uint32_t gpu_clk(AmdRx7700xtState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRx7700xtState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRx7700xtState *s=(AmdRx7700xtState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-rx7700xt-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-rx7700xt-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-rx7700xt-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-rx7700xt-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_rx7700xt={.name="amd-rx7700xt",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRx7700xtState),VMSTATE_UINT32(intr_en,AmdRx7700xtState),VMSTATE_UINT32(pfifo_intr_en,AmdRx7700xtState),VMSTATE_UINT32(clock_mhz,AmdRx7700xtState),VMSTATE_UINT64(clock_last_ns,AmdRx7700xtState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AmdRx7700xtState[] = {
    DEFINE_PROP_STRING("gpu-name", AmdRx7700xtState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AmdRx7700xtState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AmdRx7700xtState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="AMD Radeon RX 7700 XT (Navi32, RDNA 3.0)"; dc->vmsd=&vms_amd_rx7700xt; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRx7700xtState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_rx7700xt={.name=TYPE_AMD_RX7700XT,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRx7700xtState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_rx7700xt(void){type_register_static(&ti_amd_rx7700xt);}
type_init(reg_amd_rx7700xt)
