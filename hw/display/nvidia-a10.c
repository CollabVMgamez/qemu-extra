/*
 * NVIDIA A10 (GA102, Ampere, 24GB GDDR6)
...
#define GPU_DEVICE_ID  0x2236
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x1482
...
#define GPU_CLK_BASE    1095
#define GPU_MEM_MHZ     1250
#define GPU_CLK_BOOST   1695
#define GPU_PCIE_GEN    4
#define TYPE_NVIDIA_A10 "nvidia-a10"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaA10State, NVIDIA_A10)
struct NvidiaA10State {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaA10State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaA10State *s=(NvidiaA10State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-a10-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-a10-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-a10-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-a10-vgaio",NV_BAR5_SIZE);
...
static const Property gpu_multi_props_NvidiaA10State[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaA10State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaA10State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaA10State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="NVIDIA A10 (GA102, Ampere, 24GB GDDR6)"; dc->vmsd=&vms_nvidia_a10; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaA10State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_a10={.name=TYPE_NVIDIA_A10,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaA10State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_a10(void){type_register_static(&ti_nvidia_a10);}
type_init(reg_nvidia_a10)
