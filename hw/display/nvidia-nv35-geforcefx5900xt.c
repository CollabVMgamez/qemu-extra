/*
 * NVIDIA GeForce FX 5900 XT (NV35, AGP 8x, 128MB DDR, 2003)
...
#define GPU_DEVICE_ID  0x0331
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x0331
...
#define TYPE_NVIDIA_NV35_GEFORCEFX5900XT "nvidia-nv35-geforcefx5900xt"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaNv35Geforcefx5900xtState, NVIDIA_NV35_GEFORCEFX5900XT)
struct NvidiaNv35Geforcefx5900xtState {
...
static uint32_t gpu_clk(NvidiaNv35Geforcefx5900xtState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaNv35Geforcefx5900xtState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaNv35Geforcefx5900xtState *s=(NvidiaNv35Geforcefx5900xtState*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-nv35-geforcefx5900xt-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b135ops,s,"nvidia-nv35-geforcefx5900xt-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b135ops,s,"nvidia-nv35-geforcefx5900xt-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b135ops,s,"nvidia-nv35-geforcefx5900xt-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_nvidia_nv35_geforcefx5900xt[]={DEFINE_PROP_UINT32("gpu-count",NvidiaNv35Geforcefx5900xtState,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",NvidiaNv35Geforcefx5900xtState,gpu_name),
    DEFINE_PROP_STRING("board-partner",NvidiaNv35Geforcefx5900xtState,board_partner)};
static const VMStateDescription vms_nvidia_nv35_geforcefx5900xt={.name="nvidia-nv35-geforcefx5900xt",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaNv35Geforcefx5900xtState),VMSTATE_UINT32(intr_en,NvidiaNv35Geforcefx5900xtState),VMSTATE_UINT32(clock_mhz,NvidiaNv35Geforcefx5900xtState),VMSTATE_UINT64(clock_last_ns,NvidiaNv35Geforcefx5900xtState),
        VMSTATE_UINT32(gpu_count,NvidiaNv35Geforcefx5900xtState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x0331;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA GeForce FX 5900 XT (NV35, AGP 8x, 128MB DDR, 2003)"; dc->vmsd=&vms_nvidia_nv35_geforcefx5900xt; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_nvidia_nv35_geforcefx5900xt);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_nv35_geforcefx5900xt={.name=TYPE_NVIDIA_NV35_GEFORCEFX5900XT,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaNv35Geforcefx5900xtState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_nv35_geforcefx5900xt(void){type_register_static(&ti_nvidia_nv35_geforcefx5900xt);}
type_init(reg_nvidia_nv35_geforcefx5900xt)
