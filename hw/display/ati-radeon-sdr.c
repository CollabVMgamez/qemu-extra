/*
 * ATI Radeon SDR (R100, AGP, 32MB SDR, 2000)
...
#define GPU_CLK_BASE    166
#define GPU_MEM_MHZ     143
#define GPU_CLK_BOOST   166
#define GPU_PCIE_GEN    1
#define GPU_VRAM_MB     32
#define TYPE_ATI_RADEON_SDR "ati-radeon-sdr"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeonSdrState, ATI_RADEON_SDR)
struct AtiRadeonSdrState {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeonSdrState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeonSdrState *s=(AtiRadeonSdrState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-sdr-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-sdr-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-sdr-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-sdr-vgaio",NV_BAR5_SIZE);
...
static void reg_ati_radeon_sdr(void){type_register_static(&ti_ati_radeon_sdr);}
type_init(reg_ati_radeon_sdr)
