/*
 * Intel Iris Pro Graphics 6200 (Broadwell GT3e, 48EU + 128MB eDRAM)
 * Vendor: 0x8086, Device: 0x1622
 * Class: 0x0300, PCIe
 * Usage: -device intel-iris-pro-6200
...
#define TYPE_INTELIRISPRO6200 "intel-iris-pro-6200"
OBJECT_DECLARE_SIMPLE_TYPE(IntelIrisPro6200State, INTELIRISPRO6200)
struct IntelIrisPro6200State {
...
static uint32_t igpu_clk(IntelIrisPro6200State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelIrisPro6200State *s = opaque;
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelIrisPro6200State *s = INTELIRISPRO6200(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-iris-pro-6200-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-iris-pro-6200-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelIrisPro6200State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-iris-pro-6200",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelIrisPro6200State),VMSTATE_UINT32(clock_mhz,IntelIrisPro6200State),VMSTATE_UINT64(clock_last_ns,IntelIrisPro6200State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Iris Pro Graphics 6200 (Broadwell GT3e, 48EU + 128MB eDRAM)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELIRISPRO6200,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelIrisPro6200State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
