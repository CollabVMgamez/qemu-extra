/*
 * Intel HD Graphics 4000 (Braswell GT1, 12EU)
 * Vendor: 0x8086, Device: 0x22B1
 * Class: 0x0300, PCIe
 * Usage: -device intel-hd-4000-braswell
...
#define GPU_DEV      0x22B1
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x22B1
#define GPU_REV      0x0C
#define GPU_CLASS    0x0300
#define BAR0_SIZE (8*MiB)
#define BAR2_SIZE (128*MiB)
#define GPU_CLK_BASE  320
#define GPU_CLK_BOOST 640
#define TYPE_INTELHD4000BRASWELL "intel-hd-4000-braswell"
OBJECT_DECLARE_SIMPLE_TYPE(IntelHd4000BraswellState, INTELHD4000BRASWELL)
struct IntelHd4000BraswellState {
...
static uint32_t igpu_clk(IntelHd4000BraswellState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelHd4000BraswellState *s = opaque;
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelHd4000BraswellState *s = INTELHD4000BRASWELL(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-hd-4000-braswell-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-hd-4000-braswell-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelHd4000BraswellState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-hd-4000-braswell",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelHd4000BraswellState),VMSTATE_UINT32(clock_mhz,IntelHd4000BraswellState),VMSTATE_UINT64(clock_last_ns,IntelHd4000BraswellState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel HD Graphics 4000 (Braswell GT1, 12EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELHD4000BRASWELL,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelHd4000BraswellState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
