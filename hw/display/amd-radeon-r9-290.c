/*
 * AMD Radeon R9 290 (Hawaii, PCIe x16, 4GB GDDR5, 2013)
...
#define GPU_DEVICE_ID  0x67B1
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x67B1
...
#define GPU_CLK_BASE    947
#define GPU_MEM_MHZ     1250
#define GPU_CLK_BOOST   947
#define GPU_PCIE_GEN    3
#define GPU_VRAM_MB     4096
#define TYPE_AMD_RADEON_R9_290 "amd-radeon-r9-290"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRadeonR9290State, AMD_RADEON_R9_290)
struct AmdRadeonR9290State {
...
static uint32_t gpu_clk(AmdRadeonR9290State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRadeonR9290State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRadeonR9290State *s=(AmdRadeonR9290State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-radeon-r9-290-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-radeon-r9-290-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-radeon-r9-290-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-radeon-r9-290-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_radeon_r9_290={.name="amd-radeon-r9-290",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRadeonR9290State),VMSTATE_UINT32(intr_en,AmdRadeonR9290State),VMSTATE_UINT32(clock_mhz,AmdRadeonR9290State),VMSTATE_UINT64(clock_last_ns,AmdRadeonR9290State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AmdRadeonR9290State[] = {
    DEFINE_PROP_STRING("gpu-name", AmdRadeonR9290State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AmdRadeonR9290State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AmdRadeonR9290State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x67B1;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="AMD Radeon R9 290 (Hawaii, PCIe x16, 4GB GDDR5, 2013)"; dc->vmsd=&vms_amd_radeon_r9_290; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRadeonR9290State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_radeon_r9_290={.name=TYPE_AMD_RADEON_R9_290,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRadeonR9290State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_radeon_r9_290(void){type_register_static(&ti_amd_radeon_r9_290);}
type_init(reg_amd_radeon_r9_290)
