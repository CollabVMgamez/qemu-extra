/*
 * NVIDIA GeForce GTX 590 (GF110, Fermi, Dual-GPU)
...
#define GPU_DEVICE_ID    0x1087
#define GPU_SUBSYS_VID   0x10DE
#define GPU_SUBSYS_DID   0x0855
...
#define GPU_MEM_MHZ     854
#define GPU_CLK_BOOST    607
#define GPU_PCIE_GEN     2

#define TYPE_NVIDIA_GTX590 "nvidia-gtx590"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx590State, NVIDIA_GTX590)
...
static uint32_t gpu_get_clock(NvidiaGtx590State *s)
{
...
{
    NvidiaGtx590State *s = (NvidiaGtx590State *)pdev; s->gpu_count=1;
    ...
    gpu_setup_caps(pdev);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops,  s, "nvidia-gtx590-mmio", NV_BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar1, OBJECT(s), &bar1_ops,  s, "nvidia-gtx590-vram", NV_BAR1_SIZE);
    pci_register_bar(pdev, 1, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar1);
    memory_region_init_io(&s->bar3, OBJECT(s), &bar35_ops, s, "nvidia-gtx590-ramin",NV_BAR3_SIZE);
    pci_register_bar(pdev, 3, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar3);
    memory_region_init_io(&s->bar5, OBJECT(s), &bar35_ops, s, "nvidia-gtx590-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vmstate_nvidia_gtx590 = {
    .name = "nvidia-gtx590", .version_id = 1, .minimum_version_id = 1,
    .fields = (const VMStateField[]) {
        VMSTATE_PCI_DEVICE(parent_obj, NvidiaGtx590State),
...
static const TypeInfo nvidia_gtx590_info = {
    .name          = TYPE_NVIDIA_GTX590,
    .parent        = TYPE_PCI_DEVICE,
    .instance_size = sizeof(NvidiaGtx590State),
    .class_init    = gpu_class_init,
    .interfaces    = (InterfaceInfo[]) {
        { INTERFACE_CONVENTIONAL_PCI_DEVICE }, { },
    },
};
static void nvidia_gtx590_register(void) { type_register_static(&nvidia_gtx590_info); }
type_init(nvidia_gtx590_register)
