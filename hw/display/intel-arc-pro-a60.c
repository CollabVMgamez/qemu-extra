/*
 * Intel Arc Pro A60 (ACM-G10, 75W, 12GB)
 * Vendor: 0x8086, Device: 0x56A6
 * Class: 0x0302, PCIe
 * Usage: -device intel-arc-pro-a60
...
#define TYPE_INTELARCPROA60 "intel-arc-pro-a60"
OBJECT_DECLARE_SIMPLE_TYPE(IntelArcProA60State, INTELARCPROA60)
struct IntelArcProA60State {
...
static uint32_t igpu_clk(IntelArcProA60State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelArcProA60State *s = INTELARCPROA60(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-arc-pro-a60-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-arc-pro-a60-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelArcProA60State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-arc-pro-a60",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelArcProA60State),VMSTATE_UINT32(clock_mhz,IntelArcProA60State),VMSTATE_UINT64(clock_last_ns,IntelArcProA60State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Arc Pro A60 (ACM-G10, 75W, 12GB)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELARCPROA60,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelArcProA60State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
