/*
 * Intel UHD Graphics 630 (Comet Lake-H GT2, 24EU)
 * Vendor: 0x8086, Device: 0x9B51
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-630-cml
...
#define TYPE_INTELUHD630CML "intel-uhd-630-cml"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhd630CmlState, INTELUHD630CML)
struct IntelUhd630CmlState {
...
static uint32_t igpu_clk(IntelUhd630CmlState *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhd630CmlState *s = INTELUHD630CML(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-630-cml-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-630-cml-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhd630CmlState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-630-cml",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhd630CmlState),VMSTATE_UINT32(clock_mhz,IntelUhd630CmlState),VMSTATE_UINT64(clock_last_ns,IntelUhd630CmlState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics 630 (Comet Lake-H GT2, 24EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHD630CML,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhd630CmlState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
