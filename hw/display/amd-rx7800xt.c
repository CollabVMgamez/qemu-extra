/*
 * AMD Radeon RX 7800 XT (Navi32, RDNA 3.0)
...
#define GPU_CLK_BASE    1124
#define GPU_MEM_MHZ     2250
#define GPU_CLK_BOOST   2430
#define GPU_PCIE_GEN    4
#define TYPE_AMD_RX7800XT "amd-rx7800xt"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRx7800xtState, AMD_RX7800XT)
struct AmdRx7800xtState {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRx7800xtState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRx7800xtState *s=(AmdRx7800xtState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-rx7800xt-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-rx7800xt-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-rx7800xt-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-rx7800xt-vgaio",NV_BAR5_SIZE);
...
static const Property gpu_multi_props_AmdRx7800xtState[] = {
    DEFINE_PROP_STRING("gpu-name", AmdRx7800xtState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AmdRx7800xtState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AmdRx7800xtState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="AMD Radeon RX 7800 XT (Navi32, RDNA 3.0)"; dc->vmsd=&vms_amd_rx7800xt; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRx7800xtState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_rx7800xt={.name=TYPE_AMD_RX7800XT,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRx7800xtState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_rx7800xt(void){type_register_static(&ti_amd_rx7800xt);}
type_init(reg_amd_rx7800xt)
