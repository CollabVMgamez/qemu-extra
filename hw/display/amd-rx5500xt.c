/*
 * AMD Radeon RX 5500 XT (Navi14, RDNA 1.0)
...
#define GPU_DEVICE_ID  0x7340
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x5500
...
#define GPU_CLK_BOOST   1845
#define GPU_PCIE_GEN    4
#define TYPE_AMD_RX5500XT "amd-rx5500xt"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRx5500xtState, AMD_RX5500XT)
struct AmdRx5500xtState {
...
static uint32_t gpu_clk(AmdRx5500xtState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRx5500xtState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRx5500xtState *s=(AmdRx5500xtState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-rx5500xt-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-rx5500xt-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-rx5500xt-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-rx5500xt-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_rx5500xt={.name="amd-rx5500xt",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRx5500xtState),VMSTATE_UINT32(intr_en,AmdRx5500xtState),VMSTATE_UINT32(pfifo_intr_en,AmdRx5500xtState),VMSTATE_UINT32(clock_mhz,AmdRx5500xtState),VMSTATE_UINT64(clock_last_ns,AmdRx5500xtState),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x7340;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="AMD Radeon RX 5500 XT (Navi14, RDNA 1.0)"; dc->vmsd=&vms_amd_rx5500xt; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRx5500xtState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_rx5500xt={.name=TYPE_AMD_RX5500XT,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRx5500xtState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_rx5500xt(void){type_register_static(&ti_amd_rx5500xt);}
type_init(reg_amd_rx5500xt)
