/*
 * Intel HD Graphics 2500 (Ivy Bridge Server GT1, 6EU)
 * Vendor: 0x8086, Device: 0x015A
 * Class: 0x0300, PCIe
 * Usage: -device intel-hd-2500-ivb-server
...
#define GPU_DEV      0x015A
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2101
#define GPU_REV      0x09
...
#define TYPE_INTELHD2500IVBSERVER "intel-hd-2500-ivb-server"
OBJECT_DECLARE_SIMPLE_TYPE(IntelHd2500IvbServerState, INTELHD2500IVBSERVER)
struct IntelHd2500IvbServerState {
...
static uint32_t igpu_clk(IntelHd2500IvbServerState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelHd2500IvbServerState *s = opaque;
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelHd2500IvbServerState *s = INTELHD2500IVBSERVER(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-hd-2500-ivb-server-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-hd-2500-ivb-server-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelHd2500IvbServerState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-hd-2500-ivb-server",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelHd2500IvbServerState),VMSTATE_UINT32(clock_mhz,IntelHd2500IvbServerState),VMSTATE_UINT64(clock_last_ns,IntelHd2500IvbServerState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel HD Graphics 2500 (Ivy Bridge Server GT1, 6EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELHD2500IVBSERVER,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelHd2500IvbServerState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
