/*
 * AMD Radeon R9 Nano (Fiji, PCIe x16, 4GB HBM, 2015)
...
#define TYPE_AMD_RADEON_R9_NANO "amd-radeon-r9-nano"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRadeonR9NanoState, AMD_RADEON_R9_NANO)
struct AmdRadeonR9NanoState {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRadeonR9NanoState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRadeonR9NanoState *s=(AmdRadeonR9NanoState*)p;
    ...
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-radeon-r9-nano-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-radeon-r9-nano-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-radeon-r9-nano-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_radeon_r9_nano={.name="amd-radeon-r9-nano",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRadeonR9NanoState),VMSTATE_UINT32(intr_en,AmdRadeonR9NanoState),VMSTATE_UINT32(clock_mhz,AmdRadeonR9NanoState),VMSTATE_UINT64(clock_last_ns,AmdRadeonR9NanoState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AmdRadeonR9NanoState[] = {
    DEFINE_PROP_STRING("gpu-name", AmdRadeonR9NanoState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AmdRadeonR9NanoState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AmdRadeonR9NanoState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="AMD Radeon R9 Nano (Fiji, PCIe x16, 4GB HBM, 2015)"; dc->vmsd=&vms_amd_radeon_r9_nano; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRadeonR9NanoState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_radeon_r9_nano={.name=TYPE_AMD_RADEON_R9_NANO,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRadeonR9NanoState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_radeon_r9_nano(void){type_register_static(&ti_amd_radeon_r9_nano);}
type_init(reg_amd_radeon_r9_nano)
