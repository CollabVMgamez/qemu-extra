/* NVIDIA Tegra X2 GPU (GP10B, Jetson TX2)
 * Vendor: 0x10DE, Device: 0x15B0
 * Usage: -device nvidia-tegra-x2-gpu
...
#define GPU_DEV 0x15B0
#define GPU_SS_V 0x10DE
#define GPU_SS_D 0x17D1
#define GPU_CLASS 0x0302
#define BAR0_SIZE (4*MiB)
#define BAR1_SIZE (64*MiB)
#define TYPE_NVIDIA_TEGRA_X2_GPU "nvidia-tegra-x2-gpu"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaTegraX2GpuState, NVIDIA_TEGRA_X2_GPU)
struct NvidiaTegraX2GpuState { PCIDevice parent_obj; MemoryRegion bar0,bar1; uint32_t clock_mhz; uint64_t clock_last_ns; char *gpu_name;     char *board_partner;
};
...
static void realize(PCIDevice*p,Error**e){
    NvidiaTegraX2GpuState*s=(NvidiaTegraX2GpuState*)p; s->clock_mhz=0;s->clock_last_ns=0;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0,s,"nvidia-tegra-x2-gpu-mmio",BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1,s,"nvidia-tegra-x2-gpu-vram",BAR1_SIZE);
    pci_register_bar(p,2,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
}
static const Property props[]={DEFINE_PROP_STRING("gpu-name",NvidiaTegraX2GpuState,gpu_name),    DEFINE_PROP_STRING("board-partner", NvidiaTegraX2GpuState, board_partner),
};
static const VMStateDescription vms={.name="nvidia-tegra-x2-gpu",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaTegraX2GpuState),VMSTATE_UINT32(clock_mhz,NvidiaTegraX2GpuState),VMSTATE_UINT64(clock_last_ns,NvidiaTegraX2GpuState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass*k,const void*d){DeviceClass*dc=DEVICE_CLASS(k);PCIDeviceClass*pc=PCI_DEVICE_CLASS(k);pc->realize=realize;pc->vendor_id=GPU_VID;pc->device_id=GPU_DEV;pc->class_id=GPU_CLASS;pc->revision=0;dc->desc="NVIDIA Tegra X2 GPU (GP10B, Jetson TX2)";dc->vmsd=&vms;dc->hotpluggable=false;device_class_set_props(dc,props);set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);}
static const TypeInfo ti={.name=TYPE_NVIDIA_TEGRA_X2_GPU,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaTegraX2GpuState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
