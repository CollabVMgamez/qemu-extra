/*
 * NVIDIA GeForce 256 DDR (NV10, AGP 4x, 32MB DDR, 1999)
...
#define GPU_DEVICE_ID  0x0101
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x0101
...
#define TYPE_NVIDIA_NV10_GEFORCE256DDR "nvidia-nv10-geforce256ddr"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaNv10Geforce256ddrState, NVIDIA_NV10_GEFORCE256DDR)
struct NvidiaNv10Geforce256ddrState {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaNv10Geforce256ddrState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaNv10Geforce256ddrState *s=(NvidiaNv10Geforce256ddrState*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-nv10-geforce256ddr-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b135ops,s,"nvidia-nv10-geforce256ddr-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b135ops,s,"nvidia-nv10-geforce256ddr-ramin",NV_BAR3_SIZE);
...
static const Property gpu_props_nvidia_nv10_geforce256ddr[]={DEFINE_PROP_UINT32("gpu-count",NvidiaNv10Geforce256ddrState,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",NvidiaNv10Geforce256ddrState,gpu_name),
...
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="NVIDIA GeForce 256 DDR (NV10, AGP 4x, 32MB DDR, 1999)"; dc->vmsd=&vms_nvidia_nv10_geforce256ddr; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_nvidia_nv10_geforce256ddr);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_nv10_geforce256ddr={.name=TYPE_NVIDIA_NV10_GEFORCE256DDR,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaNv10Geforce256ddrState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_nv10_geforce256ddr(void){type_register_static(&ti_nvidia_nv10_geforce256ddr);}
type_init(reg_nvidia_nv10_geforce256ddr)
