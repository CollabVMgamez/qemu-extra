/*
 * Samsung Xclipse 920 (RDNA2, Exynos 2200, integrated)
...
#define TYPE_SAMSUNG_XCLIPSE_920 "samsung-xclipse-920"
OBJECT_DECLARE_SIMPLE_TYPE(SamsungXclipse920State, SAMSUNG_XCLIPSE_920)
struct SamsungXclipse920State {
...
static uint32_t gpu_clk(SamsungXclipse920State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    SamsungXclipse920State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    SamsungXclipse920State *s=(SamsungXclipse920State*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"samsung-xclipse-920-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&bops,s,"samsung-xclipse-920-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&bops,s,"samsung-xclipse-920-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&bops,s,"samsung-xclipse-920-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_samsung_xclipse_920[]={DEFINE_PROP_UINT32("gpu-count",SamsungXclipse920State,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",SamsungXclipse920State,gpu_name)};
static const VMStateDescription vms_samsung_xclipse_920={.name="samsung-xclipse-920",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,SamsungXclipse920State),VMSTATE_UINT32(clock_mhz,SamsungXclipse920State),VMSTATE_UINT64(clock_last_ns,SamsungXclipse920State),VMSTATE_UINT32(gpu_count,SamsungXclipse920State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x920A;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="Samsung Xclipse 920 (RDNA2, Exynos 2200, integrated)"; dc->vmsd=&vms_samsung_xclipse_920; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_samsung_xclipse_920);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_samsung_xclipse_920={.name=TYPE_SAMSUNG_XCLIPSE_920,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(SamsungXclipse920State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_samsung_xclipse_920(void){type_register_static(&ti_samsung_xclipse_920);}
type_init(reg_samsung_xclipse_920)
