/*
 * ATI Radeon 9800 XT (R360, AGP 8x, 256MB DDR, 2003)
...
#define GPU_DEVICE_ID  0x4E4A
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x4E4A
...
#define GPU_CLK_BASE    412
#define GPU_MEM_MHZ     365
#define GPU_CLK_BOOST   412
#define GPU_PCIE_GEN    1
#define GPU_VRAM_MB     256
#define TYPE_ATI_RADEON_9800XT "ati-radeon-9800xt"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeon9800xtState, ATI_RADEON_9800XT)
struct AtiRadeon9800xtState {
...
static uint32_t gpu_clk(AtiRadeon9800xtState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeon9800xtState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeon9800xtState *s=(AtiRadeon9800xtState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-9800xt-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-9800xt-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-9800xt-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-9800xt-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_9800xt={.name="ati-radeon-9800xt",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeon9800xtState),VMSTATE_UINT32(intr_en,AtiRadeon9800xtState),VMSTATE_UINT32(clock_mhz,AtiRadeon9800xtState),VMSTATE_UINT64(clock_last_ns,AtiRadeon9800xtState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeon9800xtState[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeon9800xtState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeon9800xtState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeon9800xtState, board_partner),
};
...
static void reg_ati_radeon_9800xt(void){type_register_static(&ti_ati_radeon_9800xt);}
type_init(reg_ati_radeon_9800xt)
