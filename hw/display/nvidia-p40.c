/*
 * NVIDIA Tesla P40 (GP102, Pascal, 24GB GDDR5X)
...
#define GPU_DEVICE_ID  0x1B38
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x11D9
...
#define GPU_CLK_BASE    898
#define GPU_MEM_MHZ     1350
#define GPU_CLK_BOOST   1531
#define GPU_PCIE_GEN    3
#define TYPE_NVIDIA_P40 "nvidia-p40"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaP40State, NVIDIA_P40)
struct NvidiaP40State {
...
static uint32_t gpu_clk(NvidiaP40State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaP40State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaP40State *s=(NvidiaP40State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-p40-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-p40-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-p40-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-p40-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_p40={.name="nvidia-p40",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaP40State),VMSTATE_UINT32(intr_en,NvidiaP40State),VMSTATE_UINT32(pfifo_intr_en,NvidiaP40State),VMSTATE_UINT32(clock_mhz,NvidiaP40State),VMSTATE_UINT64(clock_last_ns,NvidiaP40State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaP40State[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaP40State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaP40State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaP40State, board_partner),
};
...
static void reg_nvidia_p40(void){type_register_static(&ti_nvidia_p40);}
type_init(reg_nvidia_p40)
