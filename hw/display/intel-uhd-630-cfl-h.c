/*
 * Intel UHD Graphics 630 (Coffee Lake-H GT2, 24EU)
 * Vendor: 0x8086, Device: 0x3E98
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-630-cfl-h
...
#define GPU_DEV      0x3E98
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2218
#define GPU_REV      0x00
...
#define TYPE_INTELUHD630CFLH "intel-uhd-630-cfl-h"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhd630CflHState, INTELUHD630CFLH)
struct IntelUhd630CflHState {
...
static uint32_t igpu_clk(IntelUhd630CflHState *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhd630CflHState *s = INTELUHD630CFLH(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-630-cfl-h-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-630-cfl-h-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhd630CflHState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-630-cfl-h",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhd630CflHState),VMSTATE_UINT32(clock_mhz,IntelUhd630CflHState),VMSTATE_UINT64(clock_last_ns,IntelUhd630CflHState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics 630 (Coffee Lake-H GT2, 24EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHD630CFLH,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhd630CflHState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
