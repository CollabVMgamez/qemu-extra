/*
 * Intel GNA 2.0 (Alder Lake Neural Accelerator)
 * Vendor: 0x8086, Device: 0x4A04
 * Class: 0x1180 (Processing Accelerator)
 * Usage: -device intel-gna-2-0
...
#define GPU_DEV      0x4A04
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2204
#define GPU_REV      0x00
#define GPU_CLASS    0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_INTELGNA20 "intel-gna-2-0"
OBJECT_DECLARE_SIMPLE_TYPE(IntelGna20State, INTELGNA20)
struct IntelGna20State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    IntelGna20State *s = INTELGNA20(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-gna-2-0-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="intel-gna-2-0",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelGna20State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel GNA 2.0 (Alder Lake Neural Accelerator)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELGNA20,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelGna20State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
