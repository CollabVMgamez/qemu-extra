/* Intel Iris Xe Mobile (Alder Lake-P, 96EU)
 * Vendor: 0x8086, Device: 0x46A6
 * Usage: -device intel-iris-xe-mobile-adl
...
#define GPU_DEV 0x46A6
#define GPU_SS_V 0x8086
#define GPU_SS_D 0x400E
#define GPU_CLASS 0x0300
#define BAR0_SIZE (4*MiB)
#define BAR1_SIZE (64*MiB)
#define TYPE_INTEL_IRIS_XE_MOBILE_ADL "intel-iris-xe-mobile-adl"
OBJECT_DECLARE_SIMPLE_TYPE(IntelIrisXeMobileAdlState, INTEL_IRIS_XE_MOBILE_ADL)
struct IntelIrisXeMobileAdlState { PCIDevice parent_obj; MemoryRegion bar0,bar1; uint32_t clock_mhz; uint64_t clock_last_ns; char *gpu_name; };
...
static void realize(PCIDevice*p,Error**e){
    IntelIrisXeMobileAdlState*s=(IntelIrisXeMobileAdlState*)p; s->clock_mhz=0;s->clock_last_ns=0;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0,s,"intel-iris-xe-mobile-adl-mmio",BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1,s,"intel-iris-xe-mobile-adl-vram",BAR1_SIZE);
    pci_register_bar(p,2,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
}
static const Property props[]={DEFINE_PROP_STRING("gpu-name",IntelIrisXeMobileAdlState,gpu_name),};
static const VMStateDescription vms={.name="intel-iris-xe-mobile-adl",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelIrisXeMobileAdlState),VMSTATE_UINT32(clock_mhz,IntelIrisXeMobileAdlState),VMSTATE_UINT64(clock_last_ns,IntelIrisXeMobileAdlState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass*k,const void*d){DeviceClass*dc=DEVICE_CLASS(k);PCIDeviceClass*pc=PCI_DEVICE_CLASS(k);pc->realize=realize;pc->vendor_id=GPU_VID;pc->device_id=GPU_DEV;pc->class_id=GPU_CLASS;pc->revision=0;dc->desc="Intel Iris Xe Mobile (Alder Lake-P, 96EU)";dc->vmsd=&vms;dc->hotpluggable=false;device_class_set_props(dc,props);set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);}
static const TypeInfo ti={.name=TYPE_INTEL_IRIS_XE_MOBILE_ADL,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelIrisXeMobileAdlState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
