/*
 * ARM Mali-G76 MP12 (Exynos 9820, integrated)
...
#define GPU_CLK_BASE    702
#define GPU_CLK_BOOST   702
...
#define TYPE_MALI_G76 "mali-g76"
OBJECT_DECLARE_SIMPLE_TYPE(MaliG76State, MALI_G76)
struct MaliG76State {
...
static uint32_t gpu_clk(MaliG76State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    MaliG76State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"mali-g76-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&bops,s,"mali-g76-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&bops,s,"mali-g76-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&bops,s,"mali-g76-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_mali_g76[]={DEFINE_PROP_UINT32("gpu-count",MaliG76State,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",MaliG76State,gpu_name)};
static const VMStateDescription vms_mali_g76={.name="mali-g76",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,MaliG76State),VMSTATE_UINT32(clock_mhz,MaliG76State),VMSTATE_UINT64(clock_last_ns,MaliG76State),VMSTATE_UINT32(gpu_count,MaliG76State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0xB076;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="ARM Mali-G76 MP12 (Exynos 9820, integrated)"; dc->vmsd=&vms_mali_g76; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_mali_g76);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_mali_g76={.name=TYPE_MALI_G76,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(MaliG76State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_mali_g76(void){type_register_static(&ti_mali_g76);}
type_init(reg_mali_g76)
