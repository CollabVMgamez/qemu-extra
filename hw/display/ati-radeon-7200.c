/*
 * ATI Radeon 7200 (R100, AGP 4x, 64MB DDR, 2000)
...
#define GPU_DEVICE_ID  0x5144
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x5144
...
#define GPU_CLK_BASE    183
#define GPU_MEM_MHZ     183
#define GPU_CLK_BOOST   183
#define GPU_PCIE_GEN    1
#define GPU_VRAM_MB     64
#define TYPE_ATI_RADEON_7200 "ati-radeon-7200"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeon7200State, ATI_RADEON_7200)
struct AtiRadeon7200State {
...
static uint32_t gpu_clk(AtiRadeon7200State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeon7200State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeon7200State *s=(AtiRadeon7200State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-7200-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-7200-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-7200-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-7200-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_7200={.name="ati-radeon-7200",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeon7200State),VMSTATE_UINT32(intr_en,AtiRadeon7200State),VMSTATE_UINT32(clock_mhz,AtiRadeon7200State),VMSTATE_UINT64(clock_last_ns,AtiRadeon7200State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeon7200State[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeon7200State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeon7200State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeon7200State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x5144;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="ATI Radeon 7200 (R100, AGP 4x, 64MB DDR, 2000)"; dc->vmsd=&vms_ati_radeon_7200; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRadeon7200State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_radeon_7200={.name=TYPE_ATI_RADEON_7200,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRadeon7200State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_radeon_7200(void){type_register_static(&ti_ati_radeon_7200);}
type_init(reg_ati_radeon_7200)
