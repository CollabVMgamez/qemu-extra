/*
 * AMD Radeon RX 470 (Polaris10, GCN 4.0)
...
#define GPU_PMC_BOOT_0  0x128000C5
...
#define GPU_CLK_BASE    926
#define GPU_MEM_MHZ     1650
#define GPU_CLK_BOOST   1206
#define GPU_PCIE_GEN    3
#define TYPE_AMD_RX470 "amd-rx470"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRx470State, AMD_RX470)
struct AmdRx470State {
...
static uint32_t gpu_clk(AmdRx470State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRx470State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRx470State *s=(AmdRx470State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-rx470-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-rx470-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-rx470-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-rx470-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_rx470={.name="amd-rx470",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRx470State),VMSTATE_UINT32(intr_en,AmdRx470State),VMSTATE_UINT32(pfifo_intr_en,AmdRx470State),VMSTATE_UINT32(clock_mhz,AmdRx470State),VMSTATE_UINT64(clock_last_ns,AmdRx470State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AmdRx470State[] = {
    DEFINE_PROP_STRING("gpu-name", AmdRx470State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AmdRx470State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AmdRx470State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="AMD Radeon RX 470 (Polaris10, GCN 4.0)"; dc->vmsd=&vms_amd_rx470; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRx470State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_rx470={.name=TYPE_AMD_RX470,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRx470State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_rx470(void){type_register_static(&ti_amd_rx470);}
type_init(reg_amd_rx470)
