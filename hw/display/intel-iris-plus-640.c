/*
 * Intel Iris Plus Graphics 640 (Kaby Lake ULT GT3e, 48EU + 64MB eDRAM)
 * Vendor: 0x8086, Device: 0x5926
 * Class: 0x0300, PCIe
 * Usage: -device intel-iris-plus-640
...
#define GPU_DEV      0x5926
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2226
#define GPU_REV      0x02
...
#define TYPE_INTELIRISPLUS640 "intel-iris-plus-640"
OBJECT_DECLARE_SIMPLE_TYPE(IntelIrisPlus640State, INTELIRISPLUS640)
struct IntelIrisPlus640State {
...
static uint32_t igpu_clk(IntelIrisPlus640State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelIrisPlus640State *s = opaque;
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelIrisPlus640State *s = INTELIRISPLUS640(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-iris-plus-640-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-iris-plus-640-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelIrisPlus640State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-iris-plus-640",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelIrisPlus640State),VMSTATE_UINT32(clock_mhz,IntelIrisPlus640State),VMSTATE_UINT64(clock_last_ns,IntelIrisPlus640State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Iris Plus Graphics 640 (Kaby Lake ULT GT3e, 48EU + 64MB eDRAM)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELIRISPLUS640,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelIrisPlus640State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
