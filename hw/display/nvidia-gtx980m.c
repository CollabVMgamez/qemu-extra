/*
 * NVIDIA GeForce GTX 980M (GM204, Maxwell, Mobile)
...
#define GPU_DEVICE_ID  0x13D9
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x1098
...
#define GPU_PMC_BOOT_0  0x118000A1
...
#define GPU_CLK_BASE    1038
#define GPU_MEM_MHZ     1250
#define GPU_CLK_BOOST   1127
#define GPU_PCIE_GEN    2
#define TYPE_NVIDIA_GTX980M "nvidia-gtx980m"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx980mState, NVIDIA_GTX980M)
struct NvidiaGtx980mState {
...
static uint32_t gpu_clk(NvidiaGtx980mState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaGtx980mState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaGtx980mState *s=(NvidiaGtx980mState*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-gtx980m-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-gtx980m-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-gtx980m-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-gtx980m-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_gtx980m={.name="nvidia-gtx980m",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaGtx980mState),VMSTATE_UINT32(intr_en,NvidiaGtx980mState),VMSTATE_UINT32(pfifo_intr_en,NvidiaGtx980mState),VMSTATE_UINT32(clock_mhz,NvidiaGtx980mState),VMSTATE_UINT64(clock_last_ns,NvidiaGtx980mState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaGtx980mState[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaGtx980mState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaGtx980mState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaGtx980mState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x13D9;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA GeForce GTX 980M (GM204, Maxwell, Mobile)"; dc->vmsd=&vms_nvidia_gtx980m; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaGtx980mState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_gtx980m={.name=TYPE_NVIDIA_GTX980M,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaGtx980mState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_gtx980m(void){type_register_static(&ti_nvidia_gtx980m);}
type_init(reg_nvidia_gtx980m)
