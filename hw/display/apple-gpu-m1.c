/*
 * Apple M1 GPU (7-core, unified memory, integrated)
...
#define GPU_CLK_BASE    1278
#define GPU_CLK_BOOST   1278
...
#define TYPE_APPLE_GPU_M1 "apple-gpu-m1"
OBJECT_DECLARE_SIMPLE_TYPE(AppleGpuM1State, APPLE_GPU_M1)
struct AppleGpuM1State {
...
static uint32_t gpu_clk(AppleGpuM1State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AppleGpuM1State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AppleGpuM1State *s=(AppleGpuM1State*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"apple-gpu-m1-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&bops,s,"apple-gpu-m1-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&bops,s,"apple-gpu-m1-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&bops,s,"apple-gpu-m1-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_apple_gpu_m1[]={DEFINE_PROP_UINT32("gpu-count",AppleGpuM1State,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",AppleGpuM1State,gpu_name)};
static const VMStateDescription vms_apple_gpu_m1={.name="apple-gpu-m1",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AppleGpuM1State),VMSTATE_UINT32(clock_mhz,AppleGpuM1State),VMSTATE_UINT64(clock_last_ns,AppleGpuM1State),VMSTATE_UINT32(gpu_count,AppleGpuM1State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0xA001;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="Apple M1 GPU (7-core, unified memory, integrated)"; dc->vmsd=&vms_apple_gpu_m1; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_apple_gpu_m1);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_apple_gpu_m1={.name=TYPE_APPLE_GPU_M1,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AppleGpuM1State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_apple_gpu_m1(void){type_register_static(&ti_apple_gpu_m1);}
type_init(reg_apple_gpu_m1)
