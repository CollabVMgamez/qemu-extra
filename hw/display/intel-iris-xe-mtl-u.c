/*
 * Intel Iris Xe Graphics (Meteor Lake-U GT2, 96EU)
 * Vendor: 0x8086, Device: 0x7D45
 * Class: 0x0300, PCIe
 * Usage: -device intel-iris-xe-mtl-u
...
#define GPU_DEV      0x7D45
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2245
#define GPU_REV      0x00
...
#define TYPE_INTELIRISXEMTLU "intel-iris-xe-mtl-u"
OBJECT_DECLARE_SIMPLE_TYPE(IntelIrisXeMtlUState, INTELIRISXEMTLU)
struct IntelIrisXeMtlUState {
...
static uint32_t igpu_clk(IntelIrisXeMtlUState *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelIrisXeMtlUState *s = INTELIRISXEMTLU(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-iris-xe-mtl-u-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-iris-xe-mtl-u-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelIrisXeMtlUState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-iris-xe-mtl-u",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelIrisXeMtlUState),VMSTATE_UINT32(clock_mhz,IntelIrisXeMtlUState),VMSTATE_UINT64(clock_last_ns,IntelIrisXeMtlUState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Iris Xe Graphics (Meteor Lake-U GT2, 96EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELIRISXEMTLU,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelIrisXeMtlUState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
