/*
 * AMD Radeon HD 7850 (Pitcairn, PCIe x16, 1GB GDDR5, 2012)
...
#define GPU_CLK_BASE    860
#define GPU_MEM_MHZ     1200
#define GPU_CLK_BOOST   860
#define GPU_PCIE_GEN    3
#define GPU_VRAM_MB     1024
#define TYPE_AMD_RADEON_HD7850 "amd-radeon-hd7850"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRadeonHd7850State, AMD_RADEON_HD7850)
struct AmdRadeonHd7850State {
...
static uint32_t gpu_clk(AmdRadeonHd7850State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRadeonHd7850State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRadeonHd7850State *s=(AmdRadeonHd7850State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-radeon-hd7850-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-radeon-hd7850-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-radeon-hd7850-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-radeon-hd7850-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_radeon_hd7850={.name="amd-radeon-hd7850",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRadeonHd7850State),VMSTATE_UINT32(intr_en,AmdRadeonHd7850State),VMSTATE_UINT32(clock_mhz,AmdRadeonHd7850State),VMSTATE_UINT64(clock_last_ns,AmdRadeonHd7850State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AmdRadeonHd7850State[] = {
    DEFINE_PROP_STRING("gpu-name", AmdRadeonHd7850State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AmdRadeonHd7850State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AmdRadeonHd7850State, board_partner),
};
...
static void reg_amd_radeon_hd7850(void){type_register_static(&ti_amd_radeon_hd7850);}
type_init(reg_amd_radeon_hd7850)
