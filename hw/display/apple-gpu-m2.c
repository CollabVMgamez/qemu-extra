/*
 * Apple M2 GPU (10-core, unified memory, integrated)
...
#define GPU_CLK_BASE    1398
#define GPU_CLK_BOOST   1398
...
#define TYPE_APPLE_GPU_M2 "apple-gpu-m2"
OBJECT_DECLARE_SIMPLE_TYPE(AppleGpuM2State, APPLE_GPU_M2)
struct AppleGpuM2State {
...
static uint32_t gpu_clk(AppleGpuM2State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AppleGpuM2State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AppleGpuM2State *s=(AppleGpuM2State*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"apple-gpu-m2-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&bops,s,"apple-gpu-m2-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&bops,s,"apple-gpu-m2-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&bops,s,"apple-gpu-m2-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_apple_gpu_m2[]={DEFINE_PROP_UINT32("gpu-count",AppleGpuM2State,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",AppleGpuM2State,gpu_name)};
static const VMStateDescription vms_apple_gpu_m2={.name="apple-gpu-m2",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AppleGpuM2State),VMSTATE_UINT32(clock_mhz,AppleGpuM2State),VMSTATE_UINT64(clock_last_ns,AppleGpuM2State),VMSTATE_UINT32(gpu_count,AppleGpuM2State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0xA003;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="Apple M2 GPU (10-core, unified memory, integrated)"; dc->vmsd=&vms_apple_gpu_m2; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_apple_gpu_m2);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_apple_gpu_m2={.name=TYPE_APPLE_GPU_M2,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AppleGpuM2State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_apple_gpu_m2(void){type_register_static(&ti_apple_gpu_m2);}
type_init(reg_apple_gpu_m2)
