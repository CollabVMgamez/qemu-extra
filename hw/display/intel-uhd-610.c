/*
 * Intel UHD Graphics 610 (Coffee Lake GT1, 12EU)
 * Vendor: 0x8086, Device: 0x3E90
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-610
...
#define GPU_DEV      0x3E90
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2210
#define GPU_REV      0x00
...
#define TYPE_INTELUHD610 "intel-uhd-610"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhd610State, INTELUHD610)
struct IntelUhd610State {
...
static uint32_t igpu_clk(IntelUhd610State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhd610State *s = INTELUHD610(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-610-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-610-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhd610State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-610",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhd610State),VMSTATE_UINT32(clock_mhz,IntelUhd610State),VMSTATE_UINT64(clock_last_ns,IntelUhd610State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics 610 (Coffee Lake GT1, 12EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHD610,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhd610State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
