/*
 * NVIDIA GeForce GTX 560M (GF116, Fermi, Mobile)
...
#define GPU_DEVICE_ID    0x1251
#define GPU_SUBSYS_VID   0x10DE
#define GPU_SUBSYS_DID   0x0856
...
#define GPU_MEM_MHZ     800
#define GPU_CLK_BOOST    775
#define GPU_PCIE_GEN     2

#define TYPE_NVIDIA_GTX560M "nvidia-gtx560m"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx560mState, NVIDIA_GTX560M)

struct NvidiaGtx560mState {
...
static uint32_t gpu_get_clock(NvidiaGtx560mState *s)
{
...
{
    NvidiaGtx560mState *s = (NvidiaGtx560mState *)pdev; s->gpu_count=1;
    ...
    gpu_setup_caps(pdev);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops,  s, "nvidia-gtx560m-mmio", NV_BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar1, OBJECT(s), &bar1_ops,  s, "nvidia-gtx560m-vram", NV_BAR1_SIZE);
    pci_register_bar(pdev, 1, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar1);
    memory_region_init_io(&s->bar3, OBJECT(s), &bar35_ops, s, "nvidia-gtx560m-ramin",NV_BAR3_SIZE);
    pci_register_bar(pdev, 3, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar3);
    memory_region_init_io(&s->bar5, OBJECT(s), &bar35_ops, s, "nvidia-gtx560m-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vmstate_nvidia_gtx560m = {
    .name = "nvidia-gtx560m", .version_id = 1, .minimum_version_id = 1,
    .fields = (const VMStateField[]) {
        VMSTATE_PCI_DEVICE(parent_obj, NvidiaGtx560mState),
        VMSTATE_UINT32(intr_en,       NvidiaGtx560mState),
        VMSTATE_UINT32(pfifo_intr_en, NvidiaGtx560mState),
        VMSTATE_UINT32(clock_mhz,     NvidiaGtx560mState),
        VMSTATE_UINT32(gpu_count,       NvidiaGtx560mState            ),
        VMSTATE_UINT64(clock_last_ns, NvidiaGtx560mState),
        VMSTATE_END_OF_LIST()
    },
};

static const Property gpu_multi_props_NvidiaGtx560mState[] = {
    DEFINE_PROP_UINT32("gpu-count", NvidiaGtx560mState, gpu_count, 1),
};
static void gpu_class_init(ObjectClass *klass, const void *data)
{
    ...
    dc->desc         = "NVIDIA GeForce GTX 560M (GF116, Fermi, Mobile)";
    dc->vmsd         = &vmstate_nvidia_gtx560m;
    dc->hotpluggable = false;
    device_class_set_props(dc, gpu_multi_props_NvidiaGtx560mState);
    set_bit(DEVICE_CATEGORY_DISPLAY, dc->categories);
}

static const TypeInfo nvidia_gtx560m_info = {
    .name          = TYPE_NVIDIA_GTX560M,
    .parent        = TYPE_PCI_DEVICE,
    .instance_size = sizeof(NvidiaGtx560mState),
    .class_init    = gpu_class_init,
    .interfaces    = (InterfaceInfo[]) {
        { INTERFACE_CONVENTIONAL_PCI_DEVICE }, { },
    },
};
static void nvidia_gtx560m_register(void) { type_register_static(&nvidia_gtx560m_info); }
type_init(nvidia_gtx560m_register)
