/*
 * ATI Radeon 9700 Pro (R300, AGP 8x, 128MB DDR, 2002)
...
#define GPU_DEVICE_ID  0x4E44
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x4E45
...
#define GPU_VRAM_MB     128
#define TYPE_ATI_RADEON_9700PRO "ati-radeon-9700pro"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeon9700proState, ATI_RADEON_9700PRO)
struct AtiRadeon9700proState {
...
static uint32_t gpu_clk(AtiRadeon9700proState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeon9700proState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeon9700proState *s=(AtiRadeon9700proState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-9700pro-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-9700pro-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-9700pro-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-9700pro-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_9700pro={.name="ati-radeon-9700pro",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeon9700proState),VMSTATE_UINT32(intr_en,AtiRadeon9700proState),VMSTATE_UINT32(clock_mhz,AtiRadeon9700proState),VMSTATE_UINT64(clock_last_ns,AtiRadeon9700proState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeon9700proState[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeon9700proState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeon9700proState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeon9700proState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="ATI Radeon 9700 Pro (R300, AGP 8x, 128MB DDR, 2002)"; dc->vmsd=&vms_ati_radeon_9700pro; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRadeon9700proState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_radeon_9700pro={.name=TYPE_ATI_RADEON_9700PRO,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRadeon9700proState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_radeon_9700pro(void){type_register_static(&ti_ati_radeon_9700pro);}
type_init(reg_ati_radeon_9700pro)
