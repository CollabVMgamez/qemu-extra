/*
 * Intel HD Graphics 4000 (Ivy Bridge Server GT2, 16EU)
 * Vendor: 0x8086, Device: 0x016A
 * Class: 0x0300, PCIe
 * Usage: -device intel-hd-4000-ivb-server
...
#define TYPE_INTELHD4000IVBSERVER "intel-hd-4000-ivb-server"
OBJECT_DECLARE_SIMPLE_TYPE(IntelHd4000IvbServerState, INTELHD4000IVBSERVER)
struct IntelHd4000IvbServerState {
...
static uint32_t igpu_clk(IntelHd4000IvbServerState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelHd4000IvbServerState *s = opaque;
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelHd4000IvbServerState *s = INTELHD4000IVBSERVER(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-hd-4000-ivb-server-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-hd-4000-ivb-server-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelHd4000IvbServerState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-hd-4000-ivb-server",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelHd4000IvbServerState),VMSTATE_UINT32(clock_mhz,IntelHd4000IvbServerState),VMSTATE_UINT64(clock_last_ns,IntelHd4000IvbServerState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel HD Graphics 4000 (Ivy Bridge Server GT2, 16EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELHD4000IVBSERVER,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelHd4000IvbServerState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
