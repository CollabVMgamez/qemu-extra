/*
 * Intel UHD Graphics (Elkhart Lake GT2, 24EU)
 * Vendor: 0x8086, Device: 0x4571
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-elkhart-lake
...
#define GPU_DEV      0x4571
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2271
#define GPU_REV      0x00
...
#define TYPE_INTELUHDELKHARTLAKE "intel-uhd-elkhart-lake"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhdElkhartLakeState, INTELUHDELKHARTLAKE)
struct IntelUhdElkhartLakeState {
...
static uint32_t igpu_clk(IntelUhdElkhartLakeState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelUhdElkhartLakeState *s = opaque;
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhdElkhartLakeState *s = INTELUHDELKHARTLAKE(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-elkhart-lake-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-elkhart-lake-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhdElkhartLakeState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-elkhart-lake",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhdElkhartLakeState),VMSTATE_UINT32(clock_mhz,IntelUhdElkhartLakeState),VMSTATE_UINT64(clock_last_ns,IntelUhdElkhartLakeState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics (Elkhart Lake GT2, 24EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHDELKHARTLAKE,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhdElkhartLakeState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
