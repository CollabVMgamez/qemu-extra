/*
 * Intel UHD Graphics G1 (Ice Lake GT1.5, 32EU)
 * Vendor: 0x8086, Device: 0x8A5C
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-g1
...
#define GPU_DEV      0x8A5C
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x225C
#define GPU_REV      0x00
...
#define TYPE_INTELUHDG1 "intel-uhd-g1"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhdG1State, INTELUHDG1)
struct IntelUhdG1State {
...
static uint32_t igpu_clk(IntelUhdG1State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhdG1State *s = INTELUHDG1(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-g1-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-g1-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhdG1State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-g1",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhdG1State),VMSTATE_UINT32(clock_mhz,IntelUhdG1State),VMSTATE_UINT64(clock_last_ns,IntelUhdG1State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics G1 (Ice Lake GT1.5, 32EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHDG1,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhdG1State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
