/*
 * Intel HD Graphics 5000 (Haswell ULT GT3, 40EU)
 * Vendor: 0x8086, Device: 0x0A26
 * Class: 0x0300, PCIe
 * Usage: -device intel-hd-5000
...
#define TYPE_INTELHD5000 "intel-hd-5000"
OBJECT_DECLARE_SIMPLE_TYPE(IntelHd5000State, INTELHD5000)
struct IntelHd5000State {
...
static uint32_t igpu_clk(IntelHd5000State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelHd5000State *s = INTELHD5000(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-hd-5000-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-hd-5000-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelHd5000State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-hd-5000",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelHd5000State),VMSTATE_UINT32(clock_mhz,IntelHd5000State),VMSTATE_UINT64(clock_last_ns,IntelHd5000State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel HD Graphics 5000 (Haswell ULT GT3, 40EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELHD5000,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelHd5000State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
