/*
 * Google Cloud TPU v2 (Skylake, 8x MXU, 64GB HBM, PCIe gen3 x16, 2017)
 * QEMU stub device for P4 fork.
 *
 * PCI IDs from Google's open-source TPU driver:
 *   Vendor: 0x1AE0 (Google)
 *   Device: 0x0027 (Cloud TPU v2)
 */
...
#define TPU_DEVICE_ID   0x0027
#define TPU_SUBSYS_VID  0x1AE0
#define TPU_SUBSYS_DID  0x0027
...
#define TPU_CLK_BASE    700
#define TPU_CLK_BOOST   700
#define TPU_MEM_MHZ     800
#define TPU_PCIE_GEN    3
#define TYPE_GOOGLE_TPU_V2 "google-tpu-v2"
OBJECT_DECLARE_SIMPLE_TYPE(GoogleTpuV2State, GOOGLE_TPU_V2)
struct GoogleTpuV2State {
...
static uint32_t tpu_clk(GoogleTpuV2State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    GoogleTpuV2State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
    /* Chip ID / version register */
    case 0x000000: return 0x00027001; /* TPU v2, rev 1 */
...
static void tpu_realize(PCIDevice *p, Error **e) {
    GoogleTpuV2State *s = (GoogleTpuV2State *)p;
    ...
    tpu_caps(p);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0ops, s, "google-tpu-v2-mmio", TPU_BAR0_SIZE);
    pci_register_bar(p, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2ops, s, "google-tpu-v2-hbm", TPU_BAR2_SIZE);
    pci_register_bar(p, 2, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar2);
}
static const VMStateDescription vms_google_tpu_v2 = {
    .name="google-tpu-v2",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,GoogleTpuV2State),VMSTATE_UINT32(clock_mhz,GoogleTpuV2State),VMSTATE_UINT64(clock_last_ns,GoogleTpuV2State),VMSTATE_END_OF_LIST()}
};
static const Property tpu_props[] = {
    DEFINE_PROP_STRING("chip-name", GoogleTpuV2State, chip_name),
    DEFINE_PROP_UINT32("chip-count", GoogleTpuV2State, chip_count, 1),
    DEFINE_PROP_STRING("board-partner", GoogleTpuV2State, board_partner),
};
static void tpu_class_init(ObjectClass *k, const void *d) {
    ...
    dc->desc = "Google Cloud TPU v2 (Skylake MXU, 64GB HBM, PCIe gen3, 2017)";
    dc->vmsd = &vms_google_tpu_v2; dc->hotpluggable = false;
    device_class_set_props(dc, tpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY, dc->categories);
}
static const TypeInfo ti_google_tpu_v2 = {
    .name=TYPE_GOOGLE_TPU_V2,.parent=TYPE_PCI_DEVICE,
    .instance_size=sizeof(GoogleTpuV2State),.class_init=tpu_class_init,
    .interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}
};
static void reg_google_tpu_v2(void) { type_register_static(&ti_google_tpu_v2); }
type_init(reg_google_tpu_v2)
