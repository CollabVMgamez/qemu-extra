/*
 * NVIDIA GeForce2 MX (NV11, AGP 4x, 32MB DDR, 2001)
...
#define GPU_DEVICE_ID  0x0110
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x0110
#define GPU_REVISION   0x00
#define GPU_CLASS      0x0300
#define GPU_MEM_MHZ    175
...
#define GPU_CLK_BASE    175
#define GPU_CLK_BOOST   175
#define AGP_CAP_OFFSET  0x40

#define TYPE_NVIDIA_NV11_GEFORCE2MX "nvidia-nv11-geforce2mx"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaNv11Geforce2mxState, NVIDIA_NV11_GEFORCE2MX)
struct NvidiaNv11Geforce2mxState {
...
static uint32_t gpu_clk(NvidiaNv11Geforce2mxState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaNv11Geforce2mxState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaNv11Geforce2mxState *s=(NvidiaNv11Geforce2mxState*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-nv11-geforce2mx-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b135ops,s,"nvidia-nv11-geforce2mx-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b135ops,s,"nvidia-nv11-geforce2mx-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b135ops,s,"nvidia-nv11-geforce2mx-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_nvidia_nv11_geforce2mx[]={DEFINE_PROP_UINT32("gpu-count",NvidiaNv11Geforce2mxState,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",NvidiaNv11Geforce2mxState,gpu_name),
    DEFINE_PROP_STRING("board-partner",NvidiaNv11Geforce2mxState,board_partner)};
static const VMStateDescription vms_nvidia_nv11_geforce2mx={.name="nvidia-nv11-geforce2mx",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaNv11Geforce2mxState),VMSTATE_UINT32(intr_en,NvidiaNv11Geforce2mxState),VMSTATE_UINT32(clock_mhz,NvidiaNv11Geforce2mxState),VMSTATE_UINT64(clock_last_ns,NvidiaNv11Geforce2mxState),
        VMSTATE_UINT32(gpu_count,NvidiaNv11Geforce2mxState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x0110;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA GeForce2 MX (NV11, AGP 4x, 32MB DDR, 2001)"; dc->vmsd=&vms_nvidia_nv11_geforce2mx; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_nvidia_nv11_geforce2mx);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_nv11_geforce2mx={.name=TYPE_NVIDIA_NV11_GEFORCE2MX,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaNv11Geforce2mxState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_nv11_geforce2mx(void){type_register_static(&ti_nvidia_nv11_geforce2mx);}
type_init(reg_nvidia_nv11_geforce2mx)
