/*
 * NVIDIA H100 SXM5 (GH100, Hopper, 80GB HBM3)
...
#define TYPE_NVIDIA_H100_SXM5 "nvidia-h100-sxm5"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaH100Sxm5State, NVIDIA_H100_SXM5)
struct NvidiaH100Sxm5State {
...
static uint32_t gpu_clk(NvidiaH100Sxm5State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaH100Sxm5State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaH100Sxm5State *s=(NvidiaH100Sxm5State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-h100-sxm5-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-h100-sxm5-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-h100-sxm5-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-h100-sxm5-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_h100_sxm5={.name="nvidia-h100-sxm5",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaH100Sxm5State),VMSTATE_UINT32(intr_en,NvidiaH100Sxm5State),VMSTATE_UINT32(pfifo_intr_en,NvidiaH100Sxm5State),VMSTATE_UINT32(clock_mhz,NvidiaH100Sxm5State),VMSTATE_UINT64(clock_last_ns,NvidiaH100Sxm5State),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="NVIDIA H100 SXM5 (GH100, Hopper, 80GB HBM3)"; dc->vmsd=&vms_nvidia_h100_sxm5; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaH100Sxm5State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_h100_sxm5={.name=TYPE_NVIDIA_H100_SXM5,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaH100Sxm5State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_h100_sxm5(void){type_register_static(&ti_nvidia_h100_sxm5);}
type_init(reg_nvidia_h100_sxm5)
