/*
 * Intel Iris Pro Graphics 5200 (Haswell CRW GT3e, 40EU + 128MB eDRAM)
 * Vendor: 0x8086, Device: 0x0D22
 * Class: 0x0300, PCIe
 * Usage: -device intel-iris-pro-5200
...
#define GPU_DEV      0x0D22
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2322
#define GPU_REV      0x06
...
#define TYPE_INTELIRISPRO5200 "intel-iris-pro-5200"
OBJECT_DECLARE_SIMPLE_TYPE(IntelIrisPro5200State, INTELIRISPRO5200)
struct IntelIrisPro5200State {
...
static uint32_t igpu_clk(IntelIrisPro5200State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelIrisPro5200State *s = opaque;
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelIrisPro5200State *s = INTELIRISPRO5200(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-iris-pro-5200-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-iris-pro-5200-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelIrisPro5200State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-iris-pro-5200",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelIrisPro5200State),VMSTATE_UINT32(clock_mhz,IntelIrisPro5200State),VMSTATE_UINT64(clock_last_ns,IntelIrisPro5200State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Iris Pro Graphics 5200 (Haswell CRW GT3e, 40EU + 128MB eDRAM)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELIRISPRO5200,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelIrisPro5200State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
