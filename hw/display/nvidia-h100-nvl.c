/*
 * NVIDIA H100 NVL (GH100, Hopper, 94GB HBM3)
...
#define GPU_PFB_PARTS   0x0000005E
...
#define TYPE_NVIDIA_H100_NVL "nvidia-h100-nvl"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaH100NvlState, NVIDIA_H100_NVL)
struct NvidiaH100NvlState {
...
static uint32_t gpu_clk(NvidiaH100NvlState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaH100NvlState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-h100-nvl-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-h100-nvl-vram",NV_BAR1_SIZE);
...
static void ci(ObjectClass *k, const void *d) {
    ...
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x2322;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA H100 NVL (GH100, Hopper, 94GB HBM3)"; dc->vmsd=&vms_nvidia_h100_nvl; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaH100NvlState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_h100_nvl={.name=TYPE_NVIDIA_H100_NVL,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaH100NvlState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_h100_nvl(void){type_register_static(&ti_nvidia_h100_nvl);}
type_init(reg_nvidia_h100_nvl)
