/*
 * NVIDIA RIVA 128 (NV3, AGP 2x, 4MB, 1997)
...
#define GPU_DEVICE_ID  0x0018
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x0018
...
#define TYPE_NVIDIA_NV3_RIVA128 "nvidia-nv3-riva128"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaNv3Riva128State, NVIDIA_NV3_RIVA128)
struct NvidiaNv3Riva128State {
...
static uint32_t gpu_clk(NvidiaNv3Riva128State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaNv3Riva128State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
    ...
    case ATI_MEM_SIZE:     return (uint32_t)(4 * 1024 * 1024);
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaNv3Riva128State *s=(NvidiaNv3Riva128State*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-nv3-riva128-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b135ops,s,"nvidia-nv3-riva128-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b135ops,s,"nvidia-nv3-riva128-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b135ops,s,"nvidia-nv3-riva128-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_nvidia_nv3_riva128[]={DEFINE_PROP_UINT32("gpu-count",NvidiaNv3Riva128State,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",NvidiaNv3Riva128State,gpu_name),
    DEFINE_PROP_STRING("board-partner",NvidiaNv3Riva128State,board_partner)};
static const VMStateDescription vms_nvidia_nv3_riva128={.name="nvidia-nv3-riva128",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaNv3Riva128State),VMSTATE_UINT32(intr_en,NvidiaNv3Riva128State),VMSTATE_UINT32(clock_mhz,NvidiaNv3Riva128State),VMSTATE_UINT64(clock_last_ns,NvidiaNv3Riva128State),
        VMSTATE_UINT32(gpu_count,NvidiaNv3Riva128State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x0018;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA RIVA 128 (NV3, AGP 2x, 4MB, 1997)"; dc->vmsd=&vms_nvidia_nv3_riva128; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_nvidia_nv3_riva128);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_nv3_riva128={.name=TYPE_NVIDIA_NV3_RIVA128,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaNv3Riva128State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_nv3_riva128(void){type_register_static(&ti_nvidia_nv3_riva128);}
type_init(reg_nvidia_nv3_riva128)
