/*
 * NVIDIA GeForce GTX 650 Ti (GK106, Kepler)
...
#define GPU_DEVICE_ID    0x11C6
#define GPU_SUBSYS_VID   0x10DE
#define GPU_SUBSYS_DID   0x0961
...
#define TYPE_NVIDIA_GTX650TI "nvidia-gtx650ti"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx650tiState, NVIDIA_GTX650TI)

struct NvidiaGtx650tiState {
...
static uint32_t gpu_get_clock(NvidiaGtx650tiState *s)
{
...
{
    NvidiaGtx650tiState *s = (NvidiaGtx650tiState *)pdev; s->gpu_count=1;
    ...
    gpu_setup_caps(pdev);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops,  s, "nvidia-gtx650ti-mmio", NV_BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar1, OBJECT(s), &bar1_ops,  s, "nvidia-gtx650ti-vram", NV_BAR1_SIZE);
    pci_register_bar(pdev, 1, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar1);
    memory_region_init_io(&s->bar3, OBJECT(s), &bar35_ops, s, "nvidia-gtx650ti-ramin",NV_BAR3_SIZE);
    pci_register_bar(pdev, 3, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar3);
    memory_region_init_io(&s->bar5, OBJECT(s), &bar35_ops, s, "nvidia-gtx650ti-vgaio",NV_BAR5_SIZE);
    pci_register_bar(pdev, 5, PCI_BASE_ADDRESS_SPACE_MEMORY, &s->bar5);if(s->gpu_count>1){PCIBus*bus=pci_get_bus(pdev);const char*tn=object_get_typename(OBJECT(s));for(uint32_t i=1;i<s->gpu_count&&i<8;i++){PCIDevice*ex=pci_create_simple(bus,-1,tn);if(ex){qdev_prop_set_uint32(DEVICE(ex),"gpu-count",1);Error*le=NULL;qdev_realize(DEVICE(ex),&bus->qbus,&le);if(le){error_free(le);break;}}}}
}

static const VMStateDescription vmstate_nvidia_gtx650ti = {
    .name = "nvidia-gtx650ti", .version_id = 1, .minimum_version_id = 1,
    .fields = (const VMStateField[]) {
        VMSTATE_PCI_DEVICE(parent_obj, NvidiaGtx650tiState),
        VMSTATE_UINT32(intr_en,       NvidiaGtx650tiState),
        VMSTATE_UINT32(pfifo_intr_en, NvidiaGtx650tiState),
        VMSTATE_UINT32(clock_mhz,     NvidiaGtx650tiState),
        VMSTATE_UINT32(gpu_count,       NvidiaGtx650tiState           ),
        VMSTATE_UINT64(clock_last_ns, NvidiaGtx650tiState),
        VMSTATE_END_OF_LIST()
    },
};

static const Property gpu_multi_props_NvidiaGtx650tiState[] = {
    DEFINE_PROP_UINT32("gpu-count", NvidiaGtx650tiState, gpu_count, 1),
};
static void gpu_class_init(ObjectClass *klass, const void *data)
{
    ...
    dc->desc         = "NVIDIA GeForce GTX 650 Ti (GK106, Kepler)";
    dc->vmsd         = &vmstate_nvidia_gtx650ti;
    dc->hotpluggable = false;
    device_class_set_props(dc, gpu_multi_props_NvidiaGtx650tiState);
    set_bit(DEVICE_CATEGORY_DISPLAY, dc->categories);
}

static const TypeInfo nvidia_gtx650ti_info = {
    .name          = TYPE_NVIDIA_GTX650TI,
    .parent        = TYPE_PCI_DEVICE,
    .instance_size = sizeof(NvidiaGtx650tiState),
    .class_init    = gpu_class_init,
    .interfaces    = (InterfaceInfo[]) {
        { INTERFACE_CONVENTIONAL_PCI_DEVICE }, { },
    },
};
static void nvidia_gtx650ti_register(void) { type_register_static(&nvidia_gtx650ti_info); }
type_init(nvidia_gtx650ti_register)
