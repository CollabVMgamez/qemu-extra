/*
 * NVIDIA Tesla V100S PCIe (GV100, Volta, 32GB HBM2)
...
#define GPU_DEVICE_ID  0x1DF6
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x1213
...
#define GPU_PFB_PARTS   0x00000020
#define GPU_PFB_REFCTRL 0x80000083
#define VPD_CAP_OFFSET 0x70
#define GPU_CLK_BASE    1245
#define GPU_MEM_MHZ     1134
#define GPU_CLK_BOOST   1597
#define GPU_PCIE_GEN    3
#define TYPE_NVIDIA_V100S "nvidia-v100s"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaV100sState, NVIDIA_V100S)
struct NvidiaV100sState {
...
static uint32_t gpu_clk(NvidiaV100sState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaV100sState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaV100sState *s=(NvidiaV100sState*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-v100s-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-v100s-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-v100s-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-v100s-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_v100s={.name="nvidia-v100s",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaV100sState),VMSTATE_UINT32(intr_en,NvidiaV100sState),VMSTATE_UINT32(pfifo_intr_en,NvidiaV100sState),VMSTATE_UINT32(clock_mhz,NvidiaV100sState),VMSTATE_UINT64(clock_last_ns,NvidiaV100sState),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x1DF6;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA Tesla V100S PCIe (GV100, Volta, 32GB HBM2)"; dc->vmsd=&vms_nvidia_v100s; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaV100sState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_v100s={.name=TYPE_NVIDIA_V100S,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaV100sState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_v100s(void){type_register_static(&ti_nvidia_v100s);}
type_init(reg_nvidia_v100s)
