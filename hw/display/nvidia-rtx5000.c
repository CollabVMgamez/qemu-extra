/*
 * NVIDIA Quadro RTX 5000 (TU104, Turing, 16GB GDDR6)
...
#define GPU_CLK_BASE    1164
#define GPU_MEM_MHZ     1750
#define GPU_CLK_BOOST   1620
#define GPU_PCIE_GEN    3
#define TYPE_NVIDIA_RTX5000 "nvidia-rtx5000"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaRtx5000State, NVIDIA_RTX5000)
struct NvidiaRtx5000State {
...
static uint32_t gpu_clk(NvidiaRtx5000State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaRtx5000State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaRtx5000State *s=(NvidiaRtx5000State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-rtx5000-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-rtx5000-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-rtx5000-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-rtx5000-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_rtx5000={.name="nvidia-rtx5000",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaRtx5000State),VMSTATE_UINT32(intr_en,NvidiaRtx5000State),VMSTATE_UINT32(pfifo_intr_en,NvidiaRtx5000State),VMSTATE_UINT32(clock_mhz,NvidiaRtx5000State),VMSTATE_UINT64(clock_last_ns,NvidiaRtx5000State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaRtx5000State[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaRtx5000State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaRtx5000State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaRtx5000State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="NVIDIA Quadro RTX 5000 (TU104, Turing, 16GB GDDR6)"; dc->vmsd=&vms_nvidia_rtx5000; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaRtx5000State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_rtx5000={.name=TYPE_NVIDIA_RTX5000,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaRtx5000State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_rtx5000(void){type_register_static(&ti_nvidia_rtx5000);}
type_init(reg_nvidia_rtx5000)
