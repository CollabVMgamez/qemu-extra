/*
 * NVIDIA GeForce GTX 480M (GF100, Fermi, Mobile)
...
#define GPU_DEVICE_ID    0x0637
#define GPU_SUBSYS_VID   0x10DE
#define GPU_SUBSYS_DID   0x0845
...
#define TYPE_NVIDIA_GTX480M "nvidia-gtx480m"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx480mState, NVIDIA_GTX480M)
...
static uint32_t gpu_get_clock(NvidiaGtx480mState *s)
{
...
{
    NvidiaGtx480mState *s = (NvidiaGtx480mState *)pdev; s->gpu_count=1;
    ...
    gpu_setup_caps(pdev);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops,  s, "nvidia-gtx480m-mmio", NV_BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar1, OBJECT(s), &bar1_ops,  s, "nvidia-gtx480m-vram", NV_BAR1_SIZE);
    pci_register_bar(pdev, 1, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar1);
    memory_region_init_io(&s->bar3, OBJECT(s), &bar35_ops, s, "nvidia-gtx480m-ramin",NV_BAR3_SIZE);
    pci_register_bar(pdev, 3, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar3);
    memory_region_init_io(&s->bar5, OBJECT(s), &bar35_ops, s, "nvidia-gtx480m-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vmstate_nvidia_gtx480m = {
    .name = "nvidia-gtx480m", .version_id = 1, .minimum_version_id = 1,
    .fields = (const VMStateField[]) {
        VMSTATE_PCI_DEVICE(parent_obj, NvidiaGtx480mState),
...
static const TypeInfo nvidia_gtx480m_info = {
    .name          = TYPE_NVIDIA_GTX480M,
    .parent        = TYPE_PCI_DEVICE,
    .instance_size = sizeof(NvidiaGtx480mState),
    .class_init    = gpu_class_init,
    .interfaces    = (InterfaceInfo[]) {
        { INTERFACE_CONVENTIONAL_PCI_DEVICE }, { },
    },
};
static void nvidia_gtx480m_register(void) { type_register_static(&nvidia_gtx480m_info); }
type_init(nvidia_gtx480m_register)
