/*
 * Intel Iris Xe2 Graphics (Lunar Lake GT2, 128EU)
 * Vendor: 0x8086, Device: 0x6420
 * Class: 0x0300, PCIe
 * Usage: -device intel-iris-xe2-lnl
...
#define GPU_DEV      0x6420
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2220
#define GPU_REV      0x00
#define GPU_CLASS    0x0300
#define BAR0_SIZE (32*MiB)
#define BAR2_SIZE (512*MiB)
#define GPU_CLK_BASE  300
#define GPU_CLK_BOOST 1800
#define TYPE_INTELIRISXE2LNL "intel-iris-xe2-lnl"
OBJECT_DECLARE_SIMPLE_TYPE(IntelIrisXe2LnlState, INTELIRISXE2LNL)
struct IntelIrisXe2LnlState {
...
static uint32_t igpu_clk(IntelIrisXe2LnlState *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelIrisXe2LnlState *s = INTELIRISXE2LNL(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-iris-xe2-lnl-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-iris-xe2-lnl-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelIrisXe2LnlState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-iris-xe2-lnl",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelIrisXe2LnlState),VMSTATE_UINT32(clock_mhz,IntelIrisXe2LnlState),VMSTATE_UINT64(clock_last_ns,IntelIrisXe2LnlState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Iris Xe2 Graphics (Lunar Lake GT2, 128EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELIRISXE2LNL,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelIrisXe2LnlState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
