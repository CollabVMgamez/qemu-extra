/*
 * Qualcomm Adreno 660 (Snapdragon 888, integrated)
...
#define TYPE_ADRENO_660 "adreno-660"
OBJECT_DECLARE_SIMPLE_TYPE(Adreno660State, ADRENO_660)
struct Adreno660State {
...
static uint32_t gpu_clk(Adreno660State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    Adreno660State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    Adreno660State *s=(Adreno660State*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"adreno-660-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&bops,s,"adreno-660-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&bops,s,"adreno-660-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&bops,s,"adreno-660-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_adreno_660[]={DEFINE_PROP_UINT32("gpu-count",Adreno660State,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",Adreno660State,gpu_name)};
static const VMStateDescription vms_adreno_660={.name="adreno-660",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,Adreno660State),VMSTATE_UINT32(clock_mhz,Adreno660State),VMSTATE_UINT64(clock_last_ns,Adreno660State),VMSTATE_UINT32(gpu_count,Adreno660State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x6060;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="Qualcomm Adreno 660 (Snapdragon 888, integrated)"; dc->vmsd=&vms_adreno_660; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_adreno_660);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_adreno_660={.name=TYPE_ADRENO_660,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(Adreno660State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_adreno_660(void){type_register_static(&ti_adreno_660);}
type_init(reg_adreno_660)
