/*
 * NVIDIA Tesla P100 PCIe (GP100, Pascal, 16GB HBM2)
...
#define GPU_DEVICE_ID  0x15F8
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x15F9
...
#define GPU_CLK_BOOST   1328
#define GPU_PCIE_GEN    3
#define TYPE_NVIDIA_P100_PCIE "nvidia-p100-pcie"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaP100PcieState, NVIDIA_P100_PCIE)
struct NvidiaP100PcieState {
...
static uint32_t gpu_clk(NvidiaP100PcieState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaP100PcieState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaP100PcieState *s=(NvidiaP100PcieState*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-p100-pcie-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-p100-pcie-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-p100-pcie-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-p100-pcie-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_p100_pcie={.name="nvidia-p100-pcie",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaP100PcieState),VMSTATE_UINT32(intr_en,NvidiaP100PcieState),VMSTATE_UINT32(pfifo_intr_en,NvidiaP100PcieState),VMSTATE_UINT32(clock_mhz,NvidiaP100PcieState),VMSTATE_UINT64(clock_last_ns,NvidiaP100PcieState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaP100PcieState[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaP100PcieState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaP100PcieState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaP100PcieState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x15F8;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA Tesla P100 PCIe (GP100, Pascal, 16GB HBM2)"; dc->vmsd=&vms_nvidia_p100_pcie; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaP100PcieState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_p100_pcie={.name=TYPE_NVIDIA_P100_PCIE,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaP100PcieState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_p100_pcie(void){type_register_static(&ti_nvidia_p100_pcie);}
type_init(reg_nvidia_p100_pcie)
