/*
 * NVIDIA Tesla V100 SXM2 (GV100, Volta, 16GB HBM2)
...
#define GPU_DEVICE_ID  0x1DB1
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x1211
...
#define GPU_PMC_BOOT_0  0x148000A1
...
#define GPU_CLK_BASE    1132
#define GPU_MEM_MHZ     877
#define GPU_CLK_BOOST   1530
#define GPU_PCIE_GEN    3
#define TYPE_NVIDIA_V100_SXM2 "nvidia-v100-sxm2"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaV100Sxm2State, NVIDIA_V100_SXM2)
struct NvidiaV100Sxm2State {
...
static uint32_t gpu_clk(NvidiaV100Sxm2State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaV100Sxm2State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaV100Sxm2State *s=(NvidiaV100Sxm2State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-v100-sxm2-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-v100-sxm2-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-v100-sxm2-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-v100-sxm2-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_v100_sxm2={.name="nvidia-v100-sxm2",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaV100Sxm2State),VMSTATE_UINT32(intr_en,NvidiaV100Sxm2State),VMSTATE_UINT32(pfifo_intr_en,NvidiaV100Sxm2State),VMSTATE_UINT32(clock_mhz,NvidiaV100Sxm2State),VMSTATE_UINT64(clock_last_ns,NvidiaV100Sxm2State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaV100Sxm2State[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaV100Sxm2State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaV100Sxm2State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaV100Sxm2State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x1DB1;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA Tesla V100 SXM2 (GV100, Volta, 16GB HBM2)"; dc->vmsd=&vms_nvidia_v100_sxm2; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaV100Sxm2State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_v100_sxm2={.name=TYPE_NVIDIA_V100_SXM2,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaV100Sxm2State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_v100_sxm2(void){type_register_static(&ti_nvidia_v100_sxm2);}
type_init(reg_nvidia_v100_sxm2)
