/*
 * ATI Radeon 9100 (R200, AGP 4x, 128MB DDR, 2002)
...
#define GPU_DEVICE_ID  0x514D
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x514D
...
#define TYPE_ATI_RADEON_9100 "ati-radeon-9100"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeon9100State, ATI_RADEON_9100)
struct AtiRadeon9100State {
...
static uint32_t gpu_clk(AtiRadeon9100State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeon9100State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeon9100State *s=(AtiRadeon9100State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-9100-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-9100-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-9100-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-9100-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_9100={.name="ati-radeon-9100",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeon9100State),VMSTATE_UINT32(intr_en,AtiRadeon9100State),VMSTATE_UINT32(clock_mhz,AtiRadeon9100State),VMSTATE_UINT64(clock_last_ns,AtiRadeon9100State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeon9100State[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeon9100State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeon9100State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeon9100State, board_partner),
};
...
static void reg_ati_radeon_9100(void){type_register_static(&ti_ati_radeon_9100);}
type_init(reg_ati_radeon_9100)
