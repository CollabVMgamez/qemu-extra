/*
 * NVIDIA GeForce FX 5950 Ultra (NV38, AGP 8x, 256MB DDR, 2003)
...
#define GPU_DEVICE_ID  0x0350
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x0350
...
#define GPU_CLK_BASE    500
#define GPU_CLK_BOOST   500
#define AGP_CAP_OFFSET  0x40

#define TYPE_NVIDIA_NV38_GEFORCEFX5900ULTRA "nvidia-nv38-geforcefx5900ultra"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaNv38Geforcefx5900ultraState, NVIDIA_NV38_GEFORCEFX5900ULTRA)
struct NvidiaNv38Geforcefx5900ultraState {
...
static uint32_t gpu_clk(NvidiaNv38Geforcefx5900ultraState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaNv38Geforcefx5900ultraState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
    ...
    case ATI_MEM_SIZE:     return (uint32_t)(256 * 1024 * 1024);
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaNv38Geforcefx5900ultraState *s=(NvidiaNv38Geforcefx5900ultraState*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-nv38-geforcefx5900ultra-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b135ops,s,"nvidia-nv38-geforcefx5900ultra-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b135ops,s,"nvidia-nv38-geforcefx5900ultra-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b135ops,s,"nvidia-nv38-geforcefx5900ultra-vgaio",NV_BAR5_SIZE);
...
static const Property gpu_props_nvidia_nv38_geforcefx5900ultra[]={DEFINE_PROP_UINT32("gpu-count",NvidiaNv38Geforcefx5900ultraState,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",NvidiaNv38Geforcefx5900ultraState,gpu_name),
    DEFINE_PROP_STRING("board-partner",NvidiaNv38Geforcefx5900ultraState,board_partner)};
static const VMStateDescription vms_nvidia_nv38_geforcefx5900ultra={.name="nvidia-nv38-geforcefx5900ultra",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaNv38Geforcefx5900ultraState),VMSTATE_UINT32(intr_en,NvidiaNv38Geforcefx5900ultraState),VMSTATE_UINT32(clock_mhz,NvidiaNv38Geforcefx5900ultraState),VMSTATE_UINT64(clock_last_ns,NvidiaNv38Geforcefx5900ultraState),
        VMSTATE_UINT32(gpu_count,NvidiaNv38Geforcefx5900ultraState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x0350;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA GeForce FX 5950 Ultra (NV38, AGP 8x, 256MB DDR, 2003)"; dc->vmsd=&vms_nvidia_nv38_geforcefx5900ultra; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_nvidia_nv38_geforcefx5900ultra);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_nv38_geforcefx5900ultra={.name=TYPE_NVIDIA_NV38_GEFORCEFX5900ULTRA,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaNv38Geforcefx5900ultraState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_nv38_geforcefx5900ultra(void){type_register_static(&ti_nvidia_nv38_geforcefx5900ultra);}
type_init(reg_nvidia_nv38_geforcefx5900ultra)
