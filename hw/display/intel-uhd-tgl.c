/*
 * Intel UHD Graphics (Tiger Lake-U GT1, 48EU)
 * Vendor: 0x8086, Device: 0x9A60
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-tgl
...
#define GPU_DEV      0x9A60
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2260
#define GPU_REV      0x00
...
#define TYPE_INTELUHDTGL "intel-uhd-tgl"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhdTglState, INTELUHDTGL)
struct IntelUhdTglState {
...
static uint32_t igpu_clk(IntelUhdTglState *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhdTglState *s = INTELUHDTGL(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-tgl-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-tgl-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhdTglState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-tgl",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhdTglState),VMSTATE_UINT32(clock_mhz,IntelUhdTglState),VMSTATE_UINT64(clock_last_ns,IntelUhdTglState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics (Tiger Lake-U GT1, 48EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHDTGL,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhdTglState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
