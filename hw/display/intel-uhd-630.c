/*
 * Intel UHD Graphics 630 (Coffee Lake GT2, integrated)
 * Vendor: 0x8086, Device: 0x3E92
 * Class: 0x0300 (VGA compatible), PCIe x16 3.0 (typically)
 * Execution Units: 24 EU, Base 350MHz, Boost 1200MHz
 * Usage: -device intel-uhd-630
...
#define GPU_DEV      0x3E92
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2212
#define GPU_REV      0x00
#define GPU_CLASS    0x0300
#define BAR0_SIZE (16*MiB)
#define BAR2_SIZE (256*MiB)
#define GPU_CLK_BASE  350
#define GPU_CLK_BOOST 1200
#define TYPE_INTEL_UHD_630 "intel-uhd-630"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUHD630State, INTEL_UHD_630)
struct IntelUHD630State {
...
static uint32_t igpu_clk(IntelUHD630State *s) {
...
/* Intel MMIO register stubs — GT mailbox / render clock */
#define MBOX_ADDR 0x138124  /* GT0_PERF_LIMIT_REASONS */
#define RPSTAT1   0x0A01C   /* GT frequency status */
#define CDCLK_CTL 0x46000
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelUHD630State *s = opaque;
    uint32_t clk = igpu_clk(s);
    switch (addr) {
    case RPSTAT1:    return (clk & 0x1FF) << 23; /* RP0 freq field */
    case CDCLK_CTL:  return 0x00000002; /* CDCLK 337.5MHz */
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUHD630State *s = INTEL_UHD_630(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-630-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-630-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUHD630State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-630",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUHD630State),VMSTATE_UINT32(clock_mhz,IntelUHD630State),VMSTATE_UINT64(clock_last_ns,IntelUHD630State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics 630 (Coffee Lake GT2, 24EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTEL_UHD_630,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUHD630State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
