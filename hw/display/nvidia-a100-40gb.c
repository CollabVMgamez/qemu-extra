/*
 * NVIDIA A100 SXM4 (GA100, Ampere, 40GB HBM2e)
...
#define GPU_DEVICE_ID  0x20B0
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x145F
...
#define GPU_PMC_BOOT_0  0x178000A1
#define GPU_PFB_BOOT_0  0x00000015
#define GPU_PFB_PARTS   0x00000028
#define GPU_PFB_REFCTRL 0x80000083
#define VPD_CAP_OFFSET 0x70
#define GPU_CLK_BASE    765
#define GPU_MEM_MHZ     1215
#define GPU_CLK_BOOST   1410
#define GPU_PCIE_GEN    4
#define TYPE_NVIDIA_A100_40GB "nvidia-a100-40gb"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaA10040gbState, NVIDIA_A100_40GB)
struct NvidiaA10040gbState {
...
static uint32_t gpu_clk(NvidiaA10040gbState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaA10040gbState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaA10040gbState *s=(NvidiaA10040gbState*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-a100-40gb-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-a100-40gb-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-a100-40gb-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-a100-40gb-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_a100_40gb={.name="nvidia-a100-40gb",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaA10040gbState),VMSTATE_UINT32(intr_en,NvidiaA10040gbState),VMSTATE_UINT32(pfifo_intr_en,NvidiaA10040gbState),VMSTATE_UINT32(clock_mhz,NvidiaA10040gbState),VMSTATE_UINT64(clock_last_ns,NvidiaA10040gbState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaA10040gbState[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaA10040gbState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaA10040gbState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaA10040gbState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x20B0;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA A100 SXM4 (GA100, Ampere, 40GB HBM2e)"; dc->vmsd=&vms_nvidia_a100_40gb; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaA10040gbState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_a100_40gb={.name=TYPE_NVIDIA_A100_40GB,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaA10040gbState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_a100_40gb(void){type_register_static(&ti_nvidia_a100_40gb);}
type_init(reg_nvidia_a100_40gb)
