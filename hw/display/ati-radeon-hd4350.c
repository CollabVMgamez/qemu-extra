/*
 * ATI Radeon HD 4350 (RV710, PCIe, 512MB DDR2, 2008)
...
#define GPU_DEVICE_ID  0x954F
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x954F
...
#define TYPE_ATI_RADEON_HD4350 "ati-radeon-hd4350"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeonHd4350State, ATI_RADEON_HD4350)
struct AtiRadeonHd4350State {
...
static uint32_t gpu_clk(AtiRadeonHd4350State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeonHd4350State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeonHd4350State *s=(AtiRadeonHd4350State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-hd4350-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-hd4350-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-hd4350-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-hd4350-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_hd4350={.name="ati-radeon-hd4350",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeonHd4350State),VMSTATE_UINT32(intr_en,AtiRadeonHd4350State),VMSTATE_UINT32(clock_mhz,AtiRadeonHd4350State),VMSTATE_UINT64(clock_last_ns,AtiRadeonHd4350State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeonHd4350State[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeonHd4350State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeonHd4350State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeonHd4350State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x954F;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="ATI Radeon HD 4350 (RV710, PCIe, 512MB DDR2, 2008)"; dc->vmsd=&vms_ati_radeon_hd4350; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRadeonHd4350State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_radeon_hd4350={.name=TYPE_ATI_RADEON_HD4350,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRadeonHd4350State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_radeon_hd4350(void){type_register_static(&ti_ati_radeon_hd4350);}
type_init(reg_ati_radeon_hd4350)
