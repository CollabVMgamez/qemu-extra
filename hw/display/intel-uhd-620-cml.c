/*
 * Intel UHD Graphics 620 (Comet Lake-U GT2, 24EU)
 * Vendor: 0x8086, Device: 0x9B41
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-620-cml
...
#define GPU_DEV      0x9B41
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2241
#define GPU_REV      0x02
...
#define TYPE_INTELUHD620CML "intel-uhd-620-cml"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhd620CmlState, INTELUHD620CML)
struct IntelUhd620CmlState {
...
static uint32_t igpu_clk(IntelUhd620CmlState *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhd620CmlState *s = INTELUHD620CML(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-620-cml-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-620-cml-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhd620CmlState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-620-cml",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhd620CmlState),VMSTATE_UINT32(clock_mhz,IntelUhd620CmlState),VMSTATE_UINT64(clock_last_ns,IntelUhd620CmlState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics 620 (Comet Lake-U GT2, 24EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHD620CML,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhd620CmlState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
