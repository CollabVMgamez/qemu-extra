/*
 * AMD Radeon R9 Fury X (Fiji, PCIe x16, 4GB HBM, 2015)
...
#define GPU_DEVICE_ID  0x7300
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x7301
...
#define TYPE_AMD_RADEON_R9_FURYX "amd-radeon-r9-furyx"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRadeonR9FuryxState, AMD_RADEON_R9_FURYX)
struct AmdRadeonR9FuryxState {
...
static uint32_t gpu_clk(AmdRadeonR9FuryxState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRadeonR9FuryxState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRadeonR9FuryxState *s=(AmdRadeonR9FuryxState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-radeon-r9-furyx-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-radeon-r9-furyx-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-radeon-r9-furyx-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-radeon-r9-furyx-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_radeon_r9_furyx={.name="amd-radeon-r9-furyx",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRadeonR9FuryxState),VMSTATE_UINT32(intr_en,AmdRadeonR9FuryxState),VMSTATE_UINT32(clock_mhz,AmdRadeonR9FuryxState),VMSTATE_UINT64(clock_last_ns,AmdRadeonR9FuryxState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AmdRadeonR9FuryxState[] = {
    DEFINE_PROP_STRING("gpu-name", AmdRadeonR9FuryxState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AmdRadeonR9FuryxState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AmdRadeonR9FuryxState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x7300;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="AMD Radeon R9 Fury X (Fiji, PCIe x16, 4GB HBM, 2015)"; dc->vmsd=&vms_amd_radeon_r9_furyx; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRadeonR9FuryxState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_radeon_r9_furyx={.name=TYPE_AMD_RADEON_R9_FURYX,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRadeonR9FuryxState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_radeon_r9_furyx(void){type_register_static(&ti_amd_radeon_r9_furyx);}
type_init(reg_amd_radeon_r9_furyx)
