/*
 * NVIDIA Quadro RTX 6000 (TU102, Turing, 24GB GDDR6)
...
#define GPU_DEVICE_ID  0x1E30
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x12BA
#define GPU_REVISION   0xA2
...
#define GPU_PMC_BOOT_0  0x168000A2
#define GPU_PFB_BOOT_0  0x00000015
#define GPU_PFB_PARTS   0x0000000C
#define GPU_PFB_REFCTRL 0x80000073
#define VPD_CAP_OFFSET 0x70
#define GPU_CLK_BASE    1440
#define GPU_MEM_MHZ     1750
#define GPU_CLK_BOOST   1770
#define GPU_PCIE_GEN    3
#define TYPE_NVIDIA_RTX6000 "nvidia-rtx6000"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaRtx6000State, NVIDIA_RTX6000)
struct NvidiaRtx6000State {
...
static uint32_t gpu_clk(NvidiaRtx6000State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaRtx6000State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaRtx6000State *s=(NvidiaRtx6000State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-rtx6000-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-rtx6000-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-rtx6000-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-rtx6000-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_rtx6000={.name="nvidia-rtx6000",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaRtx6000State),VMSTATE_UINT32(intr_en,NvidiaRtx6000State),VMSTATE_UINT32(pfifo_intr_en,NvidiaRtx6000State),VMSTATE_UINT32(clock_mhz,NvidiaRtx6000State),VMSTATE_UINT64(clock_last_ns,NvidiaRtx6000State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaRtx6000State[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaRtx6000State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaRtx6000State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaRtx6000State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x1E30;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA Quadro RTX 6000 (TU102, Turing, 24GB GDDR6)"; dc->vmsd=&vms_nvidia_rtx6000; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaRtx6000State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_rtx6000={.name=TYPE_NVIDIA_RTX6000,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaRtx6000State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_rtx6000(void){type_register_static(&ti_nvidia_rtx6000);}
type_init(reg_nvidia_rtx6000)
