/*
 * NVIDIA GeForce GTX 690 (GK104, Kepler, Dual-GPU)
...
#define GPU_DEVICE_ID    0x1188
#define GPU_SUBSYS_VID   0x10DE
#define GPU_SUBSYS_DID   0x0966
...
#define GPU_CLK_BOOST    1019
#define GPU_PCIE_GEN     2

#define TYPE_NVIDIA_GTX690 "nvidia-gtx690"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx690State, NVIDIA_GTX690)

struct NvidiaGtx690State {
...
static uint32_t gpu_get_clock(NvidiaGtx690State *s)
{
...
{
    NvidiaGtx690State *s = (NvidiaGtx690State *)pdev; s->gpu_count=1;
    ...
    gpu_setup_caps(pdev);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops,  s, "nvidia-gtx690-mmio", NV_BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar1, OBJECT(s), &bar1_ops,  s, "nvidia-gtx690-vram", NV_BAR1_SIZE);
    pci_register_bar(pdev, 1, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar1);
    memory_region_init_io(&s->bar3, OBJECT(s), &bar35_ops, s, "nvidia-gtx690-ramin",NV_BAR3_SIZE);
    pci_register_bar(pdev, 3, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar3);
    memory_region_init_io(&s->bar5, OBJECT(s), &bar35_ops, s, "nvidia-gtx690-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vmstate_nvidia_gtx690 = {
    .name = "nvidia-gtx690", .version_id = 1, .minimum_version_id = 1,
    .fields = (const VMStateField[]) {
        VMSTATE_PCI_DEVICE(parent_obj, NvidiaGtx690State),
        VMSTATE_UINT32(intr_en,       NvidiaGtx690State),
        VMSTATE_UINT32(pfifo_intr_en, NvidiaGtx690State),
        VMSTATE_UINT32(clock_mhz,     NvidiaGtx690State),
        VMSTATE_UINT32(gpu_count,       NvidiaGtx690State             ),
        VMSTATE_UINT64(clock_last_ns, NvidiaGtx690State),
        VMSTATE_END_OF_LIST()
    },
};

static const Property gpu_multi_props_NvidiaGtx690State[] = {
    DEFINE_PROP_UINT32("gpu-count", NvidiaGtx690State, gpu_count, 1),
};
static void gpu_class_init(ObjectClass *klass, const void *data)
{
    ...
    dc->desc         = "NVIDIA GeForce GTX 690 (GK104, Kepler, Dual-GPU)";
    dc->vmsd         = &vmstate_nvidia_gtx690;
    dc->hotpluggable = false;
    device_class_set_props(dc, gpu_multi_props_NvidiaGtx690State);
    set_bit(DEVICE_CATEGORY_DISPLAY, dc->categories);
}

static const TypeInfo nvidia_gtx690_info = {
    .name          = TYPE_NVIDIA_GTX690,
    .parent        = TYPE_PCI_DEVICE,
    .instance_size = sizeof(NvidiaGtx690State),
    .class_init    = gpu_class_init,
    .interfaces    = (InterfaceInfo[]) {
        { INTERFACE_CONVENTIONAL_PCI_DEVICE }, { },
    },
};
static void nvidia_gtx690_register(void) { type_register_static(&nvidia_gtx690_info); }
type_init(nvidia_gtx690_register)
