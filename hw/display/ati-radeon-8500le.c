/*
 * ATI Radeon 8500 LE (R200, AGP 4x, 64MB DDR, 2002)
...
#define GPU_DEVICE_ID  0x514C
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x514C
#define GPU_REVISION   0x01
...
#define GPU_VRAM_MB     64
#define TYPE_ATI_RADEON_8500LE "ati-radeon-8500le"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeon8500leState, ATI_RADEON_8500LE)
struct AtiRadeon8500leState {
...
static uint32_t gpu_clk(AtiRadeon8500leState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeon8500leState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeon8500leState *s=(AtiRadeon8500leState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-8500le-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-8500le-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-8500le-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-8500le-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_8500le={.name="ati-radeon-8500le",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeon8500leState),VMSTATE_UINT32(intr_en,AtiRadeon8500leState),VMSTATE_UINT32(clock_mhz,AtiRadeon8500leState),VMSTATE_UINT64(clock_last_ns,AtiRadeon8500leState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeon8500leState[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeon8500leState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeon8500leState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeon8500leState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x514C;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="ATI Radeon 8500 LE (R200, AGP 4x, 64MB DDR, 2002)"; dc->vmsd=&vms_ati_radeon_8500le; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRadeon8500leState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_radeon_8500le={.name=TYPE_ATI_RADEON_8500LE,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRadeon8500leState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_radeon_8500le(void){type_register_static(&ti_ati_radeon_8500le);}
type_init(reg_ati_radeon_8500le)
