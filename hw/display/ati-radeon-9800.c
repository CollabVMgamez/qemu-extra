/*
 * ATI Radeon 9800 (R350, AGP 8x, 128MB DDR, 2003)
...
#define TYPE_ATI_RADEON_9800 "ati-radeon-9800"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeon9800State, ATI_RADEON_9800)
struct AtiRadeon9800State {
...
static uint32_t gpu_clk(AtiRadeon9800State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeon9800State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeon9800State *s=(AtiRadeon9800State*)p;
    ...
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-9800-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-9800-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-9800-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_9800={.name="ati-radeon-9800",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeon9800State),VMSTATE_UINT32(intr_en,AtiRadeon9800State),VMSTATE_UINT32(clock_mhz,AtiRadeon9800State),VMSTATE_UINT64(clock_last_ns,AtiRadeon9800State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeon9800State[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeon9800State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeon9800State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeon9800State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="ATI Radeon 9800 (R350, AGP 8x, 128MB DDR, 2003)"; dc->vmsd=&vms_ati_radeon_9800; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRadeon9800State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_radeon_9800={.name=TYPE_ATI_RADEON_9800,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRadeon9800State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_radeon_9800(void){type_register_static(&ti_ati_radeon_9800);}
type_init(reg_ati_radeon_9800)
