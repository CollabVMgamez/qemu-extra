/*
 * AMD Radeon RX 560 (Polaris11, GCN 4.0)
...
#define GPU_DEVICE_ID  0x67FF
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x0B32
#define GPU_REVISION   0xCF
...
#define GPU_PMC_BOOT_0  0x128000CF
...
#define GPU_CLK_BOOST   1275
#define GPU_PCIE_GEN    3
#define TYPE_AMD_RX560 "amd-rx560"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRx560State, AMD_RX560)
struct AmdRx560State {
...
static uint32_t gpu_clk(AmdRx560State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRx560State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRx560State *s=(AmdRx560State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-rx560-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-rx560-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-rx560-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-rx560-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_rx560={.name="amd-rx560",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRx560State),VMSTATE_UINT32(intr_en,AmdRx560State),VMSTATE_UINT32(pfifo_intr_en,AmdRx560State),VMSTATE_UINT32(clock_mhz,AmdRx560State),VMSTATE_UINT64(clock_last_ns,AmdRx560State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AmdRx560State[] = {
    DEFINE_PROP_STRING("gpu-name", AmdRx560State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AmdRx560State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AmdRx560State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x67FF;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="AMD Radeon RX 560 (Polaris11, GCN 4.0)"; dc->vmsd=&vms_amd_rx560; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRx560State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_rx560={.name=TYPE_AMD_RX560,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRx560State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_rx560(void){type_register_static(&ti_amd_rx560);}
type_init(reg_amd_rx560)
