/*
 * NVIDIA GeForce GTX 980 Ti (GM200, Maxwell)
...
#define GPU_DEVICE_ID  0x17C8
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x1094
...
#define GPU_CLK_BOOST   1075
#define GPU_PCIE_GEN    2
#define TYPE_NVIDIA_GTX980TI "nvidia-gtx980ti"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx980tiState, NVIDIA_GTX980TI)
struct NvidiaGtx980tiState {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaGtx980tiState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaGtx980tiState *s=(NvidiaGtx980tiState*)p; s->gpu_count=1;
    ...
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-gtx980ti-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-gtx980ti-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-gtx980ti-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_gtx980ti={.name="nvidia-gtx980ti",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaGtx980tiState),VMSTATE_UINT32(intr_en,NvidiaGtx980tiState),VMSTATE_UINT32(pfifo_intr_en,NvidiaGtx980tiState),VMSTATE_UINT32(clock_mhz,NvidiaGtx980tiState),VMSTATE_UINT64(clock_last_ns,NvidiaGtx980tiState),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x17C8;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA GeForce GTX 980 Ti (GM200, Maxwell)"; dc->vmsd=&vms_nvidia_gtx980ti; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaGtx980tiState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_gtx980ti={.name=TYPE_NVIDIA_GTX980TI,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaGtx980tiState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_gtx980ti(void){type_register_static(&ti_nvidia_gtx980ti);}
type_init(reg_nvidia_gtx980ti)
