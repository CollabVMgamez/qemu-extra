/*
 * NVIDIA GeForce GTX 660M (GK107, Kepler, Mobile)
...
#define GPU_DEVICE_ID    0x0FD4
#define GPU_SUBSYS_VID   0x10DE
#define GPU_SUBSYS_DID   0x0968
...
#define TYPE_NVIDIA_GTX660M "nvidia-gtx660m"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx660mState, NVIDIA_GTX660M)

struct NvidiaGtx660mState {
...
static uint32_t gpu_get_clock(NvidiaGtx660mState *s)
{
...
{
    NvidiaGtx660mState *s = (NvidiaGtx660mState *)pdev; s->gpu_count=1;
    ...
    gpu_setup_caps(pdev);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops,  s, "nvidia-gtx660m-mmio", NV_BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar1, OBJECT(s), &bar1_ops,  s, "nvidia-gtx660m-vram", NV_BAR1_SIZE);
    pci_register_bar(pdev, 1, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar1);
    memory_region_init_io(&s->bar3, OBJECT(s), &bar35_ops, s, "nvidia-gtx660m-ramin",NV_BAR3_SIZE);
    pci_register_bar(pdev, 3, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar3);
    memory_region_init_io(&s->bar5, OBJECT(s), &bar35_ops, s, "nvidia-gtx660m-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vmstate_nvidia_gtx660m = {
    .name = "nvidia-gtx660m", .version_id = 1, .minimum_version_id = 1,
    .fields = (const VMStateField[]) {
        VMSTATE_PCI_DEVICE(parent_obj, NvidiaGtx660mState),
        VMSTATE_UINT32(intr_en,       NvidiaGtx660mState),
        VMSTATE_UINT32(pfifo_intr_en, NvidiaGtx660mState),
        VMSTATE_UINT32(clock_mhz,     NvidiaGtx660mState),
        VMSTATE_UINT32(gpu_count,       NvidiaGtx660mState            ),
        VMSTATE_UINT64(clock_last_ns, NvidiaGtx660mState),
        VMSTATE_END_OF_LIST()
    },
};

static const Property gpu_multi_props_NvidiaGtx660mState[] = {
    DEFINE_PROP_UINT32("gpu-count", NvidiaGtx660mState, gpu_count, 1),
};
static void gpu_class_init(ObjectClass *klass, const void *data)
{
    ...
    dc->desc         = "NVIDIA GeForce GTX 660M (GK107, Kepler, Mobile)";
    dc->vmsd         = &vmstate_nvidia_gtx660m;
    dc->hotpluggable = false;
    device_class_set_props(dc, gpu_multi_props_NvidiaGtx660mState);
    set_bit(DEVICE_CATEGORY_DISPLAY, dc->categories);
}

static const TypeInfo nvidia_gtx660m_info = {
    .name          = TYPE_NVIDIA_GTX660M,
    .parent        = TYPE_PCI_DEVICE,
    .instance_size = sizeof(NvidiaGtx660mState),
    .class_init    = gpu_class_init,
    .interfaces    = (InterfaceInfo[]) {
        { INTERFACE_CONVENTIONAL_PCI_DEVICE }, { },
    },
};
static void nvidia_gtx660m_register(void) { type_register_static(&nvidia_gtx660m_info); }
type_init(nvidia_gtx660m_register)
