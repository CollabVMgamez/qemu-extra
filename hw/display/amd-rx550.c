/*
 * AMD Radeon RX 550 (Polaris12, GCN 4.0)
...
#define GPU_CLK_BASE    1100
#define GPU_MEM_MHZ     1500
#define GPU_CLK_BOOST   1183
#define GPU_PCIE_GEN    3
#define TYPE_AMD_RX550 "amd-rx550"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRx550State, AMD_RX550)
struct AmdRx550State {
...
static uint32_t gpu_clk(AmdRx550State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRx550State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRx550State *s=(AmdRx550State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-rx550-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-rx550-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-rx550-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-rx550-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_rx550={.name="amd-rx550",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRx550State),VMSTATE_UINT32(intr_en,AmdRx550State),VMSTATE_UINT32(pfifo_intr_en,AmdRx550State),VMSTATE_UINT32(clock_mhz,AmdRx550State),VMSTATE_UINT64(clock_last_ns,AmdRx550State),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x699F;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="AMD Radeon RX 550 (Polaris12, GCN 4.0)"; dc->vmsd=&vms_amd_rx550; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRx550State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_rx550={.name=TYPE_AMD_RX550,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRx550State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_rx550(void){type_register_static(&ti_amd_rx550);}
type_init(reg_amd_rx550)
