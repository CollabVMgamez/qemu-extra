/*
 * Intel UHD Graphics 770 (Rocket Lake GT1, 32EU)
 * Vendor: 0x8086, Device: 0x4C8C
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-770-rkl
...
#define GPU_DEV      0x4C8C
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x228C
#define GPU_REV      0x00
#define GPU_CLASS    0x0300
#define BAR0_SIZE (16*MiB)
#define BAR2_SIZE (256*MiB)
#define GPU_CLK_BASE  350
#define GPU_CLK_BOOST 1300
#define TYPE_INTELUHD770RKL "intel-uhd-770-rkl"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhd770RklState, INTELUHD770RKL)
struct IntelUhd770RklState {
...
static uint32_t igpu_clk(IntelUhd770RklState *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhd770RklState *s = INTELUHD770RKL(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-770-rkl-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-770-rkl-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhd770RklState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-770-rkl",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhd770RklState),VMSTATE_UINT32(clock_mhz,IntelUhd770RklState),VMSTATE_UINT64(clock_last_ns,IntelUhd770RklState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics 770 (Rocket Lake GT1, 32EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHD770RKL,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhd770RklState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
