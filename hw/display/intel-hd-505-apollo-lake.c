/*
 * Intel HD Graphics 505 (Apollo Lake GT2, 18EU)
 * Vendor: 0x8086, Device: 0x5A84
 * Class: 0x0300, PCIe
 * Usage: -device intel-hd-505-apollo-lake
...
#define GPU_DEV      0x5A84
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2284
#define GPU_REV      0x0E
#define GPU_CLASS    0x0300
#define BAR0_SIZE (8*MiB)
#define BAR2_SIZE (128*MiB)
#define GPU_CLK_BASE  375
#define GPU_CLK_BOOST 750
#define TYPE_INTELHD505APOLLOLAKE "intel-hd-505-apollo-lake"
OBJECT_DECLARE_SIMPLE_TYPE(IntelHd505ApolloLakeState, INTELHD505APOLLOLAKE)
struct IntelHd505ApolloLakeState {
...
static uint32_t igpu_clk(IntelHd505ApolloLakeState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelHd505ApolloLakeState *s = opaque;
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelHd505ApolloLakeState *s = INTELHD505APOLLOLAKE(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-hd-505-apollo-lake-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-hd-505-apollo-lake-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelHd505ApolloLakeState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-hd-505-apollo-lake",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelHd505ApolloLakeState),VMSTATE_UINT32(clock_mhz,IntelHd505ApolloLakeState),VMSTATE_UINT64(clock_last_ns,IntelHd505ApolloLakeState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel HD Graphics 505 (Apollo Lake GT2, 18EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELHD505APOLLOLAKE,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelHd505ApolloLakeState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
