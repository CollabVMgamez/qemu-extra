/*
 * ATI Radeon HD 4890 (RV790, PCIe x16, 512MB GDDR5, 2009)
...
#define GPU_DEVICE_ID  0x9460
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x9460
...
#define GPU_MEM_MHZ     975
#define GPU_CLK_BOOST   850
#define GPU_PCIE_GEN    2
#define GPU_VRAM_MB     512
#define TYPE_ATI_RADEON_HD4890 "ati-radeon-hd4890"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeonHd4890State, ATI_RADEON_HD4890)
struct AtiRadeonHd4890State {
...
static uint32_t gpu_clk(AtiRadeonHd4890State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeonHd4890State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeonHd4890State *s=(AtiRadeonHd4890State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-hd4890-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-hd4890-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-hd4890-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-hd4890-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_hd4890={.name="ati-radeon-hd4890",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeonHd4890State),VMSTATE_UINT32(intr_en,AtiRadeonHd4890State),VMSTATE_UINT32(clock_mhz,AtiRadeonHd4890State),VMSTATE_UINT64(clock_last_ns,AtiRadeonHd4890State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeonHd4890State[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeonHd4890State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeonHd4890State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeonHd4890State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x9460;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="ATI Radeon HD 4890 (RV790, PCIe x16, 512MB GDDR5, 2009)"; dc->vmsd=&vms_ati_radeon_hd4890; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRadeonHd4890State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_radeon_hd4890={.name=TYPE_ATI_RADEON_HD4890,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRadeonHd4890State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_radeon_hd4890(void){type_register_static(&ti_ati_radeon_hd4890);}
type_init(reg_ati_radeon_hd4890)
