/*
 * NVIDIA GeForce GTX 550 Ti (GF116, Fermi)
...
#define GPU_DEVICE_ID    0x1244
#define GPU_SUBSYS_VID   0x10DE
#define GPU_SUBSYS_DID   0x0850
...
#define GPU_MEM_MHZ     1026
#define GPU_CLK_BOOST    900
#define GPU_PCIE_GEN     2

#define TYPE_NVIDIA_GTX550TI "nvidia-gtx550ti"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx550tiState, NVIDIA_GTX550TI)
...
static uint32_t gpu_get_clock(NvidiaGtx550tiState *s)
{
...
{
    NvidiaGtx550tiState *s = (NvidiaGtx550tiState *)pdev; s->gpu_count=1;
    ...
    gpu_setup_caps(pdev);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops,  s, "nvidia-gtx550ti-mmio", NV_BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar1, OBJECT(s), &bar1_ops,  s, "nvidia-gtx550ti-vram", NV_BAR1_SIZE);
    pci_register_bar(pdev, 1, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar1);
    memory_region_init_io(&s->bar3, OBJECT(s), &bar35_ops, s, "nvidia-gtx550ti-ramin",NV_BAR3_SIZE);
    pci_register_bar(pdev, 3, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar3);
    memory_region_init_io(&s->bar5, OBJECT(s), &bar35_ops, s, "nvidia-gtx550ti-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vmstate_nvidia_gtx550ti = {
    .name = "nvidia-gtx550ti", .version_id = 1, .minimum_version_id = 1,
    .fields = (const VMStateField[]) {
        VMSTATE_PCI_DEVICE(parent_obj, NvidiaGtx550tiState),
...
static const TypeInfo nvidia_gtx550ti_info = {
    .name          = TYPE_NVIDIA_GTX550TI,
    .parent        = TYPE_PCI_DEVICE,
    .instance_size = sizeof(NvidiaGtx550tiState),
    .class_init    = gpu_class_init,
    .interfaces    = (InterfaceInfo[]) {
        { INTERFACE_CONVENTIONAL_PCI_DEVICE }, { },
    },
};
static void nvidia_gtx550ti_register(void) { type_register_static(&nvidia_gtx550ti_info); }
type_init(nvidia_gtx550ti_register)
