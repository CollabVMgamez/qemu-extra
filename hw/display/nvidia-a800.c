/*
 * NVIDIA A800 (GA100, Ampere, 80GB HBM2e, China export)
...
#define TYPE_NVIDIA_A800 "nvidia-a800"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaA800State, NVIDIA_A800)
struct NvidiaA800State {
...
static uint32_t gpu_clk(NvidiaA800State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaA800State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaA800State *s=(NvidiaA800State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-a800-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-a800-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-a800-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-a800-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_a800={.name="nvidia-a800",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaA800State),VMSTATE_UINT32(intr_en,NvidiaA800State),VMSTATE_UINT32(pfifo_intr_en,NvidiaA800State),VMSTATE_UINT32(clock_mhz,NvidiaA800State),VMSTATE_UINT64(clock_last_ns,NvidiaA800State),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="NVIDIA A800 (GA100, Ampere, 80GB HBM2e, China export)"; dc->vmsd=&vms_nvidia_a800; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaA800State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_a800={.name=TYPE_NVIDIA_A800,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaA800State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_a800(void){type_register_static(&ti_nvidia_a800);}
type_init(reg_nvidia_a800)
