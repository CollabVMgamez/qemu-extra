/*
 * NVIDIA Tesla K80 (GK210, Kepler, 24GB GDDR5, Dual-GPU)
...
#define GPU_DEVICE_ID  0x102D
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x106C
...
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaK80State, NVIDIA_K80)
struct NvidiaK80State {
...
static uint32_t gpu_clk(NvidiaK80State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaK80State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaK80State *s=(NvidiaK80State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-k80-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-k80-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-k80-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-k80-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_k80={.name="nvidia-k80",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaK80State),VMSTATE_UINT32(intr_en,NvidiaK80State),VMSTATE_UINT32(pfifo_intr_en,NvidiaK80State),VMSTATE_UINT32(clock_mhz,NvidiaK80State),VMSTATE_UINT64(clock_last_ns,NvidiaK80State),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x102D;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA Tesla K80 (GK210, Kepler, 24GB GDDR5, Dual-GPU)"; dc->vmsd=&vms_nvidia_k80; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaK80State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_k80={.name=TYPE_NVIDIA_K80,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaK80State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_k80(void){type_register_static(&ti_nvidia_k80);}
type_init(reg_nvidia_k80)
