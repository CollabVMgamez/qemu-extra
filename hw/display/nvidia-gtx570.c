/*
 * NVIDIA GeForce GTX 570 (GF110, Fermi)
...
#define GPU_DEVICE_ID    0x1081
#define GPU_SUBSYS_VID   0x10DE
#define GPU_SUBSYS_DID   0x0853
...
#define GPU_PFB_PARTS    0x00000005
#define GPU_PFB_REFCTRL  0x00000053
#define GPU_CLK_BASE     732
#define GPU_MEM_MHZ     950
#define GPU_CLK_BOOST    732
#define GPU_PCIE_GEN     2

#define TYPE_NVIDIA_GTX570 "nvidia-gtx570"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx570State, NVIDIA_GTX570)

struct NvidiaGtx570State {
...
static uint32_t gpu_get_clock(NvidiaGtx570State *s)
{
...
{
    NvidiaGtx570State *s = (NvidiaGtx570State *)pdev; s->gpu_count=1;
    ...
    gpu_setup_caps(pdev);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops,  s, "nvidia-gtx570-mmio", NV_BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar1, OBJECT(s), &bar1_ops,  s, "nvidia-gtx570-vram", NV_BAR1_SIZE);
    pci_register_bar(pdev, 1, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar1);
    memory_region_init_io(&s->bar3, OBJECT(s), &bar35_ops, s, "nvidia-gtx570-ramin",NV_BAR3_SIZE);
    pci_register_bar(pdev, 3, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar3);
    memory_region_init_io(&s->bar5, OBJECT(s), &bar35_ops, s, "nvidia-gtx570-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vmstate_nvidia_gtx570 = {
    .name = "nvidia-gtx570", .version_id = 1, .minimum_version_id = 1,
    .fields = (const VMStateField[]) {
        VMSTATE_PCI_DEVICE(parent_obj, NvidiaGtx570State),
        VMSTATE_UINT32(intr_en,       NvidiaGtx570State),
        VMSTATE_UINT32(pfifo_intr_en, NvidiaGtx570State),
        VMSTATE_UINT32(clock_mhz,     NvidiaGtx570State),
        VMSTATE_UINT32(gpu_count,       NvidiaGtx570State             ),
        VMSTATE_UINT64(clock_last_ns, NvidiaGtx570State),
        VMSTATE_END_OF_LIST()
    },
};

static const Property gpu_multi_props_NvidiaGtx570State[] = {
    DEFINE_PROP_UINT32("gpu-count", NvidiaGtx570State, gpu_count, 1),
};
static void gpu_class_init(ObjectClass *klass, const void *data)
{
    ...
    dc->desc         = "NVIDIA GeForce GTX 570 (GF110, Fermi)";
    dc->vmsd         = &vmstate_nvidia_gtx570;
    dc->hotpluggable = false;
    device_class_set_props(dc, gpu_multi_props_NvidiaGtx570State);
    set_bit(DEVICE_CATEGORY_DISPLAY, dc->categories);
}

static const TypeInfo nvidia_gtx570_info = {
    .name          = TYPE_NVIDIA_GTX570,
    .parent        = TYPE_PCI_DEVICE,
    .instance_size = sizeof(NvidiaGtx570State),
    .class_init    = gpu_class_init,
    .interfaces    = (InterfaceInfo[]) {
        { INTERFACE_CONVENTIONAL_PCI_DEVICE }, { },
    },
};
static void nvidia_gtx570_register(void) { type_register_static(&nvidia_gtx570_info); }
type_init(nvidia_gtx570_register)
