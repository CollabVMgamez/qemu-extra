/*
 * NVIDIA RIVA TNT (NV4, AGP 2x, 16MB, 1998)
...
#define GPU_DEVICE_ID  0x0020
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x0020
...
#define GPU_PMC_BOOT_0  0x00000004
...
#define TYPE_NVIDIA_NV4_TNT "nvidia-nv4-tnt"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaNv4TntState, NVIDIA_NV4_TNT)
struct NvidiaNv4TntState {
...
static uint32_t gpu_clk(NvidiaNv4TntState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaNv4TntState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaNv4TntState *s=(NvidiaNv4TntState*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-nv4-tnt-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b135ops,s,"nvidia-nv4-tnt-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b135ops,s,"nvidia-nv4-tnt-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b135ops,s,"nvidia-nv4-tnt-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_nvidia_nv4_tnt[]={DEFINE_PROP_UINT32("gpu-count",NvidiaNv4TntState,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",NvidiaNv4TntState,gpu_name),
    DEFINE_PROP_STRING("board-partner",NvidiaNv4TntState,board_partner)};
static const VMStateDescription vms_nvidia_nv4_tnt={.name="nvidia-nv4-tnt",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaNv4TntState),VMSTATE_UINT32(intr_en,NvidiaNv4TntState),VMSTATE_UINT32(clock_mhz,NvidiaNv4TntState),VMSTATE_UINT64(clock_last_ns,NvidiaNv4TntState),
        VMSTATE_UINT32(gpu_count,NvidiaNv4TntState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x0020;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA RIVA TNT (NV4, AGP 2x, 16MB, 1998)"; dc->vmsd=&vms_nvidia_nv4_tnt; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_nvidia_nv4_tnt);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_nv4_tnt={.name=TYPE_NVIDIA_NV4_TNT,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaNv4TntState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_nv4_tnt(void){type_register_static(&ti_nvidia_nv4_tnt);}
type_init(reg_nvidia_nv4_tnt)
