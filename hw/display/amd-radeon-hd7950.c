/*
 * AMD Radeon HD 7950 (Tahiti, PCIe x16, 3GB GDDR5, 2012)
...
#define TYPE_AMD_RADEON_HD7950 "amd-radeon-hd7950"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRadeonHd7950State, AMD_RADEON_HD7950)
struct AmdRadeonHd7950State {
...
static uint32_t gpu_clk(AmdRadeonHd7950State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRadeonHd7950State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRadeonHd7950State *s=(AmdRadeonHd7950State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-radeon-hd7950-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-radeon-hd7950-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-radeon-hd7950-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-radeon-hd7950-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_radeon_hd7950={.name="amd-radeon-hd7950",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRadeonHd7950State),VMSTATE_UINT32(intr_en,AmdRadeonHd7950State),VMSTATE_UINT32(clock_mhz,AmdRadeonHd7950State),VMSTATE_UINT64(clock_last_ns,AmdRadeonHd7950State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AmdRadeonHd7950State[] = {
    DEFINE_PROP_STRING("gpu-name", AmdRadeonHd7950State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AmdRadeonHd7950State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AmdRadeonHd7950State, board_partner),
};
...
static void reg_amd_radeon_hd7950(void){type_register_static(&ti_amd_radeon_hd7950);}
type_init(reg_amd_radeon_hd7950)
