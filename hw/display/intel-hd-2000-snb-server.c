/*
 * Intel HD Graphics 2000 (Sandy Bridge Server GT1, 6EU)
 * Vendor: 0x8086, Device: 0x010A
 * Class: 0x0300, PCIe
 * Usage: -device intel-hd-2000-snb-server
...
#define GPU_DEV      0x010A
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2001
#define GPU_REV      0x09
#define GPU_CLASS    0x0300
#define BAR0_SIZE (8*MiB)
#define BAR2_SIZE (128*MiB)
#define GPU_CLK_BASE  850
#define GPU_CLK_BOOST 1350
#define TYPE_INTELHD2000SNBSERVER "intel-hd-2000-snb-server"
OBJECT_DECLARE_SIMPLE_TYPE(IntelHd2000SnbServerState, INTELHD2000SNBSERVER)
struct IntelHd2000SnbServerState {
...
static uint32_t igpu_clk(IntelHd2000SnbServerState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelHd2000SnbServerState *s = opaque;
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelHd2000SnbServerState *s = INTELHD2000SNBSERVER(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-hd-2000-snb-server-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-hd-2000-snb-server-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelHd2000SnbServerState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-hd-2000-snb-server",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelHd2000SnbServerState),VMSTATE_UINT32(clock_mhz,IntelHd2000SnbServerState),VMSTATE_UINT64(clock_last_ns,IntelHd2000SnbServerState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel HD Graphics 2000 (Sandy Bridge Server GT1, 6EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELHD2000SNBSERVER,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelHd2000SnbServerState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
