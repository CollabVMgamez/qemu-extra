/*
 * Intel HD Graphics 4400 (Haswell ULT GT2, 20EU)
 * Vendor: 0x8086, Device: 0x0A16
 * Class: 0x0300, PCIe
 * Usage: -device intel-hd-4400
...
#define GPU_DEV      0x0A16
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2216
#define GPU_REV      0x06
...
#define TYPE_INTELHD4400 "intel-hd-4400"
OBJECT_DECLARE_SIMPLE_TYPE(IntelHd4400State, INTELHD4400)
struct IntelHd4400State {
...
static uint32_t igpu_clk(IntelHd4400State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelHd4400State *s = INTELHD4400(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-hd-4400-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-hd-4400-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelHd4400State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-hd-4400",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelHd4400State),VMSTATE_UINT32(clock_mhz,IntelHd4400State),VMSTATE_UINT64(clock_last_ns,IntelHd4400State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel HD Graphics 4400 (Haswell ULT GT2, 20EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELHD4400,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelHd4400State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
