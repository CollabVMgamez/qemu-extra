/*
 * NVIDIA L40 (AD102, Ada Lovelace, 48GB GDDR6X)
...
#define GPU_DEVICE_ID  0x26B5
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x17AA
...
#define GPU_CLK_BOOST   2490
#define GPU_PCIE_GEN    4
#define TYPE_NVIDIA_L40 "nvidia-l40"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaL40State, NVIDIA_L40)
struct NvidiaL40State {
...
static uint32_t gpu_clk(NvidiaL40State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaL40State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaL40State *s=(NvidiaL40State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-l40-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-l40-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-l40-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-l40-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_l40={.name="nvidia-l40",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaL40State),VMSTATE_UINT32(intr_en,NvidiaL40State),VMSTATE_UINT32(pfifo_intr_en,NvidiaL40State),VMSTATE_UINT32(clock_mhz,NvidiaL40State),VMSTATE_UINT64(clock_last_ns,NvidiaL40State),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x26B5;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA L40 (AD102, Ada Lovelace, 48GB GDDR6X)"; dc->vmsd=&vms_nvidia_l40; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaL40State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_l40={.name=TYPE_NVIDIA_L40,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaL40State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_l40(void){type_register_static(&ti_nvidia_l40);}
type_init(reg_nvidia_l40)
