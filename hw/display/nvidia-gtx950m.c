/*
 * NVIDIA GeForce GTX 950M (GM107, Maxwell, Mobile)
...
#define GPU_DEVICE_ID  0x139A
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x1095
...
#define GPU_CLK_BASE    914
#define GPU_MEM_MHZ     900
#define GPU_CLK_BOOST   1038
#define GPU_PCIE_GEN    2
#define TYPE_NVIDIA_GTX950M "nvidia-gtx950m"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx950mState, NVIDIA_GTX950M)
struct NvidiaGtx950mState {
...
static uint32_t gpu_clk(NvidiaGtx950mState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaGtx950mState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaGtx950mState *s=(NvidiaGtx950mState*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-gtx950m-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-gtx950m-vram",NV_BAR1_SIZE);
...
static const Property gpu_multi_props_NvidiaGtx950mState[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaGtx950mState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaGtx950mState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaGtx950mState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x139A;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA GeForce GTX 950M (GM107, Maxwell, Mobile)"; dc->vmsd=&vms_nvidia_gtx950m; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaGtx950mState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_gtx950m={.name=TYPE_NVIDIA_GTX950M,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaGtx950mState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_gtx950m(void){type_register_static(&ti_nvidia_gtx950m);}
type_init(reg_nvidia_gtx950m)
