/*
 * NVIDIA B100 SXM (GB100, Blackwell, 192GB HBM3e)
...
#define GPU_DEVICE_ID  0x2900
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x17F3
...
#define GPU_PMC_BOOT_0  0x198000A1
#define GPU_PFB_BOOT_0  0x00000015
#define GPU_PFB_PARTS   0x000000C0
#define GPU_PFB_REFCTRL 0x800000B3
#define VPD_CAP_OFFSET 0x70
#define GPU_CLK_BASE    700
#define GPU_MEM_MHZ     1750
#define GPU_CLK_BOOST   2100
#define GPU_PCIE_GEN    5
#define TYPE_NVIDIA_B100 "nvidia-b100"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaB100State, NVIDIA_B100)
struct NvidiaB100State {
...
static uint32_t gpu_clk(NvidiaB100State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaB100State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaB100State *s=(NvidiaB100State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-b100-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-b100-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-b100-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-b100-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_b100={.name="nvidia-b100",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaB100State),VMSTATE_UINT32(intr_en,NvidiaB100State),VMSTATE_UINT32(pfifo_intr_en,NvidiaB100State),VMSTATE_UINT32(clock_mhz,NvidiaB100State),VMSTATE_UINT64(clock_last_ns,NvidiaB100State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaB100State[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaB100State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaB100State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaB100State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x2900;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA B100 SXM (GB100, Blackwell, 192GB HBM3e)"; dc->vmsd=&vms_nvidia_b100; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaB100State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_b100={.name=TYPE_NVIDIA_B100,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaB100State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_b100(void){type_register_static(&ti_nvidia_b100);}
type_init(reg_nvidia_b100)
