/*
 * Intel Iris Plus Graphics G4 (Ice Lake GT1, 48EU)
 * Vendor: 0x8086, Device: 0x8A51
 * Class: 0x0300, PCIe
 * Usage: -device intel-iris-plus-g4
...
#define GPU_DEV      0x8A51
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2251
#define GPU_REV      0x00
...
#define TYPE_INTELIRISPLUSG4 "intel-iris-plus-g4"
OBJECT_DECLARE_SIMPLE_TYPE(IntelIrisPlusG4State, INTELIRISPLUSG4)
struct IntelIrisPlusG4State {
...
static uint32_t igpu_clk(IntelIrisPlusG4State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelIrisPlusG4State *s = INTELIRISPLUSG4(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-iris-plus-g4-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-iris-plus-g4-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelIrisPlusG4State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-iris-plus-g4",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelIrisPlusG4State),VMSTATE_UINT32(clock_mhz,IntelIrisPlusG4State),VMSTATE_UINT64(clock_last_ns,IntelIrisPlusG4State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Iris Plus Graphics G4 (Ice Lake GT1, 48EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELIRISPLUSG4,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelIrisPlusG4State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
