/*
 * NVIDIA GeForce GTX 1070 Ti (GP104, Pascal)
...
#define GPU_DEVICE_ID  0x1B82
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x1104
...
#define GPU_CLK_BASE    1607
#define GPU_MEM_MHZ     2002
#define GPU_CLK_BOOST   1683
#define GPU_PCIE_GEN    3
#define TYPE_NVIDIA_GTX1070TI "nvidia-gtx1070ti"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx1070tiState, NVIDIA_GTX1070TI)
struct NvidiaGtx1070tiState {
...
static uint32_t gpu_clk(NvidiaGtx1070tiState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaGtx1070tiState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaGtx1070tiState *s=(NvidiaGtx1070tiState*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-gtx1070ti-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-gtx1070ti-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-gtx1070ti-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-gtx1070ti-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_gtx1070ti={.name="nvidia-gtx1070ti",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaGtx1070tiState),VMSTATE_UINT32(intr_en,NvidiaGtx1070tiState),VMSTATE_UINT32(pfifo_intr_en,NvidiaGtx1070tiState),VMSTATE_UINT32(clock_mhz,NvidiaGtx1070tiState),VMSTATE_UINT64(clock_last_ns,NvidiaGtx1070tiState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaGtx1070tiState[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaGtx1070tiState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaGtx1070tiState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaGtx1070tiState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x1B82;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA GeForce GTX 1070 Ti (GP104, Pascal)"; dc->vmsd=&vms_nvidia_gtx1070ti; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaGtx1070tiState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_gtx1070ti={.name=TYPE_NVIDIA_GTX1070TI,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaGtx1070tiState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_gtx1070ti(void){type_register_static(&ti_nvidia_gtx1070ti);}
type_init(reg_nvidia_gtx1070ti)
