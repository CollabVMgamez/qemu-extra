/* Apple GPU M5 Ultra (estimated)
 * Vendor: 0x106B, Device: 0x4035
 * Usage: -device apple-gpu-m5-ultra
...
#define GPU_DEV 0x4035
#define GPU_SS_V 0x106B
#define GPU_SS_D 0x0011
#define GPU_CLASS 0x0300
#define BAR0_SIZE (4*MiB)
#define BAR1_SIZE (64*MiB)
#define TYPE_APPLE_GPU_M5_ULTRA "apple-gpu-m5-ultra"
OBJECT_DECLARE_SIMPLE_TYPE(AppleGpuM5UltraState, APPLE_GPU_M5_ULTRA)
struct AppleGpuM5UltraState { PCIDevice parent_obj; MemoryRegion bar0,bar1; uint32_t clock_mhz; uint64_t clock_last_ns; char *gpu_name; };
...
static void realize(PCIDevice*p,Error**e){{
    AppleGpuM5UltraState*s=(AppleGpuM5UltraState*)p; s->clock_mhz=0;s->clock_last_ns=0;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0,s,"apple-gpu-m5-ultra-mmio",BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1,s,"apple-gpu-m5-ultra-vram",BAR1_SIZE);
    pci_register_bar(p,2,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
}}
static const Property props[]={DEFINE_PROP_STRING("gpu-name",AppleGpuM5UltraState,gpu_name),};
static const VMStateDescription vms={.name="apple-gpu-m5-ultra",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AppleGpuM5UltraState),VMSTATE_UINT32(clock_mhz,AppleGpuM5UltraState),VMSTATE_UINT64(clock_last_ns,AppleGpuM5UltraState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass*k,const void*d){DeviceClass*dc=DEVICE_CLASS(k);PCIDeviceClass*pc=PCI_DEVICE_CLASS(k);pc->realize=realize;pc->vendor_id=GPU_VID;pc->device_id=GPU_DEV;pc->class_id=GPU_CLASS;pc->revision=0;dc->desc="Apple GPU M5 Ultra (estimated)";dc->vmsd=&vms;dc->hotpluggable=false;device_class_set_props(dc,props);set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);}
static const TypeInfo ti={.name=TYPE_APPLE_GPU_M5_ULTRA,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AppleGpuM5UltraState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
