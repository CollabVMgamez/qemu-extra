...
struct AmdMi200State { PCIDevice parent_obj; MemoryRegion bar0,bar1; uint32_t clock_mhz; uint64_t clock_last_ns; char *gpu_name;     char *board_partner;
};
...
static void realize(PCIDevice*p,Error**e){ AmdMi200State*s=(AmdMi200State*)p; s->clock_mhz=CLK_BASE;s->clock_last_ns=0; p->config[PCI_CLASS_PROG]=0; pci_set_word(p->config+PCI_SUBSYSTEM_VENDOR_ID,GPU_SS_V);
    if (s->board_partner) {
...
static const Property props[]={DEFINE_PROP_STRING("gpu-name",AmdMi200State,gpu_name),    DEFINE_PROP_STRING("board-partner", AmdMi200State, board_partner),
};
