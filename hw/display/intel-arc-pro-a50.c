/*
 * Intel Arc Pro A50 (ACM-G11, 75W, 6GB)
 * Vendor: 0x8086, Device: 0x56B8
 * Class: 0x0302, PCIe
 * Usage: -device intel-arc-pro-a50
...
#define GPU_DEV      0x56B8
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x22B8
#define GPU_REV      0x05
#define GPU_CLASS    0x0302
#define BAR0_SIZE (64*MiB)
#define BAR2_SIZE (8192*MiB)
#define GPU_CLK_BASE  300
#define GPU_CLK_BOOST 2000
#define TYPE_INTELARCPROA50 "intel-arc-pro-a50"
OBJECT_DECLARE_SIMPLE_TYPE(IntelArcProA50State, INTELARCPROA50)
struct IntelArcProA50State {
...
static uint32_t igpu_clk(IntelArcProA50State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelArcProA50State *s = INTELARCPROA50(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-arc-pro-a50-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-arc-pro-a50-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelArcProA50State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-arc-pro-a50",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelArcProA50State),VMSTATE_UINT32(clock_mhz,IntelArcProA50State),VMSTATE_UINT64(clock_last_ns,IntelArcProA50State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Arc Pro A50 (ACM-G11, 75W, 6GB)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELARCPROA50,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelArcProA50State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
