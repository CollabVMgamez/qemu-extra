/*
 * AMD Radeon RX 6600 XT (Navi23, RDNA 2.0)
...
#define GPU_PMC_BOOT_0  0x158000CF
...
#define GPU_CLK_BASE    1968
#define GPU_MEM_MHZ     2000
#define GPU_CLK_BOOST   2589
#define GPU_PCIE_GEN    4
#define TYPE_AMD_RX6600XT "amd-rx6600xt"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRx6600xtState, AMD_RX6600XT)
struct AmdRx6600xtState {
...
static uint32_t gpu_clk(AmdRx6600xtState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRx6600xtState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRx6600xtState *s=(AmdRx6600xtState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-rx6600xt-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-rx6600xt-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-rx6600xt-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-rx6600xt-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_rx6600xt={.name="amd-rx6600xt",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRx6600xtState),VMSTATE_UINT32(intr_en,AmdRx6600xtState),VMSTATE_UINT32(pfifo_intr_en,AmdRx6600xtState),VMSTATE_UINT32(clock_mhz,AmdRx6600xtState),VMSTATE_UINT64(clock_last_ns,AmdRx6600xtState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AmdRx6600xtState[] = {
    DEFINE_PROP_STRING("gpu-name", AmdRx6600xtState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AmdRx6600xtState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AmdRx6600xtState, board_partner),
};
...
static void reg_amd_rx6600xt(void){type_register_static(&ti_amd_rx6600xt);}
type_init(reg_amd_rx6600xt)
