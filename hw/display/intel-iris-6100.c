/*
 * Intel Iris Graphics 6100 (Broadwell GT3, 48EU)
 * Vendor: 0x8086, Device: 0x162B
 * Class: 0x0300, PCIe
 * Usage: -device intel-iris-6100
...
#define GPU_DEV      0x162B
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x222B
#define GPU_REV      0x08
...
#define TYPE_INTELIRIS6100 "intel-iris-6100"
OBJECT_DECLARE_SIMPLE_TYPE(IntelIris6100State, INTELIRIS6100)
struct IntelIris6100State {
...
static uint32_t igpu_clk(IntelIris6100State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelIris6100State *s = INTELIRIS6100(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-iris-6100-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-iris-6100-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelIris6100State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-iris-6100",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelIris6100State),VMSTATE_UINT32(clock_mhz,IntelIris6100State),VMSTATE_UINT64(clock_last_ns,IntelIris6100State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Iris Graphics 6100 (Broadwell GT3, 48EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELIRIS6100,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelIris6100State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
