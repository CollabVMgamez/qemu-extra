...
#define CLK_BASE 2452
#define CLK_BOOST 2452
#define PMC_BOOT_0 0x192000A1
#define PFB_PARTS 0x00000006
#define PFB_REFCTRL 0x800006C9
#define TYPE_NVIDIA_RTX5070TI "nvidia-rtx5070ti"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaRtx5070tiState, NVIDIA_RTX5070TI)
struct NvidiaRtx5070tiState { PCIDevice parent_obj; MemoryRegion bar0,bar1,bar3,bar5; uint32_t intr_en,pfifo_intr_en,clock_mhz; uint64_t clock_last_ns; uint32_t gpu_count; char *gpu_name,*board_partner; };
static uint32_t clk_f(NvidiaRtx5070tiState*s){ uint64_t now=qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL); if(now-s->clock_last_ns>100000000ULL){s->clock_last_ns=now;s->clock_mhz=s->clock_mhz*1664525u+1013904223u;s->clock_mhz=CLK_BASE+(s->clock_mhz%(CLK_BOOST-CLK_BASE+1));} return s->clock_mhz; }
static uint64_t b0r(void*op,hwaddr a,unsigned sz){ (void)sz; NvidiaRtx5070tiState*s=(NvidiaRtx5070tiState*)op; uint32_t m; uint64_t n;
switch(a&~3ULL){
...
static void b0w(void*o,hwaddr a,uint64_t v,unsigned s){ NvidiaRtx5070tiState*ss=(NvidiaRtx5070tiState*)o; if((a&~3ULL)==0x000140)ss->intr_en=(uint32_t)v; else if((a&~3ULL)==0x002100)ss->pfifo_intr_en=(uint32_t)v; }
...
static void realize(PCIDevice*p,Error**e){ NvidiaRtx5070tiState*s=(NvidiaRtx5070tiState*)p; s->gpu_count=1; s->clock_mhz=CLK_BASE;s->clock_last_ns=0; p->config[PCI_CLASS_PROG]=0;pci_set_word(p->config+PCI_SUBSYSTEM_VENDOR_ID,GPU_SS_V);
    if (s->board_partner) {
...
pci_set_word(p->config+PCI_SUBSYSTEM_ID,GPU_SS_D); uint8_t*c=p->config;c[PCI_CAPABILITY_LIST]=0x50;c[PCI_STATUS]|=PCI_STATUS_CAP_LIST;c[0x50]=0x05;c[0x51]=0x60;c[0x52]=0x01;c[0x53]=0;c[0x60]=0x10;c[0x61]=0;pci_set_word(c+0x62,0x0002);pci_set_long(c+0x64,0x00008000|5);pci_set_long(c+0x6C,(0x10<<4)|5);pci_set_word(c+0x72,(0x10<<4)|5|(1<<13)); memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-rtx5070ti-mmio",NV_BAR0_SIZE);pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0); memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-rtx5070ti-vram",NV_BAR1_SIZE);pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1); memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-rtx5070ti-ramin",NV_BAR3_SIZE);pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3); memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-rtx5070ti-vgaio",NV_BAR5_SIZE);pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);if(s->gpu_count>1){PCIBus*bus=pci_get_bus(p);const char*tn=object_get_typename(OBJECT(s));for(uint32_t i=1;i<s->gpu_count&&i<8;i++){PCIDevice*ex=pci_new(-1,tn);if(ex){qdev_prop_set_uint32(DEVICE(ex),"gpu-count",1);pci_realize_and_unref(ex,bus,&error_abort);pci_set_word(ex->config+PCI_CLASS_DEVICE,0x0302);}}} }
static const Property props[]={DEFINE_PROP_STRING("gpu-name",NvidiaRtx5070tiState,gpu_name),DEFINE_PROP_UINT32("gpu-count",NvidiaRtx5070tiState,gpu_count,1),DEFINE_PROP_STRING("board-partner",NvidiaRtx5070tiState,board_partner),};
static const VMStateDescription vms={.name="nvidia-rtx5070ti",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaRtx5070tiState),VMSTATE_UINT32(intr_en,NvidiaRtx5070tiState),VMSTATE_UINT32(pfifo_intr_en,NvidiaRtx5070tiState),VMSTATE_UINT32(clock_mhz,NvidiaRtx5070tiState),VMSTATE_UINT64(clock_last_ns,NvidiaRtx5070tiState),VMSTATE_END_OF_LIST()}};
...
static const TypeInfo ti={.name=TYPE_NVIDIA_RTX5070TI,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaRtx5070tiState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
