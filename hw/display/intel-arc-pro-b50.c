/*
 * Intel Arc Pro B50 (Battlemage BMG-G21, 75W, 12GB)
 * Vendor: 0x8086, Device: 0xE20A
 * Class: 0x0302, PCIe
 * Usage: -device intel-arc-pro-b50
...
#define GPU_DEV      0xE20A
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x220A
#define GPU_REV      0x01
#define GPU_CLASS    0x0302
#define BAR0_SIZE (64*MiB)
#define BAR2_SIZE (12288*MiB)
#define GPU_CLK_BASE  300
#define GPU_CLK_BOOST 2200
#define TYPE_INTELARCPROB50 "intel-arc-pro-b50"
OBJECT_DECLARE_SIMPLE_TYPE(IntelArcProB50State, INTELARCPROB50)
struct IntelArcProB50State {
...
static uint32_t igpu_clk(IntelArcProB50State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelArcProB50State *s = INTELARCPROB50(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-arc-pro-b50-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-arc-pro-b50-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelArcProB50State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-arc-pro-b50",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelArcProB50State),VMSTATE_UINT32(clock_mhz,IntelArcProB50State),VMSTATE_UINT64(clock_last_ns,IntelArcProB50State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Arc Pro B50 (Battlemage BMG-G21, 75W, 12GB)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELARCPROB50,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelArcProB50State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
