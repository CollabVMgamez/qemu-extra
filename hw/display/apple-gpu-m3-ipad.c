/* Apple GPU M3 iPad (9-core)
 * Vendor: 0x106B, Device: 0x4030
 * Usage: -device apple-gpu-m3-ipad
...
#define GPU_DEV 0x4030
#define GPU_SS_V 0x106B
#define GPU_SS_D 0x0025
#define GPU_CLASS 0x0300
#define BAR0_SIZE (4*MiB)
#define BAR1_SIZE (64*MiB)
#define TYPE_APPLE_GPU_M3_IPAD "apple-gpu-m3-ipad"
OBJECT_DECLARE_SIMPLE_TYPE(AppleGpuM3IpadState, APPLE_GPU_M3_IPAD)
struct AppleGpuM3IpadState { PCIDevice parent_obj; MemoryRegion bar0,bar1; uint32_t clock_mhz; uint64_t clock_last_ns; char *gpu_name; };
...
static void realize(PCIDevice*p,Error**e){
    AppleGpuM3IpadState*s=(AppleGpuM3IpadState*)p; s->clock_mhz=0;s->clock_last_ns=0;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0,s,"apple-gpu-m3-ipad-mmio",BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1,s,"apple-gpu-m3-ipad-vram",BAR1_SIZE);
    pci_register_bar(p,2,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
}
static const Property props[]={DEFINE_PROP_STRING("gpu-name",AppleGpuM3IpadState,gpu_name),};
static const VMStateDescription vms={.name="apple-gpu-m3-ipad",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AppleGpuM3IpadState),VMSTATE_UINT32(clock_mhz,AppleGpuM3IpadState),VMSTATE_UINT64(clock_last_ns,AppleGpuM3IpadState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass*k,const void*d){DeviceClass*dc=DEVICE_CLASS(k);PCIDeviceClass*pc=PCI_DEVICE_CLASS(k);pc->realize=realize;pc->vendor_id=GPU_VID;pc->device_id=GPU_DEV;pc->class_id=GPU_CLASS;pc->revision=0;dc->desc="Apple GPU M3 iPad (9-core)";dc->vmsd=&vms;dc->hotpluggable=false;device_class_set_props(dc,props);set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);}
static const TypeInfo ti={.name=TYPE_APPLE_GPU_M3_IPAD,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AppleGpuM3IpadState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
