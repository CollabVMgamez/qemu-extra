/*
 * Intel UHD Graphics 730 (Raptor Lake-S GT1, 24EU)
 * Vendor: 0x8086, Device: 0xA781
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-730-rpl-s
...
#define GPU_DEV      0xA781
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2281
#define GPU_REV      0x0C
#define GPU_CLASS    0x0300
#define BAR0_SIZE (16*MiB)
#define BAR2_SIZE (256*MiB)
#define GPU_CLK_BASE  300
#define GPU_CLK_BOOST 1550
#define TYPE_INTELUHD730RPLS "intel-uhd-730-rpl-s"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhd730RplSState, INTELUHD730RPLS)
struct IntelUhd730RplSState {
...
static uint32_t igpu_clk(IntelUhd730RplSState *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhd730RplSState *s = INTELUHD730RPLS(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-730-rpl-s-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-730-rpl-s-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhd730RplSState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-730-rpl-s",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhd730RplSState),VMSTATE_UINT32(clock_mhz,IntelUhd730RplSState),VMSTATE_UINT64(clock_last_ns,IntelUhd730RplSState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics 730 (Raptor Lake-S GT1, 24EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHD730RPLS,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhd730RplSState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
