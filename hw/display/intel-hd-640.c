/*
 * Intel HD Graphics 640 (Kaby Lake ULT GT3.5, 23EU)
 * Vendor: 0x8086, Device: 0x5921
 * Class: 0x0300, PCIe
 * Usage: -device intel-hd-640
...
#define GPU_DEV      0x5921
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2221
#define GPU_REV      0x02
...
#define TYPE_INTELHD640 "intel-hd-640"
OBJECT_DECLARE_SIMPLE_TYPE(IntelHd640State, INTELHD640)
struct IntelHd640State {
...
static uint32_t igpu_clk(IntelHd640State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelHd640State *s = INTELHD640(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-hd-640-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-hd-640-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelHd640State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-hd-640",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelHd640State),VMSTATE_UINT32(clock_mhz,IntelHd640State),VMSTATE_UINT64(clock_last_ns,IntelHd640State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel HD Graphics 640 (Kaby Lake ULT GT3.5, 23EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELHD640,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelHd640State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
