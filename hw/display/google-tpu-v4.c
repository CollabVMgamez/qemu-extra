/*
 * Google Cloud TPU v4 (32GB HBM per chip, PCIe gen4 x16, ICI mesh, 2021)
 * QEMU stub device for P4 fork.
 *
 * Vendor: 0x1AE0 (Google)
 * Device: 0x005E (Cloud TPU v4)
 */
...
#define TPU_DEVICE_ID   0x005E
#define TPU_SUBSYS_VID  0x1AE0
#define TPU_SUBSYS_DID  0x005E
#define TPU_REVISION    0x01
#define TPU_CLASS       0x1200
#define TPU_BAR0_SIZE   (16*MiB)
#define TPU_BAR2_SIZE   (256*MiB)
#define TPU_CLK_BASE    1050
#define TPU_CLK_BOOST   1050
#define TPU_MEM_MHZ     1600
#define TPU_PCIE_GEN    4
#define TYPE_GOOGLE_TPU_V4 "google-tpu-v4"
OBJECT_DECLARE_SIMPLE_TYPE(GoogleTpuV4State, GOOGLE_TPU_V4)
struct GoogleTpuV4State {
...
static uint32_t tpu_clk(GoogleTpuV4State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    GoogleTpuV4State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
    case 0x000000: return 0x00047001;
...
static void tpu_realize(PCIDevice *p, Error **e) {
    GoogleTpuV4State *s = (GoogleTpuV4State *)p;
    ...
    tpu_caps(p);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0ops, s, "google-tpu-v4-mmio", TPU_BAR0_SIZE);
    pci_register_bar(p, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2ops, s, "google-tpu-v4-hbm", TPU_BAR2_SIZE);
    pci_register_bar(p, 2, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar2);
}
static const VMStateDescription vms_google_tpu_v4 = {
    .name="google-tpu-v4",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,GoogleTpuV4State),VMSTATE_UINT32(clock_mhz,GoogleTpuV4State),VMSTATE_UINT64(clock_last_ns,GoogleTpuV4State),VMSTATE_END_OF_LIST()}
};
static const Property tpu_props[] = {
    DEFINE_PROP_STRING("chip-name", GoogleTpuV4State, chip_name),
    DEFINE_PROP_UINT32("chip-count", GoogleTpuV4State, chip_count, 1),
    DEFINE_PROP_STRING("board-partner", GoogleTpuV4State, board_partner),
};
static void tpu_class_init(ObjectClass *k, const void *d) {
    ...
    dc->desc = "Google Cloud TPU v4 (32GB HBM per chip, PCIe gen4, ICI mesh, 2021)";
    dc->vmsd = &vms_google_tpu_v4; dc->hotpluggable = false;
    device_class_set_props(dc, tpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY, dc->categories);
}
static const TypeInfo ti_google_tpu_v4 = {
    .name=TYPE_GOOGLE_TPU_V4,.parent=TYPE_PCI_DEVICE,
    .instance_size=sizeof(GoogleTpuV4State),.class_init=tpu_class_init,
    .interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}
};
static void reg_google_tpu_v4(void) { type_register_static(&ti_google_tpu_v4); }
type_init(reg_google_tpu_v4)
