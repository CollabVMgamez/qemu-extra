/*
 * Intel Iris Plus Graphics 655 (Coffee Lake GT3e, 48EU + 128MB eDRAM)
 * Vendor: 0x8086, Device: 0x3EA5
 * Class: 0x0300, PCIe
 * Usage: -device intel-iris-plus-655
...
#define GPU_DEV      0x3EA5
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x22A5
#define GPU_REV      0x00
...
#define TYPE_INTELIRISPLUS655 "intel-iris-plus-655"
OBJECT_DECLARE_SIMPLE_TYPE(IntelIrisPlus655State, INTELIRISPLUS655)
struct IntelIrisPlus655State {
...
static uint32_t igpu_clk(IntelIrisPlus655State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelIrisPlus655State *s = opaque;
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelIrisPlus655State *s = INTELIRISPLUS655(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-iris-plus-655-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-iris-plus-655-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelIrisPlus655State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-iris-plus-655",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelIrisPlus655State),VMSTATE_UINT32(clock_mhz,IntelIrisPlus655State),VMSTATE_UINT64(clock_last_ns,IntelIrisPlus655State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Iris Plus Graphics 655 (Coffee Lake GT3e, 48EU + 128MB eDRAM)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELIRISPLUS655,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelIrisPlus655State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
