/*
 * NVIDIA A16 (GA102, Ampere, 4x16GB GDDR6)
...
#define GPU_DEVICE_ID  0x25B6
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x14AD
...
#define GPU_CLK_BOOST   1430
#define GPU_PCIE_GEN    4
#define TYPE_NVIDIA_A16 "nvidia-a16"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaA16State, NVIDIA_A16)
struct NvidiaA16State {
...
static uint32_t gpu_clk(NvidiaA16State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaA16State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaA16State *s=(NvidiaA16State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-a16-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-a16-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-a16-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-a16-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_a16={.name="nvidia-a16",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaA16State),VMSTATE_UINT32(intr_en,NvidiaA16State),VMSTATE_UINT32(pfifo_intr_en,NvidiaA16State),VMSTATE_UINT32(clock_mhz,NvidiaA16State),VMSTATE_UINT64(clock_last_ns,NvidiaA16State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaA16State[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaA16State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaA16State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaA16State, board_partner),
};
...
static void reg_nvidia_a16(void){type_register_static(&ti_nvidia_a16);}
type_init(reg_nvidia_a16)
