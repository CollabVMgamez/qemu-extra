/* Samsung Xclipse 940 (Exynos 2400, AMD RDNA3)
 * Vendor: 0x144D, Device: 0x9400
 * Usage: -device samsung-xclipse-940
...
struct SamsungXclipse940State { PCIDevice parent_obj; MemoryRegion bar0,bar1; uint32_t clock_mhz; uint64_t clock_last_ns; char *gpu_name; };
...
static void realize(PCIDevice*p,Error**e){{
    SamsungXclipse940State*s=(SamsungXclipse940State*)p; s->clock_mhz=0;s->clock_last_ns=0;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0,s,"samsung-xclipse-940-mmio",BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1,s,"samsung-xclipse-940-vram",BAR1_SIZE);
    pci_register_bar(p,2,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
}}
static const Property props[]={DEFINE_PROP_STRING("gpu-name",SamsungXclipse940State,gpu_name),};
static const VMStateDescription vms={.name="samsung-xclipse-940",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,SamsungXclipse940State),VMSTATE_UINT32(clock_mhz,SamsungXclipse940State),VMSTATE_UINT64(clock_last_ns,SamsungXclipse940State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass*k,const void*d){DeviceClass*dc=DEVICE_CLASS(k);PCIDeviceClass*pc=PCI_DEVICE_CLASS(k);pc->realize=realize;pc->vendor_id=GPU_VID;pc->device_id=GPU_DEV;pc->class_id=GPU_CLASS;pc->revision=0;dc->desc="Samsung Xclipse 940 (Exynos 2400, AMD RDNA3)";dc->vmsd=&vms;dc->hotpluggable=false;device_class_set_props(dc,props);set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);}
static const TypeInfo ti={.name=TYPE_SAMSUNG_XCLIPSE_940,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(SamsungXclipse940State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
