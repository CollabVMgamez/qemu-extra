/*
 * Samsung Xclipse 940 (RDNA3, Exynos 2400, integrated)
...
#define GPU_VENDOR_ID  0x144D
#define GPU_DEVICE_ID  0x940A
#define GPU_SUBSYS_VID 0x144D
#define GPU_SUBSYS_DID 0x940A
#define GPU_REVISION   0xA1
#define GPU_CLASS      0x0300
#define GPU_MEM_MHZ    890
#define NV_BAR0_SIZE (16*MiB)
#define NV_BAR1_SIZE (64*MiB)
#define NV_BAR3_SIZE (8*MiB)
#define NV_BAR5_SIZE (64*KiB)
#define GPU_PMC_BOOT_0  0x53000002
...
#define GPU_CLK_BASE    890
#define GPU_CLK_BOOST   890
...
#define TYPE_SAMSUNG_XCLIPSE_940 "samsung-xclipse-940"
OBJECT_DECLARE_SIMPLE_TYPE(SamsungXclipse940State, SAMSUNG_XCLIPSE_940)
struct SamsungXclipse940State {
...
static uint32_t gpu_clk(SamsungXclipse940State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    SamsungXclipse940State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    SamsungXclipse940State *s=(SamsungXclipse940State*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"samsung-xclipse-940-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&bops,s,"samsung-xclipse-940-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&bops,s,"samsung-xclipse-940-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&bops,s,"samsung-xclipse-940-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_samsung_xclipse_940[]={DEFINE_PROP_UINT32("gpu-count",SamsungXclipse940State,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",SamsungXclipse940State,gpu_name)};
static const VMStateDescription vms_samsung_xclipse_940={.name="samsung-xclipse-940",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,SamsungXclipse940State),VMSTATE_UINT32(clock_mhz,SamsungXclipse940State),VMSTATE_UINT64(clock_last_ns,SamsungXclipse940State),VMSTATE_UINT32(gpu_count,SamsungXclipse940State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x940A;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="Samsung Xclipse 940 (RDNA3, Exynos 2400, integrated)"; dc->vmsd=&vms_samsung_xclipse_940; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_samsung_xclipse_940);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_samsung_xclipse_940={.name=TYPE_SAMSUNG_XCLIPSE_940,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(SamsungXclipse940State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_samsung_xclipse_940(void){type_register_static(&ti_samsung_xclipse_940);}
type_init(reg_samsung_xclipse_940)
