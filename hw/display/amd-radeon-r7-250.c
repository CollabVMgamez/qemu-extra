/*
 * AMD Radeon R7 250 (Oland, PCIe, 1GB GDDR5, 2013)
...
#define GPU_CLK_BASE    925
#define GPU_MEM_MHZ     1150
#define GPU_CLK_BOOST   925
#define GPU_PCIE_GEN    3
#define GPU_VRAM_MB     1024
#define TYPE_AMD_RADEON_R7_250 "amd-radeon-r7-250"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRadeonR7250State, AMD_RADEON_R7_250)
struct AmdRadeonR7250State {
...
static uint32_t gpu_clk(AmdRadeonR7250State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRadeonR7250State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRadeonR7250State *s=(AmdRadeonR7250State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-radeon-r7-250-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-radeon-r7-250-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-radeon-r7-250-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-radeon-r7-250-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_radeon_r7_250={.name="amd-radeon-r7-250",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRadeonR7250State),VMSTATE_UINT32(intr_en,AmdRadeonR7250State),VMSTATE_UINT32(clock_mhz,AmdRadeonR7250State),VMSTATE_UINT64(clock_last_ns,AmdRadeonR7250State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AmdRadeonR7250State[] = {
    DEFINE_PROP_STRING("gpu-name", AmdRadeonR7250State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AmdRadeonR7250State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AmdRadeonR7250State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="AMD Radeon R7 250 (Oland, PCIe, 1GB GDDR5, 2013)"; dc->vmsd=&vms_amd_radeon_r7_250; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRadeonR7250State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_radeon_r7_250={.name=TYPE_AMD_RADEON_R7_250,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRadeonR7250State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_radeon_r7_250(void){type_register_static(&ti_amd_radeon_r7_250);}
type_init(reg_amd_radeon_r7_250)
