/*
 * ARM Mali-G72 MP12 (Exynos 9810, integrated)
...
#define TYPE_MALI_G72 "mali-g72"
OBJECT_DECLARE_SIMPLE_TYPE(MaliG72State, MALI_G72)
struct MaliG72State {
...
static uint32_t gpu_clk(MaliG72State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    MaliG72State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"mali-g72-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&bops,s,"mali-g72-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&bops,s,"mali-g72-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&bops,s,"mali-g72-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_mali_g72[]={DEFINE_PROP_UINT32("gpu-count",MaliG72State,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",MaliG72State,gpu_name)};
static const VMStateDescription vms_mali_g72={.name="mali-g72",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,MaliG72State),VMSTATE_UINT32(clock_mhz,MaliG72State),VMSTATE_UINT64(clock_last_ns,MaliG72State),VMSTATE_UINT32(gpu_count,MaliG72State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0xB072;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="ARM Mali-G72 MP12 (Exynos 9810, integrated)"; dc->vmsd=&vms_mali_g72; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_mali_g72);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_mali_g72={.name=TYPE_MALI_G72,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(MaliG72State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_mali_g72(void){type_register_static(&ti_mali_g72);}
type_init(reg_mali_g72)
