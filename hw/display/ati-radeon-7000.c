/*
 * ATI Radeon 7000 / VE (RV100, AGP, 32MB DDR, 2001)
...
#define GPU_DEVICE_ID  0x5159
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x5159
...
#define GPU_CLK_BASE    143
#define GPU_MEM_MHZ     143
#define GPU_CLK_BOOST   143
#define GPU_PCIE_GEN    1
#define GPU_VRAM_MB     32
#define TYPE_ATI_RADEON_7000 "ati-radeon-7000"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeon7000State, ATI_RADEON_7000)
struct AtiRadeon7000State {
...
static uint32_t gpu_clk(AtiRadeon7000State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeon7000State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeon7000State *s=(AtiRadeon7000State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-7000-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-7000-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-7000-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-7000-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_7000={.name="ati-radeon-7000",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeon7000State),VMSTATE_UINT32(intr_en,AtiRadeon7000State),VMSTATE_UINT32(clock_mhz,AtiRadeon7000State),VMSTATE_UINT64(clock_last_ns,AtiRadeon7000State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeon7000State[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeon7000State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeon7000State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeon7000State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="ATI Radeon 7000 / VE (RV100, AGP, 32MB DDR, 2001)"; dc->vmsd=&vms_ati_radeon_7000; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRadeon7000State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_radeon_7000={.name=TYPE_ATI_RADEON_7000,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRadeon7000State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_radeon_7000(void){type_register_static(&ti_ati_radeon_7000);}
type_init(reg_ati_radeon_7000)
