/*
 * Cerebras WSE-3 (Wafer Scale Engine 3, 4 trillion transistors, 44GB SRAM,
 * 900,000 AI cores, PCIe gen5 x16, 2023)
 * QEMU stub device for P4 fork.
 *
 * Vendor ID 0x1DA2 (Cerebras Systems), Device 0x0003.
 */
...
#define WSE_DEVICE_ID   0x0003
#define WSE_SUBSYS_VID  0x1DA2
#define WSE_SUBSYS_DID  0x0003
#define WSE_REVISION    0x01
#define WSE_CLASS       0x1200
#define WSE_BAR0_SIZE   (16*MiB)
#define WSE_BAR2_SIZE   (256*MiB)
/* WSE-3: 900 MHz fabric clock, 44 GB on-chip SRAM */
#define WSE_CLK_BASE    900
#define WSE_CLK_BOOST   900
#define WSE_MEM_MHZ     900
#define WSE_PCIE_GEN    5
#define TYPE_CEREBRAS_WSE3 "cerebras-wse-3"
OBJECT_DECLARE_SIMPLE_TYPE(CerebrasWse3State, CEREBRAS_WSE3)
struct CerebrasWse3State {
...
static uint32_t wse_clk(CerebrasWse3State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    CerebrasWse3State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
    case 0x000000: return 0xCE030001; /* Cerebras WSE gen3 */
    case 0x000004: return 0x00000001;
    /* Core count: 900,000 */
    case 0x000008: return 900000;
    /* SRAM: 44 GB in KiB */
    case 0x00000C: return (uint32_t)(44ULL * 1024 * 1024);
...
static void wse_realize(PCIDevice *p, Error **e) {
    CerebrasWse3State *s = (CerebrasWse3State *)p;
    ...
    wse_caps(p);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0ops, s, "cerebras-wse-3-mmio", WSE_BAR0_SIZE);
    pci_register_bar(p, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2ops, s, "cerebras-wse-3-sram", WSE_BAR2_SIZE);
    pci_register_bar(p, 2, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar2);
}
static const VMStateDescription vms_cerebras_wse3 = {
    .name="cerebras-wse-3",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,CerebrasWse3State),VMSTATE_UINT32(clock_mhz,CerebrasWse3State),VMSTATE_UINT64(clock_last_ns,CerebrasWse3State),VMSTATE_END_OF_LIST()}
};
static const Property wse_props[] = {
    DEFINE_PROP_STRING("chip-name", CerebrasWse3State, chip_name),
    DEFINE_PROP_UINT32("chip-count", CerebrasWse3State, chip_count, 1),
    DEFINE_PROP_STRING("board-partner", CerebrasWse3State, board_partner),
};
static void wse_class_init(ObjectClass *k, const void *d) {
    ...
    dc->desc = "Cerebras WSE-3 (Wafer Scale Engine 3, 900K AI cores, 44GB SRAM, 2023)";
    dc->vmsd = &vms_cerebras_wse3; dc->hotpluggable = false;
    device_class_set_props(dc, wse_props);
    set_bit(DEVICE_CATEGORY_DISPLAY, dc->categories);
}
static const TypeInfo ti_cerebras_wse3 = {
    .name=TYPE_CEREBRAS_WSE3,.parent=TYPE_PCI_DEVICE,
    .instance_size=sizeof(CerebrasWse3State),.class_init=wse_class_init,
    .interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}
};
static void reg_cerebras_wse3(void) { type_register_static(&ti_cerebras_wse3); }
type_init(reg_cerebras_wse3)
