/*
 * AMD Radeon RX Vega 56 (Vega10, GCN 5.0)
...
#define GPU_PMC_BOOT_0  0x138000C1
...
#define GPU_CLK_BASE    1156
#define GPU_MEM_MHZ     800
#define GPU_CLK_BOOST   1471
#define GPU_PCIE_GEN    3
#define TYPE_AMD_RX_VEGA56 "amd-rx-vega56"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRxVega56State, AMD_RX_VEGA56)
struct AmdRxVega56State {
...
static uint32_t gpu_clk(AmdRxVega56State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRxVega56State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRxVega56State *s=(AmdRxVega56State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-rx-vega56-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-rx-vega56-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-rx-vega56-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-rx-vega56-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_rx_vega56={.name="amd-rx-vega56",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRxVega56State),VMSTATE_UINT32(intr_en,AmdRxVega56State),VMSTATE_UINT32(pfifo_intr_en,AmdRxVega56State),VMSTATE_UINT32(clock_mhz,AmdRxVega56State),VMSTATE_UINT64(clock_last_ns,AmdRxVega56State),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="AMD Radeon RX Vega 56 (Vega10, GCN 5.0)"; dc->vmsd=&vms_amd_rx_vega56; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRxVega56State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_rx_vega56={.name=TYPE_AMD_RX_VEGA56,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRxVega56State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_rx_vega56(void){type_register_static(&ti_amd_rx_vega56);}
type_init(reg_amd_rx_vega56)
