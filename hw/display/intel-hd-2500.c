/*
 * Intel HD Graphics 2500 (Ivy Bridge GT1, 6EU)
 * Vendor: 0x8086, Device: 0x0152
 * Class: 0x0300, PCIe
 * Usage: -device intel-hd-2500
...
#define TYPE_INTELHD2500 "intel-hd-2500"
OBJECT_DECLARE_SIMPLE_TYPE(IntelHd2500State, INTELHD2500)
struct IntelHd2500State {
...
static uint32_t igpu_clk(IntelHd2500State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelHd2500State *s = INTELHD2500(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-hd-2500-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-hd-2500-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelHd2500State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-hd-2500",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelHd2500State),VMSTATE_UINT32(clock_mhz,IntelHd2500State),VMSTATE_UINT64(clock_last_ns,IntelHd2500State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel HD Graphics 2500 (Ivy Bridge GT1, 6EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELHD2500,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelHd2500State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
