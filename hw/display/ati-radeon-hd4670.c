/*
 * ATI Radeon HD 4670 (RV730, PCIe, 512MB GDDR3, 2008)
...
#define GPU_DEVICE_ID  0x9498
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x9499
...
#define TYPE_ATI_RADEON_HD4670 "ati-radeon-hd4670"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeonHd4670State, ATI_RADEON_HD4670)
struct AtiRadeonHd4670State {
...
static uint32_t gpu_clk(AtiRadeonHd4670State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeonHd4670State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeonHd4670State *s=(AtiRadeonHd4670State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-hd4670-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-hd4670-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-hd4670-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-hd4670-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_hd4670={.name="ati-radeon-hd4670",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeonHd4670State),VMSTATE_UINT32(intr_en,AtiRadeonHd4670State),VMSTATE_UINT32(clock_mhz,AtiRadeonHd4670State),VMSTATE_UINT64(clock_last_ns,AtiRadeonHd4670State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeonHd4670State[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeonHd4670State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeonHd4670State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeonHd4670State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="ATI Radeon HD 4670 (RV730, PCIe, 512MB GDDR3, 2008)"; dc->vmsd=&vms_ati_radeon_hd4670; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRadeonHd4670State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_radeon_hd4670={.name=TYPE_ATI_RADEON_HD4670,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRadeonHd4670State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_radeon_hd4670(void){type_register_static(&ti_ati_radeon_hd4670);}
type_init(reg_ati_radeon_hd4670)
