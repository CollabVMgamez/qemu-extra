/*
 * NVIDIA GeForce 6800 (NV41, AGP 8x, 256MB DDR, 2004)
...
#define GPU_DEVICE_ID  0x00C8
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x00C8
#define GPU_REVISION   0x00
#define GPU_CLASS      0x0300
#define GPU_MEM_MHZ    325
...
#define GPU_CLK_BASE    325
#define GPU_CLK_BOOST   325
#define AGP_CAP_OFFSET  0x40

#define TYPE_NVIDIA_NV41_GEFORCE6800 "nvidia-nv41-geforce6800"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaNv41Geforce6800State, NVIDIA_NV41_GEFORCE6800)
struct NvidiaNv41Geforce6800State {
...
static uint32_t gpu_clk(NvidiaNv41Geforce6800State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaNv41Geforce6800State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaNv41Geforce6800State *s=(NvidiaNv41Geforce6800State*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-nv41-geforce6800-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b135ops,s,"nvidia-nv41-geforce6800-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b135ops,s,"nvidia-nv41-geforce6800-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b135ops,s,"nvidia-nv41-geforce6800-vgaio",NV_BAR5_SIZE);
...
static const Property gpu_props_nvidia_nv41_geforce6800[]={DEFINE_PROP_UINT32("gpu-count",NvidiaNv41Geforce6800State,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",NvidiaNv41Geforce6800State,gpu_name),
    DEFINE_PROP_STRING("board-partner",NvidiaNv41Geforce6800State,board_partner)};
static const VMStateDescription vms_nvidia_nv41_geforce6800={.name="nvidia-nv41-geforce6800",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaNv41Geforce6800State),VMSTATE_UINT32(intr_en,NvidiaNv41Geforce6800State),VMSTATE_UINT32(clock_mhz,NvidiaNv41Geforce6800State),VMSTATE_UINT64(clock_last_ns,NvidiaNv41Geforce6800State),
        VMSTATE_UINT32(gpu_count,NvidiaNv41Geforce6800State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x00C8;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA GeForce 6800 (NV41, AGP 8x, 256MB DDR, 2004)"; dc->vmsd=&vms_nvidia_nv41_geforce6800; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_nvidia_nv41_geforce6800);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_nv41_geforce6800={.name=TYPE_NVIDIA_NV41_GEFORCE6800,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaNv41Geforce6800State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_nv41_geforce6800(void){type_register_static(&ti_nvidia_nv41_geforce6800);}
type_init(reg_nvidia_nv41_geforce6800)
