/*
 * Intel Arc Pro A30M (ACM-G11 Mobile, 35W, 4GB)
 * Vendor: 0x8086, Device: 0x56C0
 * Class: 0x0302, PCIe
 * Usage: -device intel-arc-pro-a30m
...
#define GPU_SUBSYS_D 0x22C0
#define GPU_REV      0x05
#define GPU_CLASS    0x0302
#define BAR0_SIZE (32*MiB)
#define BAR2_SIZE (4096*MiB)
#define GPU_CLK_BASE  300
#define GPU_CLK_BOOST 1800
#define TYPE_INTELARCPROA30M "intel-arc-pro-a30m"
OBJECT_DECLARE_SIMPLE_TYPE(IntelArcProA30mState, INTELARCPROA30M)
struct IntelArcProA30mState {
...
static uint32_t igpu_clk(IntelArcProA30mState *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelArcProA30mState *s = INTELARCPROA30M(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-arc-pro-a30m-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-arc-pro-a30m-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelArcProA30mState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-arc-pro-a30m",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelArcProA30mState),VMSTATE_UINT32(clock_mhz,IntelArcProA30mState),VMSTATE_UINT64(clock_last_ns,IntelArcProA30mState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Arc Pro A30M (ACM-G11 Mobile, 35W, 4GB)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELARCPROA30M,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelArcProA30mState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
