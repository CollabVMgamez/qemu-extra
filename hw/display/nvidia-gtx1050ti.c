/*
 * NVIDIA GeForce GTX 1050 Ti (GP107, Pascal)
...
#define GPU_DEVICE_ID  0x1C82
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x1101
...
#define GPU_CLK_BASE    1290
#define GPU_MEM_MHZ     1752
#define GPU_CLK_BOOST   1392
#define GPU_PCIE_GEN    3
#define TYPE_NVIDIA_GTX1050TI "nvidia-gtx1050ti"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx1050tiState, NVIDIA_GTX1050TI)
struct NvidiaGtx1050tiState {
...
static uint32_t gpu_clk(NvidiaGtx1050tiState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaGtx1050tiState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaGtx1050tiState *s=(NvidiaGtx1050tiState*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-gtx1050ti-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-gtx1050ti-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-gtx1050ti-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-gtx1050ti-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_gtx1050ti={.name="nvidia-gtx1050ti",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaGtx1050tiState),VMSTATE_UINT32(intr_en,NvidiaGtx1050tiState),VMSTATE_UINT32(pfifo_intr_en,NvidiaGtx1050tiState),VMSTATE_UINT32(clock_mhz,NvidiaGtx1050tiState),VMSTATE_UINT64(clock_last_ns,NvidiaGtx1050tiState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaGtx1050tiState[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaGtx1050tiState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaGtx1050tiState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaGtx1050tiState, board_partner),
};
...
static void reg_nvidia_gtx1050ti(void){type_register_static(&ti_nvidia_gtx1050ti);}
type_init(reg_nvidia_gtx1050ti)
