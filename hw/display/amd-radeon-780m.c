/* AMD Radeon 780M (RDNA3 iGPU, Phoenix)
 * Vendor: 0x1002, Device: 0x15BF
 * Usage: -device amd-radeon-780m
...
#define GPU_SS_D 0x0C44
#define GPU_CLASS 0x0300
#define BAR0_SIZE (4*MiB)
#define BAR1_SIZE (64*MiB)
#define TYPE_AMD_RADEON_780M "amd-radeon-780m"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRadeon780mState, AMD_RADEON_780M)
struct AmdRadeon780mState { PCIDevice parent_obj; MemoryRegion bar0,bar1; uint32_t clock_mhz; uint64_t clock_last_ns; char *gpu_name;     char *board_partner;
};
...
static void realize(PCIDevice*p,Error**e){{
    AmdRadeon780mState*s=(AmdRadeon780mState*)p; s->clock_mhz=0;s->clock_last_ns=0;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0,s,"amd-radeon-780m-mmio",BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1,s,"amd-radeon-780m-vram",BAR1_SIZE);
    pci_register_bar(p,2,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
}}
static const Property props[]={DEFINE_PROP_STRING("gpu-name",AmdRadeon780mState,gpu_name),    DEFINE_PROP_STRING("board-partner", AmdRadeon780mState, board_partner),
};
static const VMStateDescription vms={.name="amd-radeon-780m",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRadeon780mState),VMSTATE_UINT32(clock_mhz,AmdRadeon780mState),VMSTATE_UINT64(clock_last_ns,AmdRadeon780mState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass*k,const void*d){DeviceClass*dc=DEVICE_CLASS(k);PCIDeviceClass*pc=PCI_DEVICE_CLASS(k);pc->realize=realize;pc->vendor_id=GPU_VID;pc->device_id=GPU_DEV;pc->class_id=GPU_CLASS;pc->revision=0;dc->desc="AMD Radeon 780M (RDNA3 iGPU, Phoenix)";dc->vmsd=&vms;dc->hotpluggable=false;device_class_set_props(dc,props);set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);}
static const TypeInfo ti={.name=TYPE_AMD_RADEON_780M,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRadeon780mState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
