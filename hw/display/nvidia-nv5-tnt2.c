/*
 * NVIDIA TNT2 (NV5, AGP 4x, 32MB, 1999)
...
#define GPU_DEVICE_ID  0x0028
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x0028
#define GPU_REVISION   0x00
#define GPU_CLASS      0x0300
#define GPU_MEM_MHZ    150
...
#define GPU_CLK_BASE    150
#define GPU_CLK_BOOST   150
#define AGP_CAP_OFFSET  0x40

#define TYPE_NVIDIA_NV5_TNT2 "nvidia-nv5-tnt2"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaNv5Tnt2State, NVIDIA_NV5_TNT2)
struct NvidiaNv5Tnt2State {
...
static uint32_t gpu_clk(NvidiaNv5Tnt2State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaNv5Tnt2State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaNv5Tnt2State *s=(NvidiaNv5Tnt2State*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-nv5-tnt2-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b135ops,s,"nvidia-nv5-tnt2-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b135ops,s,"nvidia-nv5-tnt2-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b135ops,s,"nvidia-nv5-tnt2-vgaio",NV_BAR5_SIZE);
...
static const Property gpu_props_nvidia_nv5_tnt2[]={DEFINE_PROP_UINT32("gpu-count",NvidiaNv5Tnt2State,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",NvidiaNv5Tnt2State,gpu_name),
    DEFINE_PROP_STRING("board-partner",NvidiaNv5Tnt2State,board_partner)};
static const VMStateDescription vms_nvidia_nv5_tnt2={.name="nvidia-nv5-tnt2",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaNv5Tnt2State),VMSTATE_UINT32(intr_en,NvidiaNv5Tnt2State),VMSTATE_UINT32(clock_mhz,NvidiaNv5Tnt2State),VMSTATE_UINT64(clock_last_ns,NvidiaNv5Tnt2State),
        VMSTATE_UINT32(gpu_count,NvidiaNv5Tnt2State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x0028;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA TNT2 (NV5, AGP 4x, 32MB, 1999)"; dc->vmsd=&vms_nvidia_nv5_tnt2; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_nvidia_nv5_tnt2);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_nv5_tnt2={.name=TYPE_NVIDIA_NV5_TNT2,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaNv5Tnt2State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_nv5_tnt2(void){type_register_static(&ti_nvidia_nv5_tnt2);}
type_init(reg_nvidia_nv5_tnt2)
