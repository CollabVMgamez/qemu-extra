/*
 * Intel Iris Pro Graphics P580 (Broadwell Server GT3e, 48EU + eDRAM)
 * Vendor: 0x8086, Device: 0x162D
 * Class: 0x0300, PCIe
 * Usage: -device intel-iris-pro-p580
...
#define GPU_DEV      0x162D
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x232D
#define GPU_REV      0x08
#define GPU_CLASS    0x0300
#define BAR0_SIZE (32*MiB)
#define BAR2_SIZE (512*MiB)
#define GPU_CLK_BASE  300
#define GPU_CLK_BOOST 1100
#define TYPE_INTELIRISPROP580 "intel-iris-pro-p580"
OBJECT_DECLARE_SIMPLE_TYPE(IntelIrisProP580State, INTELIRISPROP580)
struct IntelIrisProP580State {
...
static uint32_t igpu_clk(IntelIrisProP580State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelIrisProP580State *s = opaque;
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelIrisProP580State *s = INTELIRISPROP580(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-iris-pro-p580-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-iris-pro-p580-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelIrisProP580State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-iris-pro-p580",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelIrisProP580State),VMSTATE_UINT32(clock_mhz,IntelIrisProP580State),VMSTATE_UINT64(clock_last_ns,IntelIrisProP580State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Iris Pro Graphics P580 (Broadwell Server GT3e, 48EU + eDRAM)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELIRISPROP580,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelIrisProP580State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
