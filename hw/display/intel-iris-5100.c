/*
 * Intel Iris Graphics 5100 (Haswell ULT GT3, 40EU)
 * Vendor: 0x8086, Device: 0x0A2E
 * Class: 0x0300, PCIe
 * Usage: -device intel-iris-5100
...
#define GPU_DEV      0x0A2E
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x222E
#define GPU_REV      0x06
...
#define TYPE_INTELIRIS5100 "intel-iris-5100"
OBJECT_DECLARE_SIMPLE_TYPE(IntelIris5100State, INTELIRIS5100)
struct IntelIris5100State {
...
static uint32_t igpu_clk(IntelIris5100State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelIris5100State *s = INTELIRIS5100(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-iris-5100-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-iris-5100-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelIris5100State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-iris-5100",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelIris5100State),VMSTATE_UINT32(clock_mhz,IntelIris5100State),VMSTATE_UINT64(clock_last_ns,IntelIris5100State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel Iris Graphics 5100 (Haswell ULT GT3, 40EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELIRIS5100,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelIris5100State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
