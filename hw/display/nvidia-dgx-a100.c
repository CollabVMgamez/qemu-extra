/*
 * NVIDIA DGX A100 node GPU (GA100, 40GB, 8x per node)
...
#define TYPE_NVIDIA_DGX_A100 "nvidia-dgx-a100"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaDgxA100State, NVIDIA_DGX_A100)
struct NvidiaDgxA100State {
...
static uint32_t gpu_clk(NvidiaDgxA100State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaDgxA100State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaDgxA100State *s=(NvidiaDgxA100State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-dgx-a100-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-dgx-a100-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-dgx-a100-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-dgx-a100-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_dgx_a100={.name="nvidia-dgx-a100",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaDgxA100State),VMSTATE_UINT32(intr_en,NvidiaDgxA100State),VMSTATE_UINT32(pfifo_intr_en,NvidiaDgxA100State),VMSTATE_UINT32(clock_mhz,NvidiaDgxA100State),VMSTATE_UINT64(clock_last_ns,NvidiaDgxA100State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaDgxA100State[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaDgxA100State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaDgxA100State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaDgxA100State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="NVIDIA DGX A100 node GPU (GA100, 40GB, 8x per node)"; dc->vmsd=&vms_nvidia_dgx_a100; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaDgxA100State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_dgx_a100={.name=TYPE_NVIDIA_DGX_A100,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaDgxA100State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_dgx_a100(void){type_register_static(&ti_nvidia_dgx_a100);}
type_init(reg_nvidia_dgx_a100)
