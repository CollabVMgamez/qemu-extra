/*
 * AMD Radeon RX 7900 XTX (Navi31, RDNA 3.0)
...
#define GPU_DEVICE_ID  0x744C
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x7900
...
#define GPU_PMC_BOOT_0  0x168000C3
#define GPU_PFB_BOOT_0  0x00000015
#define GPU_PFB_PARTS   0x00000006
#define GPU_PFB_REFCTRL 0x80000083
#define GPU_CLK_BASE    1900
#define GPU_MEM_MHZ     2500
#define GPU_CLK_BOOST   2499
#define GPU_PCIE_GEN    4
#define TYPE_AMD_RX7900XTX "amd-rx7900xtx"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRx7900xtxState, AMD_RX7900XTX)
struct AmdRx7900xtxState {
...
static uint32_t gpu_clk(AmdRx7900xtxState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRx7900xtxState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRx7900xtxState *s=(AmdRx7900xtxState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-rx7900xtx-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-rx7900xtx-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-rx7900xtx-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-rx7900xtx-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_rx7900xtx={.name="amd-rx7900xtx",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRx7900xtxState),VMSTATE_UINT32(intr_en,AmdRx7900xtxState),VMSTATE_UINT32(pfifo_intr_en,AmdRx7900xtxState),VMSTATE_UINT32(clock_mhz,AmdRx7900xtxState),VMSTATE_UINT64(clock_last_ns,AmdRx7900xtxState),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="AMD Radeon RX 7900 XTX (Navi31, RDNA 3.0)"; dc->vmsd=&vms_amd_rx7900xtx; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRx7900xtxState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_rx7900xtx={.name=TYPE_AMD_RX7900XTX,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRx7900xtxState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_rx7900xtx(void){type_register_static(&ti_amd_rx7900xtx);}
type_init(reg_amd_rx7900xtx)
