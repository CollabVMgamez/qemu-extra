/*
 * Google Cloud TPU v5p (95GB HBM per chip, PCIe gen5 x16, 2023, training)
 * QEMU stub device for P4 fork.
 *
 * Vendor: 0x1AE0 (Google)
 * Device: 0x0064 (Cloud TPU v5p)
 */
...
#define TPU_DEVICE_ID   0x0064
#define TPU_SUBSYS_VID  0x1AE0
#define TPU_SUBSYS_DID  0x0064
...
#define TPU_MEM_MHZ     1800
#define TPU_PCIE_GEN    5
#define TYPE_GOOGLE_TPU_V5P "google-tpu-v5p"
OBJECT_DECLARE_SIMPLE_TYPE(GoogleTpuV5pState, GOOGLE_TPU_V5P)
...
static uint32_t tpu_clk(GoogleTpuV5pState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    GoogleTpuV5pState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
    case 0x000000: return 0x00057501; /* 5p variant */
...
static void tpu_realize(PCIDevice *p, Error **e) {
    GoogleTpuV5pState *s = (GoogleTpuV5pState *)p;
    ...
    tpu_caps(p);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0ops, s, "google-tpu-v5p-mmio", TPU_BAR0_SIZE);
    pci_register_bar(p, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2ops, s, "google-tpu-v5p-hbm", TPU_BAR2_SIZE);
    pci_register_bar(p, 2, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar2);
}
static const VMStateDescription vms_google_tpu_v5p = {
    .name="google-tpu-v5p",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,GoogleTpuV5pState),VMSTATE_UINT32(clock_mhz,GoogleTpuV5pState),VMSTATE_UINT64(clock_last_ns,GoogleTpuV5pState),VMSTATE_END_OF_LIST()}
};
static const Property tpu_props[] = {
    DEFINE_PROP_STRING("chip-name", GoogleTpuV5pState, chip_name),
    DEFINE_PROP_UINT32("chip-count", GoogleTpuV5pState, chip_count, 1),
    DEFINE_PROP_STRING("board-partner", GoogleTpuV5pState, board_partner),
};
static void tpu_class_init(ObjectClass *k, const void *d) {
    ...
    dc->desc = "Google Cloud TPU v5p (95GB HBM per chip, PCIe gen5, training, 2023)";
    dc->vmsd = &vms_google_tpu_v5p; dc->hotpluggable = false;
    device_class_set_props(dc, tpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY, dc->categories);
}
static const TypeInfo ti_google_tpu_v5p = {
    .name=TYPE_GOOGLE_TPU_V5P,.parent=TYPE_PCI_DEVICE,
    .instance_size=sizeof(GoogleTpuV5pState),.class_init=tpu_class_init,
    .interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}
};
static void reg_google_tpu_v5p(void) { type_register_static(&ti_google_tpu_v5p); }
type_init(reg_google_tpu_v5p)
