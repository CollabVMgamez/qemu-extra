/*
 * Intel UHD Graphics (Jasper Lake GT2, 24EU)
 * Vendor: 0x8086, Device: 0x4E71
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-jasper-lake
...
#define TYPE_INTELUHDJASPERLAKE "intel-uhd-jasper-lake"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhdJasperLakeState, INTELUHDJASPERLAKE)
struct IntelUhdJasperLakeState {
...
static uint32_t igpu_clk(IntelUhdJasperLakeState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelUhdJasperLakeState *s = opaque;
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhdJasperLakeState *s = INTELUHDJASPERLAKE(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-jasper-lake-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-jasper-lake-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhdJasperLakeState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-jasper-lake",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhdJasperLakeState),VMSTATE_UINT32(clock_mhz,IntelUhdJasperLakeState),VMSTATE_UINT64(clock_last_ns,IntelUhdJasperLakeState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics (Jasper Lake GT2, 24EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHDJASPERLAKE,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhdJasperLakeState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
