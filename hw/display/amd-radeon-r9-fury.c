/*
 * AMD Radeon R9 Fury (Fiji, PCIe x16, 4GB HBM, 2015)
...
#define TYPE_AMD_RADEON_R9_FURY "amd-radeon-r9-fury"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRadeonR9FuryState, AMD_RADEON_R9_FURY)
struct AmdRadeonR9FuryState {
...
static uint32_t gpu_clk(AmdRadeonR9FuryState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRadeonR9FuryState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRadeonR9FuryState *s=(AmdRadeonR9FuryState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-radeon-r9-fury-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-radeon-r9-fury-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-radeon-r9-fury-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-radeon-r9-fury-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_radeon_r9_fury={.name="amd-radeon-r9-fury",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRadeonR9FuryState),VMSTATE_UINT32(intr_en,AmdRadeonR9FuryState),VMSTATE_UINT32(clock_mhz,AmdRadeonR9FuryState),VMSTATE_UINT64(clock_last_ns,AmdRadeonR9FuryState),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="AMD Radeon R9 Fury (Fiji, PCIe x16, 4GB HBM, 2015)"; dc->vmsd=&vms_amd_radeon_r9_fury; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRadeonR9FuryState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_radeon_r9_fury={.name=TYPE_AMD_RADEON_R9_FURY,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRadeonR9FuryState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_radeon_r9_fury(void){type_register_static(&ti_amd_radeon_r9_fury);}
type_init(reg_amd_radeon_r9_fury)
