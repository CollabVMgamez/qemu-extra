/*
 * Intel HD Graphics P530 (Skylake Server GT2, 24EU)
 * Vendor: 0x8086, Device: 0x191D
 * Class: 0x0300, PCIe
 * Usage: -device intel-hd-p530
...
#define GPU_DEV      0x191D
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x221D
#define GPU_REV      0x06
...
#define TYPE_INTELHDP530 "intel-hd-p530"
OBJECT_DECLARE_SIMPLE_TYPE(IntelHdP530State, INTELHDP530)
struct IntelHdP530State {
...
static uint32_t igpu_clk(IntelHdP530State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelHdP530State *s = INTELHDP530(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-hd-p530-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-hd-p530-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelHdP530State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-hd-p530",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelHdP530State),VMSTATE_UINT32(clock_mhz,IntelHdP530State),VMSTATE_UINT64(clock_last_ns,IntelHdP530State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel HD Graphics P530 (Skylake Server GT2, 24EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELHDP530,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelHdP530State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
