/*
 * Cerebras WSE-2 (Wafer Scale Engine 2, 2.6 trillion transistors, 40GB SRAM,
 * 850,000 AI cores, PCIe gen4 x16, 2021)
 * QEMU stub device for P4 fork.
 *
 * Vendor ID 0x1DA2 (Cerebras Systems), Device 0x0002.
 */
...
#define WSE_DEVICE_ID   0x0002
#define WSE_SUBSYS_VID  0x1DA2
#define WSE_SUBSYS_DID  0x0002
#define WSE_REVISION    0x01
#define WSE_CLASS       0x1200
#define WSE_BAR0_SIZE   (16*MiB)
#define WSE_BAR2_SIZE   (256*MiB)
/* WSE-2: 1 GHz fabric clock, 40 GB on-chip SRAM */
#define WSE_CLK_BASE    1000
#define WSE_CLK_BOOST   1000
#define WSE_MEM_MHZ     1000
#define WSE_PCIE_GEN    4
#define TYPE_CEREBRAS_WSE2 "cerebras-wse-2"
OBJECT_DECLARE_SIMPLE_TYPE(CerebrasWse2State, CEREBRAS_WSE2)
struct CerebrasWse2State {
...
static uint32_t wse_clk(CerebrasWse2State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    CerebrasWse2State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
    case 0x000000: return 0xCE020001; /* Cerebras WSE gen2 */
    case 0x000004: return 0x00000001;
    /* Core count: 850,000 */
    case 0x000008: return 850000;
    /* SRAM: 40 GB in KiB */
    case 0x00000C: return (uint32_t)(40ULL * 1024 * 1024);
...
static void wse_realize(PCIDevice *p, Error **e) {
    CerebrasWse2State *s = (CerebrasWse2State *)p;
    ...
    wse_caps(p);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0ops, s, "cerebras-wse-2-mmio", WSE_BAR0_SIZE);
    pci_register_bar(p, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2ops, s, "cerebras-wse-2-sram", WSE_BAR2_SIZE);
    pci_register_bar(p, 2, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar2);
}
static const VMStateDescription vms_cerebras_wse2 = {
    .name="cerebras-wse-2",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,CerebrasWse2State),VMSTATE_UINT32(clock_mhz,CerebrasWse2State),VMSTATE_UINT64(clock_last_ns,CerebrasWse2State),VMSTATE_END_OF_LIST()}
};
static const Property wse_props[] = {
    DEFINE_PROP_STRING("chip-name", CerebrasWse2State, chip_name),
    DEFINE_PROP_UINT32("chip-count", CerebrasWse2State, chip_count, 1),
    DEFINE_PROP_STRING("board-partner", CerebrasWse2State, board_partner),
};
static void wse_class_init(ObjectClass *k, const void *d) {
    ...
    dc->desc = "Cerebras WSE-2 (Wafer Scale Engine 2, 850K AI cores, 40GB SRAM, 2021)";
    dc->vmsd = &vms_cerebras_wse2; dc->hotpluggable = false;
    device_class_set_props(dc, wse_props);
    set_bit(DEVICE_CATEGORY_DISPLAY, dc->categories);
}
static const TypeInfo ti_cerebras_wse2 = {
    .name=TYPE_CEREBRAS_WSE2,.parent=TYPE_PCI_DEVICE,
    .instance_size=sizeof(CerebrasWse2State),.class_init=wse_class_init,
    .interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}
};
static void reg_cerebras_wse2(void) { type_register_static(&ti_cerebras_wse2); }
type_init(reg_cerebras_wse2)
