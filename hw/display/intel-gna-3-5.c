/*
 * Intel GNA 3.5 (Lunar Lake Neural Accelerator)
 * Vendor: 0x8086, Device: 0x7E44
 * Class: 0x1180 (Processing Accelerator)
 * Usage: -device intel-gna-3-5
...
#define GPU_DEV      0x7E44
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2244
#define GPU_REV      0x00
#define GPU_CLASS    0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_INTELGNA35 "intel-gna-3-5"
OBJECT_DECLARE_SIMPLE_TYPE(IntelGna35State, INTELGNA35)
struct IntelGna35State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    IntelGna35State *s = INTELGNA35(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-gna-3-5-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="intel-gna-3-5",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelGna35State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel GNA 3.5 (Lunar Lake Neural Accelerator)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELGNA35,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelGna35State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
