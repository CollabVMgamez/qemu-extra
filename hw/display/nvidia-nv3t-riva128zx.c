/*
 * NVIDIA RIVA 128 ZX (NV3T, AGP 2x, 8MB, 1998)
...
#define GPU_DEVICE_ID  0x0019
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x0019
...
#define GPU_PMC_BOOT_0  0x00000003
...
#define GPU_CLK_BASE    100
#define GPU_CLK_BOOST   100
#define AGP_CAP_OFFSET  0x40

#define TYPE_NVIDIA_NV3T_RIVA128ZX "nvidia-nv3t-riva128zx"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaNv3tRiva128zxState, NVIDIA_NV3T_RIVA128ZX)
struct NvidiaNv3tRiva128zxState {
...
static uint32_t gpu_clk(NvidiaNv3tRiva128zxState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaNv3tRiva128zxState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
    ...
    case ATI_MEM_SIZE:     return (uint32_t)(8 * 1024 * 1024);
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaNv3tRiva128zxState *s=(NvidiaNv3tRiva128zxState*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-nv3t-riva128zx-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b135ops,s,"nvidia-nv3t-riva128zx-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b135ops,s,"nvidia-nv3t-riva128zx-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b135ops,s,"nvidia-nv3t-riva128zx-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_nvidia_nv3t_riva128zx[]={DEFINE_PROP_UINT32("gpu-count",NvidiaNv3tRiva128zxState,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",NvidiaNv3tRiva128zxState,gpu_name),
    DEFINE_PROP_STRING("board-partner",NvidiaNv3tRiva128zxState,board_partner)};
static const VMStateDescription vms_nvidia_nv3t_riva128zx={.name="nvidia-nv3t-riva128zx",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaNv3tRiva128zxState),VMSTATE_UINT32(intr_en,NvidiaNv3tRiva128zxState),VMSTATE_UINT32(clock_mhz,NvidiaNv3tRiva128zxState),VMSTATE_UINT64(clock_last_ns,NvidiaNv3tRiva128zxState),
        VMSTATE_UINT32(gpu_count,NvidiaNv3tRiva128zxState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x0019;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA RIVA 128 ZX (NV3T, AGP 2x, 8MB, 1998)"; dc->vmsd=&vms_nvidia_nv3t_riva128zx; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_nvidia_nv3t_riva128zx);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_nv3t_riva128zx={.name=TYPE_NVIDIA_NV3T_RIVA128ZX,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaNv3tRiva128zxState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_nv3t_riva128zx(void){type_register_static(&ti_nvidia_nv3t_riva128zx);}
type_init(reg_nvidia_nv3t_riva128zx)
