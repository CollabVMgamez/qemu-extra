/*
 * Intel UHD Graphics (Alder Lake-N GT1, 32EU)
 * Vendor: 0x8086, Device: 0x46D0
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-graphics-adl-n
...
#define GPU_DEV      0x46D0
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x22D0
#define GPU_REV      0x0C
...
#define TYPE_INTELUHDGRAPHICSADLN "intel-uhd-graphics-adl-n"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhdGraphicsAdlNState, INTELUHDGRAPHICSADLN)
struct IntelUhdGraphicsAdlNState {
...
static uint32_t igpu_clk(IntelUhdGraphicsAdlNState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    IntelUhdGraphicsAdlNState *s = opaque;
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhdGraphicsAdlNState *s = INTELUHDGRAPHICSADLN(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-graphics-adl-n-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-graphics-adl-n-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhdGraphicsAdlNState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-graphics-adl-n",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhdGraphicsAdlNState),VMSTATE_UINT32(clock_mhz,IntelUhdGraphicsAdlNState),VMSTATE_UINT64(clock_last_ns,IntelUhdGraphicsAdlNState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics (Alder Lake-N GT1, 32EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHDGRAPHICSADLN,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhdGraphicsAdlNState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
