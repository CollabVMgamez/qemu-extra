/*
 * Intel HD Graphics 630 (Kaby Lake GT2, 24EU)
 * Vendor: 0x8086, Device: 0x5912
 * Class: 0x0300, PCIe
 * Usage: -device intel-hd-630-kbl
...
#define TYPE_INTELHD630KBL "intel-hd-630-kbl"
OBJECT_DECLARE_SIMPLE_TYPE(IntelHd630KblState, INTELHD630KBL)
struct IntelHd630KblState {
...
static uint32_t igpu_clk(IntelHd630KblState *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelHd630KblState *s = INTELHD630KBL(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-hd-630-kbl-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-hd-630-kbl-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelHd630KblState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-hd-630-kbl",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelHd630KblState),VMSTATE_UINT32(clock_mhz,IntelHd630KblState),VMSTATE_UINT64(clock_last_ns,IntelHd630KblState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel HD Graphics 630 (Kaby Lake GT2, 24EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELHD630KBL,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelHd630KblState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
