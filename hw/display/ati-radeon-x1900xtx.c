/*
 * ATI Radeon X1900 XTX (R580, PCIe x16, 512MB GDDR3, 2006)
...
#define TYPE_ATI_RADEON_X1900XTX "ati-radeon-x1900xtx"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeonX1900xtxState, ATI_RADEON_X1900XTX)
struct AtiRadeonX1900xtxState {
...
static uint32_t gpu_clk(AtiRadeonX1900xtxState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeonX1900xtxState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeonX1900xtxState *s=(AtiRadeonX1900xtxState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-x1900xtx-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-x1900xtx-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-x1900xtx-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-x1900xtx-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_x1900xtx={.name="ati-radeon-x1900xtx",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeonX1900xtxState),VMSTATE_UINT32(intr_en,AtiRadeonX1900xtxState),VMSTATE_UINT32(clock_mhz,AtiRadeonX1900xtxState),VMSTATE_UINT64(clock_last_ns,AtiRadeonX1900xtxState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeonX1900xtxState[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeonX1900xtxState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeonX1900xtxState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeonX1900xtxState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="ATI Radeon X1900 XTX (R580, PCIe x16, 512MB GDDR3, 2006)"; dc->vmsd=&vms_ati_radeon_x1900xtx; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRadeonX1900xtxState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_radeon_x1900xtx={.name=TYPE_ATI_RADEON_X1900XTX,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRadeonX1900xtxState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_radeon_x1900xtx(void){type_register_static(&ti_ati_radeon_x1900xtx);}
type_init(reg_ati_radeon_x1900xtx)
