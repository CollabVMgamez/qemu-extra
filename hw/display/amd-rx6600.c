/*
 * AMD Radeon RX 6600 (Navi23, RDNA 2.0)
...
#define GPU_DEVICE_ID  0x73FF
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x6600
...
#define GPU_CLK_BASE    1626
#define GPU_MEM_MHZ     1750
#define GPU_CLK_BOOST   2491
#define GPU_PCIE_GEN    4
#define TYPE_AMD_RX6600 "amd-rx6600"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRx6600State, AMD_RX6600)
struct AmdRx6600State {
...
static uint32_t gpu_clk(AmdRx6600State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRx6600State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRx6600State *s=(AmdRx6600State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-rx6600-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-rx6600-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-rx6600-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-rx6600-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_rx6600={.name="amd-rx6600",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRx6600State),VMSTATE_UINT32(intr_en,AmdRx6600State),VMSTATE_UINT32(pfifo_intr_en,AmdRx6600State),VMSTATE_UINT32(clock_mhz,AmdRx6600State),VMSTATE_UINT64(clock_last_ns,AmdRx6600State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AmdRx6600State[] = {
    DEFINE_PROP_STRING("gpu-name", AmdRx6600State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AmdRx6600State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AmdRx6600State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="AMD Radeon RX 6600 (Navi23, RDNA 2.0)"; dc->vmsd=&vms_amd_rx6600; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRx6600State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_rx6600={.name=TYPE_AMD_RX6600,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRx6600State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_rx6600(void){type_register_static(&ti_amd_rx6600);}
type_init(reg_amd_rx6600)
