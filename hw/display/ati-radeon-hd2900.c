/*
 * ATI Radeon HD 2900 XT (R600, PCIe x16, 512MB GDDR3, 2007)
...
#define GPU_DEVICE_ID  0x9400
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x9400
...
#define GPU_CLK_BASE    742
#define GPU_MEM_MHZ     742
#define GPU_CLK_BOOST   742
#define GPU_PCIE_GEN    2
#define GPU_VRAM_MB     512
#define TYPE_ATI_RADEON_HD2900 "ati-radeon-hd2900"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeonHd2900State, ATI_RADEON_HD2900)
struct AtiRadeonHd2900State {
...
static uint32_t gpu_clk(AtiRadeonHd2900State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeonHd2900State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeonHd2900State *s=(AtiRadeonHd2900State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-hd2900-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-hd2900-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-hd2900-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-hd2900-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_hd2900={.name="ati-radeon-hd2900",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeonHd2900State),VMSTATE_UINT32(intr_en,AtiRadeonHd2900State),VMSTATE_UINT32(clock_mhz,AtiRadeonHd2900State),VMSTATE_UINT64(clock_last_ns,AtiRadeonHd2900State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeonHd2900State[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeonHd2900State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeonHd2900State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeonHd2900State, board_partner),
};
...
static void reg_ati_radeon_hd2900(void){type_register_static(&ti_ati_radeon_hd2900);}
type_init(reg_ati_radeon_hd2900)
