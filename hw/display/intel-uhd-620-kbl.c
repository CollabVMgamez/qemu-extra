/*
 * Intel UHD Graphics 620 (Kaby Lake R ULT GT2, 24EU)
 * Vendor: 0x8086, Device: 0x5917
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-620-kbl
...
#define GPU_DEV      0x5917
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x2217
#define GPU_REV      0x02
...
#define TYPE_INTELUHD620KBL "intel-uhd-620-kbl"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhd620KblState, INTELUHD620KBL)
struct IntelUhd620KblState {
...
static uint32_t igpu_clk(IntelUhd620KblState *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhd620KblState *s = INTELUHD620KBL(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-620-kbl-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-620-kbl-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhd620KblState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-620-kbl",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhd620KblState),VMSTATE_UINT32(clock_mhz,IntelUhd620KblState),VMSTATE_UINT64(clock_last_ns,IntelUhd620KblState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics 620 (Kaby Lake R ULT GT2, 24EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHD620KBL,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhd620KblState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
