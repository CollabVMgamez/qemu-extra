/*
 * NVIDIA L4 (AD104, Ada Lovelace, 24GB GDDR6)
...
#define GPU_DEVICE_ID  0x27B8
#define GPU_SUBSYS_VID 0x10DE
#define GPU_SUBSYS_DID 0x16CA
...
#define GPU_CLK_BASE    2040
#define GPU_MEM_MHZ     1500
#define GPU_CLK_BOOST   2040
#define GPU_PCIE_GEN    4
#define TYPE_NVIDIA_L4 "nvidia-l4"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaL4State, NVIDIA_L4)
struct NvidiaL4State {
...
static uint32_t gpu_clk(NvidiaL4State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaL4State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaL4State *s=(NvidiaL4State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-l4-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-l4-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-l4-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-l4-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_l4={.name="nvidia-l4",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaL4State),VMSTATE_UINT32(intr_en,NvidiaL4State),VMSTATE_UINT32(pfifo_intr_en,NvidiaL4State),VMSTATE_UINT32(clock_mhz,NvidiaL4State),VMSTATE_UINT64(clock_last_ns,NvidiaL4State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaL4State[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaL4State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaL4State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaL4State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x27B8;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="NVIDIA L4 (AD104, Ada Lovelace, 24GB GDDR6)"; dc->vmsd=&vms_nvidia_l4; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaL4State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_l4={.name=TYPE_NVIDIA_L4,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaL4State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_l4(void){type_register_static(&ti_nvidia_l4);}
type_init(reg_nvidia_l4)
