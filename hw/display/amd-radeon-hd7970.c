/*
 * AMD Radeon HD 7970 (Tahiti, PCIe x16, 3GB GDDR5, 2011)
...
#define TYPE_AMD_RADEON_HD7970 "amd-radeon-hd7970"
OBJECT_DECLARE_SIMPLE_TYPE(AmdRadeonHd7970State, AMD_RADEON_HD7970)
struct AmdRadeonHd7970State {
...
static uint32_t gpu_clk(AmdRadeonHd7970State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AmdRadeonHd7970State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AmdRadeonHd7970State *s=(AmdRadeonHd7970State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"amd-radeon-hd7970-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"amd-radeon-hd7970-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"amd-radeon-hd7970-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"amd-radeon-hd7970-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_amd_radeon_hd7970={.name="amd-radeon-hd7970",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AmdRadeonHd7970State),VMSTATE_UINT32(intr_en,AmdRadeonHd7970State),VMSTATE_UINT32(clock_mhz,AmdRadeonHd7970State),VMSTATE_UINT64(clock_last_ns,AmdRadeonHd7970State),VMSTATE_END_OF_LIST()}};
...
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="AMD Radeon HD 7970 (Tahiti, PCIe x16, 3GB GDDR5, 2011)"; dc->vmsd=&vms_amd_radeon_hd7970; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AmdRadeonHd7970State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_amd_radeon_hd7970={.name=TYPE_AMD_RADEON_HD7970,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AmdRadeonHd7970State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_amd_radeon_hd7970(void){type_register_static(&ti_amd_radeon_hd7970);}
type_init(reg_amd_radeon_hd7970)
