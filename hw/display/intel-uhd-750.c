/*
 * Intel UHD Graphics 750 (Rocket Lake GT1, 32EU)
 * Vendor: 0x8086, Device: 0x4C8A
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-750
...
#define TYPE_INTELUHD750 "intel-uhd-750"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhd750State, INTELUHD750)
struct IntelUhd750State {
...
static uint32_t igpu_clk(IntelUhd750State *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhd750State *s = INTELUHD750(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-750-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-750-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhd750State, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-750",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhd750State),VMSTATE_UINT32(clock_mhz,IntelUhd750State),VMSTATE_UINT64(clock_last_ns,IntelUhd750State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics 750 (Rocket Lake GT1, 32EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHD750,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhd750State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
