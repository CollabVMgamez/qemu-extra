/*
 * ATI Radeon HD 4650 (RV730, PCIe, 512MB DDR2, 2008)
...
#define TYPE_ATI_RADEON_HD4650 "ati-radeon-hd4650"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeonHd4650State, ATI_RADEON_HD4650)
struct AtiRadeonHd4650State {
...
static uint32_t gpu_clk(AtiRadeonHd4650State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeonHd4650State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeonHd4650State *s=(AtiRadeonHd4650State*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-hd4650-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-hd4650-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-hd4650-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-hd4650-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_hd4650={.name="ati-radeon-hd4650",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeonHd4650State),VMSTATE_UINT32(intr_en,AtiRadeonHd4650State),VMSTATE_UINT32(clock_mhz,AtiRadeonHd4650State),VMSTATE_UINT64(clock_last_ns,AtiRadeonHd4650State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeonHd4650State[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeonHd4650State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeonHd4650State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeonHd4650State, board_partner),
};
...
static void reg_ati_radeon_hd4650(void){type_register_static(&ti_ati_radeon_hd4650);}
type_init(reg_ati_radeon_hd4650)
