/*
 * MediaTek Immortalis-G720 (Dimensity 9300, integrated)
...
#define TYPE_IMMORTALIS_G720 "immortalis-g720"
OBJECT_DECLARE_SIMPLE_TYPE(ImmortalisG720State, IMMORTALIS_G720)
struct ImmortalisG720State {
...
static uint32_t gpu_clk(ImmortalisG720State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    ImmortalisG720State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    ImmortalisG720State *s=(ImmortalisG720State*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"immortalis-g720-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&bops,s,"immortalis-g720-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&bops,s,"immortalis-g720-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&bops,s,"immortalis-g720-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_immortalis_g720[]={DEFINE_PROP_UINT32("gpu-count",ImmortalisG720State,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",ImmortalisG720State,gpu_name)};
static const VMStateDescription vms_immortalis_g720={.name="immortalis-g720",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,ImmortalisG720State),VMSTATE_UINT32(clock_mhz,ImmortalisG720State),VMSTATE_UINT64(clock_last_ns,ImmortalisG720State),VMSTATE_UINT32(gpu_count,ImmortalisG720State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0xD720;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="MediaTek Immortalis-G720 (Dimensity 9300, integrated)"; dc->vmsd=&vms_immortalis_g720; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_immortalis_g720);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_immortalis_g720={.name=TYPE_IMMORTALIS_G720,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(ImmortalisG720State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_immortalis_g720(void){type_register_static(&ti_immortalis_g720);}
type_init(reg_immortalis_g720)
