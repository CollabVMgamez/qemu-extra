/*
 * Qualcomm Adreno 630 (Snapdragon 845, integrated)
...
#define GPU_CLK_BASE    710
#define GPU_CLK_BOOST   710
...
#define TYPE_ADRENO_630 "adreno-630"
OBJECT_DECLARE_SIMPLE_TYPE(Adreno630State, ADRENO_630)
struct Adreno630State {
...
static uint32_t gpu_clk(Adreno630State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    Adreno630State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    Adreno630State *s=(Adreno630State*)p;
    ...
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"adreno-630-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&bops,s,"adreno-630-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&bops,s,"adreno-630-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&bops,s,"adreno-630-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const Property gpu_props_adreno_630[]={DEFINE_PROP_UINT32("gpu-count",Adreno630State,gpu_count,1),
    DEFINE_PROP_STRING("gpu-name",Adreno630State,gpu_name)};
static const VMStateDescription vms_adreno_630={.name="adreno-630",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,Adreno630State),VMSTATE_UINT32(clock_mhz,Adreno630State),VMSTATE_UINT64(clock_last_ns,Adreno630State),VMSTATE_UINT32(gpu_count,Adreno630State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x6030;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="Qualcomm Adreno 630 (Snapdragon 845, integrated)"; dc->vmsd=&vms_adreno_630; dc->hotpluggable=false;
    device_class_set_props(dc,gpu_props_adreno_630);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_adreno_630={.name=TYPE_ADRENO_630,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(Adreno630State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_adreno_630(void){type_register_static(&ti_adreno_630);}
type_init(reg_adreno_630)
