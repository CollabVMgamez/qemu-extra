/*
 * ATI Radeon 9500 Pro (R300, AGP 8x, 128MB DDR, 2002)
...
#define TYPE_ATI_RADEON_9500PRO "ati-radeon-9500pro"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeon9500proState, ATI_RADEON_9500PRO)
struct AtiRadeon9500proState {
...
static uint32_t gpu_clk(AtiRadeon9500proState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeon9500proState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeon9500proState *s=(AtiRadeon9500proState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-9500pro-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-9500pro-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-9500pro-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-9500pro-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_9500pro={.name="ati-radeon-9500pro",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeon9500proState),VMSTATE_UINT32(intr_en,AtiRadeon9500proState),VMSTATE_UINT32(clock_mhz,AtiRadeon9500proState),VMSTATE_UINT64(clock_last_ns,AtiRadeon9500proState),VMSTATE_END_OF_LIST()}};
...
static void reg_ati_radeon_9500pro(void){type_register_static(&ti_ati_radeon_9500pro);}
type_init(reg_ati_radeon_9500pro)
