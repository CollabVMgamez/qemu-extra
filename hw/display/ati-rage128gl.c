/*
 * ATI Rage 128 GL (AGP, 32MB SDR, 1998)
...
#define GPU_DEVICE_ID  0x5245
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x5245
...
#define GPU_CLK_BASE    83
#define GPU_MEM_MHZ     83
#define GPU_CLK_BOOST   83
#define GPU_PCIE_GEN    1
#define GPU_VRAM_MB     32
#define TYPE_ATI_RAGE128GL "ati-rage128gl"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRage128glState, ATI_RAGE128GL)
struct AtiRage128glState {
...
static uint32_t gpu_clk(AtiRage128glState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRage128glState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRage128glState *s=(AtiRage128glState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-rage128gl-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-rage128gl-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-rage128gl-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-rage128gl-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_rage128gl={.name="ati-rage128gl",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRage128glState),VMSTATE_UINT32(intr_en,AtiRage128glState),VMSTATE_UINT32(clock_mhz,AtiRage128glState),VMSTATE_UINT64(clock_last_ns,AtiRage128glState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRage128glState[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRage128glState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRage128glState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRage128glState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x5245;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="ATI Rage 128 GL (AGP, 32MB SDR, 1998)"; dc->vmsd=&vms_ati_rage128gl; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRage128glState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_rage128gl={.name=TYPE_ATI_RAGE128GL,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRage128glState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_rage128gl(void){type_register_static(&ti_ati_rage128gl);}
type_init(reg_ati_rage128gl)
