/*
 * NVIDIA H800 (GH100, Hopper, 80GB HBM3, China export)
...
#define GPU_PMC_BOOT_0  0x232000A1
...
#define GPU_CLK_BOOST   1600
#define GPU_PCIE_GEN    5
#define TYPE_NVIDIA_H800 "nvidia-h800"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaH800State, NVIDIA_H800)
struct NvidiaH800State {
...
static uint32_t gpu_clk(NvidiaH800State *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    NvidiaH800State *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    NvidiaH800State *s=(NvidiaH800State*)p; s->gpu_count=1;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"nvidia-h800-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"nvidia-h800-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"nvidia-h800-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"nvidia-h800-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vms_nvidia_h800={.name="nvidia-h800",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,NvidiaH800State),VMSTATE_UINT32(intr_en,NvidiaH800State),VMSTATE_UINT32(pfifo_intr_en,NvidiaH800State),VMSTATE_UINT32(clock_mhz,NvidiaH800State),VMSTATE_UINT64(clock_last_ns,NvidiaH800State),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_NvidiaH800State[] = {
    DEFINE_PROP_STRING("gpu-name", NvidiaH800State, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", NvidiaH800State, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", NvidiaH800State, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="NVIDIA H800 (GH100, Hopper, 80GB HBM3, China export)"; dc->vmsd=&vms_nvidia_h800; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_NvidiaH800State);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_nvidia_h800={.name=TYPE_NVIDIA_H800,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(NvidiaH800State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_nvidia_h800(void){type_register_static(&ti_nvidia_h800);}
type_init(reg_nvidia_h800)
