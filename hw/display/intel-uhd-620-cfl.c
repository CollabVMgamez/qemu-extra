/*
 * Intel UHD Graphics 620 (Coffee Lake ULT GT2, 24EU)
 * Vendor: 0x8086, Device: 0x3EA0
 * Class: 0x0300, PCIe
 * Usage: -device intel-uhd-620-cfl
...
#define GPU_DEV      0x3EA0
#define GPU_SUBSYS_V 0x8086
#define GPU_SUBSYS_D 0x22A0
#define GPU_REV      0x00
...
#define TYPE_INTELUHD620CFL "intel-uhd-620-cfl"
OBJECT_DECLARE_SIMPLE_TYPE(IntelUhd620CflState, INTELUHD620CFL)
struct IntelUhd620CflState {
...
static uint32_t igpu_clk(IntelUhd620CflState *s) {
...
static void igpu_realize(PCIDevice *pdev, Error **errp) {
    IntelUhd620CflState *s = INTELUHD620CFL(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "intel-uhd-620-cfl-mmio", BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_64, &s->bar0);
    memory_region_init_io(&s->bar2, OBJECT(s), &bar2_ops, s, "intel-uhd-620-cfl-gtt", BAR2_SIZE);
...
static const Property igpu_props[] = {
    DEFINE_PROP_STRING("gpu-name", IntelUhd620CflState, gpu_name),
};
static const VMStateDescription vms = {
    .name="intel-uhd-620-cfl",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,IntelUhd620CflState),VMSTATE_UINT32(clock_mhz,IntelUhd620CflState),VMSTATE_UINT64(clock_last_ns,IntelUhd620CflState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Intel UHD Graphics 620 (Coffee Lake ULT GT2, 24EU)"; dc->vmsd=&vms; dc->hotpluggable=false;
    device_class_set_props(dc, igpu_props);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti={.name=TYPE_INTELUHD620CFL,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(IntelUhd620CflState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
