/*
 * NVIDIA GeForce GTX 460 (GF104, Fermi)
...
#define GPU_DEVICE_ID    0x0E22
#define GPU_SUBSYS_VID   0x10DE
#define GPU_SUBSYS_DID   0x0842
...
#define GPU_CLK_BASE     675
#define GPU_MEM_MHZ     900
#define GPU_CLK_BOOST    800
#define GPU_PCIE_GEN     2

#define TYPE_NVIDIA_GTX460 "nvidia-gtx460"
OBJECT_DECLARE_SIMPLE_TYPE(NvidiaGtx460State, NVIDIA_GTX460)

struct NvidiaGtx460State {
...
static uint32_t gpu_get_clock(NvidiaGtx460State *s)
{
...
{
    NvidiaGtx460State *s = (NvidiaGtx460State *)pdev; s->gpu_count=1;
    ...
    gpu_setup_caps(pdev);
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops,  s, "nvidia-gtx460-mmio", NV_BAR0_SIZE);
    pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar0);
    memory_region_init_io(&s->bar1, OBJECT(s), &bar1_ops,  s, "nvidia-gtx460-vram", NV_BAR1_SIZE);
    pci_register_bar(pdev, 1, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH, &s->bar1);
    memory_region_init_io(&s->bar3, OBJECT(s), &bar35_ops, s, "nvidia-gtx460-ramin",NV_BAR3_SIZE);
    pci_register_bar(pdev, 3, PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32, &s->bar3);
    memory_region_init_io(&s->bar5, OBJECT(s), &bar35_ops, s, "nvidia-gtx460-vgaio",NV_BAR5_SIZE);
...
static const VMStateDescription vmstate_nvidia_gtx460 = {
    .name = "nvidia-gtx460", .version_id = 1, .minimum_version_id = 1,
    .fields = (const VMStateField[]) {
        VMSTATE_PCI_DEVICE(parent_obj, NvidiaGtx460State),
        VMSTATE_UINT32(intr_en,       NvidiaGtx460State),
        VMSTATE_UINT32(pfifo_intr_en, NvidiaGtx460State),
        VMSTATE_UINT32(clock_mhz,     NvidiaGtx460State),
        VMSTATE_UINT32(gpu_count,       NvidiaGtx460State             ),
        VMSTATE_UINT64(clock_last_ns, NvidiaGtx460State),
        VMSTATE_END_OF_LIST()
    },
};

static const Property gpu_multi_props_NvidiaGtx460State[] = {
    DEFINE_PROP_UINT32("gpu-count", NvidiaGtx460State, gpu_count, 1),
};
static void gpu_class_init(ObjectClass *klass, const void *data)
{
    ...
    dc->desc         = "NVIDIA GeForce GTX 460 (GF104, Fermi)";
    dc->vmsd         = &vmstate_nvidia_gtx460;
    dc->hotpluggable = false;
    device_class_set_props(dc, gpu_multi_props_NvidiaGtx460State);
    set_bit(DEVICE_CATEGORY_DISPLAY, dc->categories);
}

static const TypeInfo nvidia_gtx460_info = {
    .name          = TYPE_NVIDIA_GTX460,
    .parent        = TYPE_PCI_DEVICE,
    .instance_size = sizeof(NvidiaGtx460State),
    .class_init    = gpu_class_init,
    .interfaces    = (InterfaceInfo[]) {
        { INTERFACE_CONVENTIONAL_PCI_DEVICE }, { },
    },
};
static void nvidia_gtx460_register(void) { type_register_static(&nvidia_gtx460_info); }
type_init(nvidia_gtx460_register)
