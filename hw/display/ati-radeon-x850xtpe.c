/*
 * ATI Radeon X850 XT Platinum (R480, AGP 8x, 256MB GDDR3, 2005)
...
#define GPU_DEVICE_ID  0x4B4A
#define GPU_SUBSYS_VID 0x1002
#define GPU_SUBSYS_DID 0x4B4A
...
#define GPU_CLK_BASE    540
#define GPU_MEM_MHZ     540
#define GPU_CLK_BOOST   540
#define GPU_PCIE_GEN    1
#define GPU_VRAM_MB     256
#define TYPE_ATI_RADEON_X850XTPE "ati-radeon-x850xtpe"
OBJECT_DECLARE_SIMPLE_TYPE(AtiRadeonX850xtpeState, ATI_RADEON_X850XTPE)
struct AtiRadeonX850xtpeState {
...
static uint32_t gpu_clk(AtiRadeonX850xtpeState *s) {
...
static uint64_t bar0_r(void *opaque, hwaddr addr, unsigned size) {
    AtiRadeonX850xtpeState *s = opaque; uint32_t m; uint64_t n;
    switch (addr & ~3ULL) {
...
static void gpu_realize(PCIDevice *p, Error **e) {
    AtiRadeonX850xtpeState *s=(AtiRadeonX850xtpeState*)p;
    ...
    gpu_caps(p);
    memory_region_init_io(&s->bar0,OBJECT(s),&b0ops,s,"ati-radeon-x850xtpe-mmio",NV_BAR0_SIZE);
    pci_register_bar(p,0,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar0);
    memory_region_init_io(&s->bar1,OBJECT(s),&b1ops,s,"ati-radeon-x850xtpe-vram",NV_BAR1_SIZE);
    pci_register_bar(p,1,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32|PCI_BASE_ADDRESS_MEM_PREFETCH,&s->bar1);
    memory_region_init_io(&s->bar3,OBJECT(s),&b35ops,s,"ati-radeon-x850xtpe-ramin",NV_BAR3_SIZE);
    pci_register_bar(p,3,PCI_BASE_ADDRESS_SPACE_MEMORY|PCI_BASE_ADDRESS_MEM_TYPE_32,&s->bar3);
    memory_region_init_io(&s->bar5,OBJECT(s),&b35ops,s,"ati-radeon-x850xtpe-vgaio",NV_BAR5_SIZE);
    pci_register_bar(p,5,PCI_BASE_ADDRESS_SPACE_MEMORY,&s->bar5);
}
static const VMStateDescription vms_ati_radeon_x850xtpe={.name="ati-radeon-x850xtpe",.version_id=1,.minimum_version_id=1,.fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AtiRadeonX850xtpeState),VMSTATE_UINT32(intr_en,AtiRadeonX850xtpeState),VMSTATE_UINT32(clock_mhz,AtiRadeonX850xtpeState),VMSTATE_UINT64(clock_last_ns,AtiRadeonX850xtpeState),VMSTATE_END_OF_LIST()}};
static const Property gpu_multi_props_AtiRadeonX850xtpeState[] = {
    DEFINE_PROP_STRING("gpu-name", AtiRadeonX850xtpeState, gpu_name),
    DEFINE_PROP_UINT32("gpu-count", AtiRadeonX850xtpeState, gpu_count, 1),
    DEFINE_PROP_STRING("board-partner", AtiRadeonX850xtpeState, board_partner),
};
static void ci(ObjectClass *k, const void *d) {
    DeviceClass *dc=DEVICE_CLASS(k); PCIDeviceClass *pc=PCI_DEVICE_CLASS(k);
    pc->realize=gpu_realize; pc->vendor_id=GPU_VENDOR_ID; pc->device_id=0x4B4A;
    pc->class_id=GPU_CLASS; pc->revision=GPU_REVISION;
    dc->desc="ATI Radeon X850 XT Platinum (R480, AGP 8x, 256MB GDDR3, 2005)"; dc->vmsd=&vms_ati_radeon_x850xtpe; dc->hotpluggable=false; device_class_set_props(dc,gpu_multi_props_AtiRadeonX850xtpeState);
    set_bit(DEVICE_CATEGORY_DISPLAY,dc->categories);
}
static const TypeInfo ti_ati_radeon_x850xtpe={.name=TYPE_ATI_RADEON_X850XTPE,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AtiRadeonX850xtpeState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
static void reg_ati_radeon_x850xtpe(void){type_register_static(&ti_ati_radeon_x850xtpe);}
type_init(reg_ati_radeon_x850xtpe)
