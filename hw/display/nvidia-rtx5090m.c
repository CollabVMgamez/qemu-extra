...
static void realize(PCIDevice*p,Error**e){ NvidiaRtx5090mState*s=(NvidiaRtx5090mState*)p; s->clock_mhz=CLK_BASE;s->clock_last_ns=0;s->gpu_count=1; p->config[PCI_CLASS_PROG]=0;pci_set_word(p->config+PCI_SUBSYSTEM_VENDOR_ID,GPU_SS_V);
    if (s->board_partner) {
