/*
 * Qualcomm Hexagon DSP v69 (QCS8250, AI Accelerator)
 * Vendor: 0x17CB, Device: 0x0306
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device qualcomm-hexagon-dsp-690
...
#define NPU_DEV    0x0306
#define NPU_SUBSYS_V 0x17CB
#define NPU_SUBSYS_D 0x0306
#define NPU_REV    0x01
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_QUALCOMMHEXAGONDSP690 "qualcomm-hexagon-dsp-690"
OBJECT_DECLARE_SIMPLE_TYPE(QualcommHexagonDsp690State, QUALCOMMHEXAGONDSP690)
struct QualcommHexagonDsp690State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    QualcommHexagonDsp690State *s = QUALCOMMHEXAGONDSP690(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "qualcomm-hexagon-dsp-690-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="qualcomm-hexagon-dsp-690",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,QualcommHexagonDsp690State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Qualcomm Hexagon DSP v69 (QCS8250, AI Accelerator)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_QUALCOMMHEXAGONDSP690,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(QualcommHexagonDsp690State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
