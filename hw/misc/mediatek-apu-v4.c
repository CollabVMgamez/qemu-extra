/*
 * MediaTek APU v4 (Dimensity 9000/9200, 7 TOPS INT8)
 * Vendor: 0x14C3, Device: 0x7925
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device mediatek-apu-v4
...
#define NPU_DEV    0x7925
#define NPU_SUBSYS_V 0x14C3
#define NPU_SUBSYS_D 0x7925
#define NPU_REV    0x00
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_MEDIATEKAPUV4 "mediatek-apu-v4"
OBJECT_DECLARE_SIMPLE_TYPE(MediatekApuV4State, MEDIATEKAPUV4)
struct MediatekApuV4State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    MediatekApuV4State *s = MEDIATEKAPUV4(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "mediatek-apu-v4-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="mediatek-apu-v4",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,MediatekApuV4State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="MediaTek APU v4 (Dimensity 9000/9200, 7 TOPS INT8)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_MEDIATEKAPUV4,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(MediatekApuV4State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
