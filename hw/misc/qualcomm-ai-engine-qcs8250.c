/*
 * Qualcomm AI Engine (QCS8250, 15 TOPS INT8)
 * Vendor: 0x17CB, Device: 0x0310
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device qualcomm-ai-engine-qcs8250
...
#define NPU_DEV    0x0310
#define NPU_SUBSYS_V 0x17CB
#define NPU_SUBSYS_D 0x0310
#define NPU_REV    0x01
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_QUALCOMMAIENGINEQCS8250 "qualcomm-ai-engine-qcs8250"
OBJECT_DECLARE_SIMPLE_TYPE(QualcommAiEngineQcs8250State, QUALCOMMAIENGINEQCS8250)
struct QualcommAiEngineQcs8250State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    QualcommAiEngineQcs8250State *s = QUALCOMMAIENGINEQCS8250(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "qualcomm-ai-engine-qcs8250-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="qualcomm-ai-engine-qcs8250",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,QualcommAiEngineQcs8250State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Qualcomm AI Engine (QCS8250, 15 TOPS INT8)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_QUALCOMMAIENGINEQCS8250,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(QualcommAiEngineQcs8250State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
