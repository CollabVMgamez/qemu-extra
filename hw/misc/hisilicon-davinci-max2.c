/*
 * HiSilicon Da Vinci Max2 (Ascend 910B, 320 TOPS FP16)
 * Vendor: 0x19E5, Device: 0xD103
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device hisilicon-davinci-max2
...
#define NPU_DEV    0xD103
#define NPU_SUBSYS_V 0x19E5
#define NPU_SUBSYS_D 0xD103
#define NPU_REV    0x00
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_HISILICONDAVINCIMAX2 "hisilicon-davinci-max2"
OBJECT_DECLARE_SIMPLE_TYPE(HisiliconDavinciMax2State, HISILICONDAVINCIMAX2)
struct HisiliconDavinciMax2State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    HisiliconDavinciMax2State *s = HISILICONDAVINCIMAX2(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "hisilicon-davinci-max2-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="hisilicon-davinci-max2",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,HisiliconDavinciMax2State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="HiSilicon Da Vinci Max2 (Ascend 910B, 320 TOPS FP16)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_HISILICONDAVINCIMAX2,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(HisiliconDavinciMax2State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
