/*
 * AWS Inferentia2 (Inf2) ML Inference Accelerator stub
 * Vendor: 0x1D0F (Amazon.com, Inc.), Device: 0xF011
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device aws-inferentia2
...
#define NPU_DEV    0xF011
#define NPU_SUBSYS_V 0x1D0F
#define NPU_SUBSYS_D 0xF011
#define NPU_REV    0x01
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_AWS_INFERENTIA2 "aws-inferentia2"
OBJECT_DECLARE_SIMPLE_TYPE(AwsInferentia2State, AWS_INFERENTIA2)
struct AwsInferentia2State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    AwsInferentia2State *s = AWS_INFERENTIA2(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "aws-inferentia2-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="aws-inferentia2",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AwsInferentia2State),VMSTATE_END_OF_LIST()}};
