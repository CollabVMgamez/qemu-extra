/*
 * Google TPU v5 (Cloud TPU, 398 TOPS BF16)
 * Vendor: 0x1AE0, Device: 0xC050
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device google-tpu-v5
...
#define NPU_DEV    0xC050
#define NPU_SUBSYS_V 0x1AE0
#define NPU_SUBSYS_D 0xC050
#define NPU_REV    0x01
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_GOOGLETPUV5 "google-tpu-v5"
OBJECT_DECLARE_SIMPLE_TYPE(GoogleTpuV5State, GOOGLETPUV5)
struct GoogleTpuV5State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    GoogleTpuV5State *s = GOOGLETPUV5(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "google-tpu-v5-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="google-tpu-v5",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,GoogleTpuV5State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Google TPU v5 (Cloud TPU, 398 TOPS BF16)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_GOOGLETPUV5,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(GoogleTpuV5State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
