/*
 * HiSilicon Ascend 310B (Edge AI, 17 TOPS FP16)
 * Vendor: 0x19E5, Device: 0xD110
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device hisilicon-ascend-310b
...
#define NPU_DEV    0xD110
#define NPU_SUBSYS_V 0x19E5
#define NPU_SUBSYS_D 0xD110
#define NPU_REV    0x00
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_HISILICONASCEND310B "hisilicon-ascend-310b"
OBJECT_DECLARE_SIMPLE_TYPE(HisiliconAscend310bState, HISILICONASCEND310B)
struct HisiliconAscend310bState {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    HisiliconAscend310bState *s = HISILICONASCEND310B(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "hisilicon-ascend-310b-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="hisilicon-ascend-310b",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,HisiliconAscend310bState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="HiSilicon Ascend 310B (Edge AI, 17 TOPS FP16)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_HISILICONASCEND310B,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(HisiliconAscend310bState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
