/*
 * HiSilicon Da Vinci Lite (Ascend 310P, 22 TOPS FP16)
 * Vendor: 0x19E5, Device: 0xD101
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device hisilicon-davinci-lite
...
#define NPU_DEV    0xD101
#define NPU_SUBSYS_V 0x19E5
#define NPU_SUBSYS_D 0xD101
#define NPU_REV    0x00
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_HISILICONDAVINCILITE "hisilicon-davinci-lite"
OBJECT_DECLARE_SIMPLE_TYPE(HisiliconDavinciLiteState, HISILICONDAVINCILITE)
struct HisiliconDavinciLiteState {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    HisiliconDavinciLiteState *s = HISILICONDAVINCILITE(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "hisilicon-davinci-lite-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="hisilicon-davinci-lite",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,HisiliconDavinciLiteState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="HiSilicon Da Vinci Lite (Ascend 310P, 22 TOPS FP16)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_HISILICONDAVINCILITE,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(HisiliconDavinciLiteState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
