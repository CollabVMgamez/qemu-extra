/*
 * Apple Neural Engine (M3 Max, 18 TOPS)
 * Vendor: 0x106B, Device: 0x180C
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device apple-neural-engine-m3-max
...
#define NPU_DEV    0x180C
#define NPU_SUBSYS_V 0x106B
#define NPU_SUBSYS_D 0x180C
#define NPU_REV    0x00
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_APPLENEURALENGINEM3MAX "apple-neural-engine-m3-max"
OBJECT_DECLARE_SIMPLE_TYPE(AppleNeuralEngineM3MaxState, APPLENEURALENGINEM3MAX)
struct AppleNeuralEngineM3MaxState {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    AppleNeuralEngineM3MaxState *s = APPLENEURALENGINEM3MAX(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "apple-neural-engine-m3-max-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="apple-neural-engine-m3-max",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AppleNeuralEngineM3MaxState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Apple Neural Engine (M3 Max, 18 TOPS)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_APPLENEURALENGINEM3MAX,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AppleNeuralEngineM3MaxState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
