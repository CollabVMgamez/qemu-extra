/*
 * HiSilicon Da Vinci Max (Ascend 910, 256 TOPS FP16)
 * Vendor: 0x19E5, Device: 0xD102
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device hisilicon-davinci-max
...
#define NPU_DEV    0xD102
#define NPU_SUBSYS_V 0x19E5
#define NPU_SUBSYS_D 0xD102
#define NPU_REV    0x00
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_HISILICONDAVINCIMAX "hisilicon-davinci-max"
OBJECT_DECLARE_SIMPLE_TYPE(HisiliconDavinciMaxState, HISILICONDAVINCIMAX)
struct HisiliconDavinciMaxState {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    HisiliconDavinciMaxState *s = HISILICONDAVINCIMAX(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "hisilicon-davinci-max-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="hisilicon-davinci-max",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,HisiliconDavinciMaxState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="HiSilicon Da Vinci Max (Ascend 910, 256 TOPS FP16)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_HISILICONDAVINCIMAX,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(HisiliconDavinciMaxState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
