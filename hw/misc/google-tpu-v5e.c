/*
 * Google TPU v5e (Cloud TPU Lite, 197 TOPS BF16)
 * Vendor: 0x1AE0, Device: 0xC052
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device google-tpu-v5e
...
#define NPU_DEV    0xC052
#define NPU_SUBSYS_V 0x1AE0
#define NPU_SUBSYS_D 0xC052
#define NPU_REV    0x01
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_GOOGLETPUV5E "google-tpu-v5e"
OBJECT_DECLARE_SIMPLE_TYPE(GoogleTpuV5eState, GOOGLETPUV5E)
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    GoogleTpuV5eState *s = GOOGLETPUV5E(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "google-tpu-v5e-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    ...
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,GoogleTpuV5eState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Google TPU v5e (Cloud TPU Lite, 197 TOPS BF16)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_GOOGLETPUV5E,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(GoogleTpuV5eState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
