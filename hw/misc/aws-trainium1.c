/*
 * AWS Trainium1 (Trn1) ML Training Accelerator stub
 * Vendor: 0x1D0F (Amazon.com, Inc.), Device: 0xF020
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device aws-trainium1
...
#define NPU_DEV    0xF020
#define NPU_SUBSYS_V 0x1D0F
#define NPU_SUBSYS_D 0xF020
#define NPU_REV    0x01
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_AWS_TRAINIUM1 "aws-trainium1"
OBJECT_DECLARE_SIMPLE_TYPE(AwsTrainium1State, AWS_TRAINIUM1)
struct AwsTrainium1State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    AwsTrainium1State *s = AWS_TRAINIUM1(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "aws-trainium1-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="aws-trainium1",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AwsTrainium1State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="AWS Trainium1 (Trn1) ML Training Accelerator"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_AWS_TRAINIUM1,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AwsTrainium1State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
