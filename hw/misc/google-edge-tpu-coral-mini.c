/*
 * Google Edge TPU (Coral USB Mini Accelerator, 4 TOPS INT8)
 * Vendor: 0x1AE0, Device: 0xC022
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device google-edge-tpu-coral-mini
...
#define NPU_DEV    0xC022
#define NPU_SUBSYS_V 0x1AE0
#define NPU_SUBSYS_D 0xC022
#define NPU_REV    0x01
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_GOOGLEEDGETPUCORALMINI "google-edge-tpu-coral-mini"
OBJECT_DECLARE_SIMPLE_TYPE(GoogleEdgeTpuCoralMiniState, GOOGLEEDGETPUCORALMINI)
struct GoogleEdgeTpuCoralMiniState {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    GoogleEdgeTpuCoralMiniState *s = GOOGLEEDGETPUCORALMINI(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "google-edge-tpu-coral-mini-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="google-edge-tpu-coral-mini",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,GoogleEdgeTpuCoralMiniState),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Google Edge TPU (Coral USB Mini Accelerator, 4 TOPS INT8)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_GOOGLEEDGETPUCORALMINI,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(GoogleEdgeTpuCoralMiniState),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
