/*
 * AWS Trainium2 (Trn2) ML Training Accelerator stub
 * Vendor: 0x1D0F (Amazon.com, Inc.), Device: 0xF021
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device aws-trainium2
...
#define NPU_DEV    0xF021
#define NPU_SUBSYS_V 0x1D0F
#define NPU_SUBSYS_D 0xF021
#define NPU_REV    0x01
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_AWS_TRAINIUM2 "aws-trainium2"
OBJECT_DECLARE_SIMPLE_TYPE(AwsTrainium2State, AWS_TRAINIUM2)
struct AwsTrainium2State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    AwsTrainium2State *s = AWS_TRAINIUM2(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "aws-trainium2-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="aws-trainium2",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AwsTrainium2State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="AWS Trainium2 (Trn2) ML Training Accelerator"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_AWS_TRAINIUM2,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AwsTrainium2State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
