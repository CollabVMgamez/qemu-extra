/*
 * Apple Neural Engine (M1, 11 TOPS)
 * Vendor: 0x106B, Device: 0x1802
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device apple-neural-engine-m1
...
#define NPU_DEV    0x1802
#define NPU_SUBSYS_V 0x106B
#define NPU_SUBSYS_D 0x1802
#define NPU_REV    0x00
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_APPLENEURALENGINEM1 "apple-neural-engine-m1"
OBJECT_DECLARE_SIMPLE_TYPE(AppleNeuralEngineM1State, APPLENEURALENGINEM1)
struct AppleNeuralEngineM1State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    AppleNeuralEngineM1State *s = APPLENEURALENGINEM1(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "apple-neural-engine-m1-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="apple-neural-engine-m1",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,AppleNeuralEngineM1State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Apple Neural Engine (M1, 11 TOPS)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_APPLENEURALENGINEM1,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(AppleNeuralEngineM1State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
