/*
 * Google Edge TPU (Coral M.2 Accelerator, 4 TOPS INT8)
 * Vendor: 0x1AE0, Device: 0xC021
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device google-edge-tpu-coral-m2
...
#define NPU_DEV    0xC021
#define NPU_SUBSYS_V 0x1AE0
#define NPU_SUBSYS_D 0xC021
#define NPU_REV    0x01
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_GOOGLEEDGETPUCORALM2 "google-edge-tpu-coral-m2"
OBJECT_DECLARE_SIMPLE_TYPE(GoogleEdgeTpuCoralM2State, GOOGLEEDGETPUCORALM2)
struct GoogleEdgeTpuCoralM2State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    GoogleEdgeTpuCoralM2State *s = GOOGLEEDGETPUCORALM2(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "google-edge-tpu-coral-m2-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="google-edge-tpu-coral-m2",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,GoogleEdgeTpuCoralM2State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Google Edge TPU (Coral M.2 Accelerator, 4 TOPS INT8)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_GOOGLEEDGETPUCORALM2,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(GoogleEdgeTpuCoralM2State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
