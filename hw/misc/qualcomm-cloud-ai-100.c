/*
 * Qualcomm Cloud AI 100 (Inference Accelerator, 400 TOPS INT8)
 * Vendor: 0x17CB, Device: 0xA100
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device qualcomm-cloud-ai-100
...
#define NPU_DEV    0xA100
#define NPU_SUBSYS_V 0x17CB
#define NPU_SUBSYS_D 0xA100
#define NPU_REV    0x01
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_QUALCOMMCLOUDAI100 "qualcomm-cloud-ai-100"
OBJECT_DECLARE_SIMPLE_TYPE(QualcommCloudAi100State, QUALCOMMCLOUDAI100)
struct QualcommCloudAi100State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    QualcommCloudAi100State *s = QUALCOMMCLOUDAI100(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "qualcomm-cloud-ai-100-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="qualcomm-cloud-ai-100",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,QualcommCloudAi100State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Qualcomm Cloud AI 100 (Inference Accelerator, 400 TOPS INT8)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_QUALCOMMCLOUDAI100,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(QualcommCloudAi100State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
