/*
 * MediaTek APU v5 (Dimensity 9300, 8 TOPS INT8)
 * Vendor: 0x14C3, Device: 0x7928
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device mediatek-apu-v5
...
#define NPU_DEV    0x7928
#define NPU_SUBSYS_V 0x14C3
#define NPU_SUBSYS_D 0x7928
#define NPU_REV    0x00
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_MEDIATEKAPUV5 "mediatek-apu-v5"
OBJECT_DECLARE_SIMPLE_TYPE(MediatekApuV5State, MEDIATEKAPUV5)
struct MediatekApuV5State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    MediatekApuV5State *s = MEDIATEKAPUV5(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "mediatek-apu-v5-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="mediatek-apu-v5",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,MediatekApuV5State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="MediaTek APU v5 (Dimensity 9300, 8 TOPS INT8)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_MEDIATEKAPUV5,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(MediatekApuV5State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
