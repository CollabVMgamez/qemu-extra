/*
 * Qualcomm AI Engine (QCS6490, 12 TOPS INT8)
 * Vendor: 0x17CB, Device: 0x0311
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device qualcomm-ai-engine-qcs6490
...
#define NPU_DEV    0x0311
#define NPU_SUBSYS_V 0x17CB
#define NPU_SUBSYS_D 0x0311
#define NPU_REV    0x01
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_QUALCOMMAIENGINEQCS6490 "qualcomm-ai-engine-qcs6490"
OBJECT_DECLARE_SIMPLE_TYPE(QualcommAiEngineQcs6490State, QUALCOMMAIENGINEQCS6490)
struct QualcommAiEngineQcs6490State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    QualcommAiEngineQcs6490State *s = QUALCOMMAIENGINEQCS6490(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "qualcomm-ai-engine-qcs6490-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="qualcomm-ai-engine-qcs6490",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,QualcommAiEngineQcs6490State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Qualcomm AI Engine (QCS6490, 12 TOPS INT8)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_QUALCOMMAIENGINEQCS6490,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(QualcommAiEngineQcs6490State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
