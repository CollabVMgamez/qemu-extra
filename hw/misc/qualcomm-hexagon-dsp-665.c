/*
 * Qualcomm Hexagon DSP v66 (SA8155P, AI Accelerator)
 * Vendor: 0x17CB, Device: 0x0303
 * Class: 0x1180 (Processing Accelerator / Neural Processor)
 * Usage: -device qualcomm-hexagon-dsp-665
...
#define NPU_DEV    0x0303
#define NPU_SUBSYS_V 0x17CB
#define NPU_SUBSYS_D 0x0303
#define NPU_REV    0x01
#define NPU_CLASS  0x1180
#define BAR0_SIZE (4*MiB)
#define TYPE_QUALCOMMHEXAGONDSP665 "qualcomm-hexagon-dsp-665"
OBJECT_DECLARE_SIMPLE_TYPE(QualcommHexagonDsp665State, QUALCOMMHEXAGONDSP665)
struct QualcommHexagonDsp665State {
...
static void dev_realize(PCIDevice *pdev, Error **errp) {
    QualcommHexagonDsp665State *s = QUALCOMMHEXAGONDSP665(pdev);
    ...
    memory_region_init_io(&s->bar0, OBJECT(s), &bar0_ops, s, "qualcomm-hexagon-dsp-665-mmio", BAR0_SIZE);
...
static const VMStateDescription vms = {
    .name="qualcomm-hexagon-dsp-665",.version_id=1,.minimum_version_id=1,
    .fields=(const VMStateField[]){VMSTATE_PCI_DEVICE(parent_obj,QualcommHexagonDsp665State),VMSTATE_END_OF_LIST()}};
static void ci(ObjectClass *k, const void *d) {
    ...
    dc->desc="Qualcomm Hexagon DSP v66 (SA8155P, AI Accelerator)"; dc->vmsd=&vms; dc->hotpluggable=false;
    set_bit(DEVICE_CATEGORY_MISC,dc->categories);
}
static const TypeInfo ti={.name=TYPE_QUALCOMMHEXAGONDSP665,.parent=TYPE_PCI_DEVICE,.instance_size=sizeof(QualcommHexagonDsp665State),.class_init=ci,.interfaces=(InterfaceInfo[]){{INTERFACE_CONVENTIONAL_PCI_DEVICE},{}}};
